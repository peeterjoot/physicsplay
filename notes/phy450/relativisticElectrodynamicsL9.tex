...
\begin{align}\label{eqn:relativisticElectrodynamicsL9:710}
...
\subsection{An aside.  Dimensions of $u^i$}
...
\begin{align}\label{eqn:relativisticElectrodynamicsL9:320}
A^i &= (\phi, \BA) \\
...
Equating these, and switching to coordinates for \ref{eqn:relativisticElectrodynamicsL9:360}, we have
...
\frac{d}{dt} m \gamma v^\alpha = e \underbrace{\left( - \inv{c} \PD{t}{A^\alpha} - \PD{x^\alpha}{\phi} \right)}_{E^\alpha} + \frac{e}{c} v^\beta \left( \PD{x^\alpha}{A^\beta} - \PD{x^\beta}{A^\alpha} \right)
\end{equation}
...
we can verify the identity \ref{eqn:relativisticElectrodynamicsL9:420} by expanding
...
Observe that this is almost a relativistic equation, but we aren't going to get to the full equation yet.  The energy component can be obtained from
...
where $\chi$ is a Lorentz scalar.  This $\PDi{x^i}{}$ is the four gradient.  Let's see this
...
\subsection{Gauge invariance of $A \cdot A$ action}

Now that we know what gauge invariance means, let's look at the portion of the potential action \ref{eqn:relativisticElectrodynamicsL9:220} discarded because it was not gauge invariant.  Under gauge transformation this becomes
...
Without the proper velocity term we don't have a way to simply re-pack the chain rule expansion and eliminate the last two terms as we did with the Lorentz force action.
