...
\subsection{An aside.  Dimensions of \texorpdfstring{$u^i$}{u i}}
...
\frac{d}{dt} m \gamma v^\alpha = e \myMathWithDescription{\left( - \inv{c} \PD{t}{A^\alpha} - \PD{x^\alpha}{\phi} \right)}{$E^\alpha$}{1} + \frac{e}{c} v^\beta \left( \PD{x^\alpha}{A^\beta} - \PD{x^\beta}{A^\alpha} \right)
\end{equation}
...
\subsection{Gauge invariance of \texorpdfstring{$A \cdot A$}{A squared} action}

Now that we know what gauge invariance means, let us look at the portion of the potential action \ref{eqn:relativisticElectrodynamicsL9:220} discarded because it was not gauge invariant.  Under gauge transformation this becomes
