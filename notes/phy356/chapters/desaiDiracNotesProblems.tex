...
\begin{equation}\label{eqn:desaiDiracNotes:2}
U = (1 + iK)\frac{1}{1-iK},
\end{equation}
...
From \eqnref{eqn:desaiDiracNotes:2} we have
...
\begin{equation}\label{eqn:desaiDiracNotes:3}
K = i \frac{ U - 1 }{U + 1}
\end{equation}
...
Therefore, provided \(2 + U + U^\dagger \ne 0\) (if it does we only showed that \(0 = 0\)), the operator \(K\) is Hermitian.  The expression \eqnref{eqn:desaiDiracNotes:3} then allows any unitary operator to be expressed as the fraction \eqnref{eqn:desaiDiracNotes:2}.
...
\begin{equation}\label{eqn:desaiDiracNotes:10}
C = -i \ln ( U ).
\end{equation}
...
so the logarithm of \eqnref{eqn:desaiDiracNotes:10} can be reduced, and we are left with

\begin{equation}\label{eqn:desaiDiracNotes:11}
...
We still have to show that \(C\) as defined in \eqnref{eqn:desaiDiracNotes:11} is Hermitian.  At a glance it looks like this may be anti Hermitian \(C^\dagger = -C\), but we really need a characterization of the eigenvalues to say.  That conjugate is

\begin{equation}\label{eqn:desaiDiracNotes:11b}
...
\begin{equation}\label{eqn:desaiDiracNotes:12}
U = 
\begin{bmatrix}
\cos\theta & \sin\theta \\
-\sin\theta & \cos\theta
...
\begin{equation}\label{eqn:desaiDiracNotes:12b}
\begin{aligned}
V &= \inv{\sqrt{2}} 
\begin{bmatrix}
1 & 1 \\
...
\begin{equation}\label{eqn:desaiDiracNotes:13}
U = e^{i \sigma_2 \theta},
\end{equation}

does in fact recover \eqnref{eqn:desaiDiracNotes:12}.  This follows directly from \((i \sigma_2)^2 = -I\), allowing us to write

\begin{equation}\label{eqn:desaiDiracNotes:14}
U = \cos\theta I + i \sigma_2 \sin\theta.
\end{equation}

Okay.  With that example worked out, we come to the conclusion that the operator specified in \eqnref{eqn:desaiDiracNotes:11}, can be Hermitian.
...
\begin{equation}\label{eqn:desaiDiracNotes:11c}
\Sigma = 
{\begin{bmatrix}
...
\begin{equation}\label{eqn:desaiDiracNotes:11d}
\begin{aligned}
\ln \Sigma 
&= 
{\begin{bmatrix}
...
\begin{equation}\label{eqn:desaiDiracNotes:11e}
...
If 
\begin{equation}\label{eqn:desaiDiracNotes:4}
U = A + iB,
\end{equation}
...
\begin{equation}\label{eqn:desaiDiracNotes:5}
\begin{aligned}
A &=
\inv{2} ( U + U^\dagger ) \\
B &=
...
We can also use the spectral decomposition of \(U\) and \(C\) above in \eqnref{eqn:desaiDiracNotes:11}, to write
...
\begin{equation}\label{eqn:desaiDiracNotes:60}
A = 
\ket{\alpha}\bra{\alpha}
...
\begin{equation}\label{eqn:desaiDiracNotes:62}
\ket{\alpha}
+ \lambda \ket{\beta}
...
\begin{equation}\label{eqn:desaiDiracNotes:63}
\begin{aligned}
1 + a \lambda^\conj &= b \\
\lambda \pm a &= b a.
\end{aligned}
...
\begin{equation}\label{eqn:desaiDiracNotes:64}
...
\begin{equation}\label{eqn:desaiDiracNotes:65}
\begin{aligned}
1 + a &= (1 + a ) a \\
1 - a &= (1 + a ) a
\end{aligned}
...
\begin{equation}\label{eqn:desaiDiracNotes:66}
\begin{aligned}
a &= \pm 1 \\
a &= \pm \sqrt{2} - 1
\end{aligned}
...
\begin{equation}\label{eqn:desaiDiracNotes:64i}
i \pm a = (1 - i a) a,
\end{equation}

or separately
\begin{equation}\label{eqn:desaiDiracNotes:64ii}
\begin{aligned}
a^2 + 1 &= 0 \\
%a^2 + 2 a i + 1 &= 0 
(a + i)^2 + 2 &= 0
...
\begin{equation}\label{eqn:desaiDiracNotes:66i}
\begin{aligned}
a &= \pm i \\
a &= i (-1 \pm \sqrt{2} ),
\end{aligned}
...
\begin{equation}\label{eqn:desaiDiracNotes:66j}
\begin{aligned}
b &= 1 \pm 1 \\
b &= \pm \sqrt{2}.
\end{aligned}
...
\begin{aligned}%\label{eqn:desaiDiracNotes:70}
...
\begin{equation}\label{eqn:desaiDiracNotes:71}
\{ A \} = 
\begin{bmatrix}
1 & \lambda^\conj \\
\lambda & \mu
...
\begin{equation}\label{eqn:desaiDiracNotes:72}
\begin{aligned}
\sigma &= 2,0 \\
...
\begin{equation}\label{eqn:desaiDiracNotes:72i}
\ket{\sigma_2} = \inv{\sqrt{2}}\left( \ket{\alpha} + \ket{\beta} \right),
\end{equation}

and for the \(\sigma=0\) case we have

\begin{equation}\label{eqn:desaiDiracNotes:72ii}
\ket{\sigma_0} = \inv{\sqrt{2}}\left( \ket{\alpha} - \ket{\beta} \right).
\end{equation}

Ignoring the normalization constant used here, this is consistent with \eqnref{eqn:desaiDiracNotes:67} as it should be.

\item \(\lambda = 1\), \(\mu = -1\)

\begin{equation}\label{eqn:desaiDiracNotes:73}
\begin{aligned}
\sigma &= \pm \sqrt{2} \\
...
\begin{equation}\label{eqn:desaiDiracNotes:73i}
\ket{\sigma_{\pm}} = \ket{\alpha} + (-1 \pm \sqrt{2}) \ket{\beta}
\end{equation}

This is consistent with \eqnref{eqn:desaiDiracNotes:68} as expected.

\item \(\lambda = i\), \(\mu = 1\)

\begin{equation}\label{eqn:desaiDiracNotes:74}
\begin{aligned}
\sigma &= 2,0 \\
...
\begin{equation}\label{eqn:desaiDiracNotes:74i}
\ket{\sigma_0} = \inv{\sqrt{2}}\left( i\ket{\alpha} + \ket{\beta} \right)
\end{equation}

\begin{equation}\label{eqn:desaiDiracNotes:74ii}
\ket{\sigma_2} = \inv{\sqrt{2}}\left( -i\ket{\alpha} + \ket{\beta} \right)
\end{equation}

Checking against \eqnref{eqn:desaiDiracNotes:67ip} shows that \(\ket{\sigma_2}\) above only differs by a constant as expected.

\item \(\lambda = i\), \(\mu = -1\)

\begin{equation}\label{eqn:desaiDiracNotes:75}
\begin{aligned}
\sigma &= \pm \sqrt{2} \\
...
\begin{equation}\label{eqn:desaiDiracNotes:75i}
\ket{\sigma_{\pm}} = \ket{\alpha} + i (-1 \pm \sqrt{2}) \ket{\beta}.
\end{equation}

This matches the previous calculation summarized by \eqnref{eqn:desaiDiracNotes:67in}.
