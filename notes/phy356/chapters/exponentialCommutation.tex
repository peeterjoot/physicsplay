...
Let us perform the school boy exercise to prove \ref{eqn:exponentialCommutation:1} and explore the restrictions for such a proof.  We assume a power series definition of the exponential operator, and do not assume the values $x,y$ are numeric, instead just that they can be multiplied.  A commutative multiplication will not be assumed.
...
To attempt to put this into $e^{x + y}$ form we will need to change the order that we evaluate the double sum, and here a picture (\ref{fig:gridSummation}) is helpful.

\imageFigure{figures/gridSummation}{Double sum diagonal ordering}{fig:gridSummation}{0.4}
