...
\subsubsection{Probability per unit length at \texorpdfstring{$x=0$}{x equal zero}}
...
\sum_n
\Abs{A_n}^2 (H_n(0))^2  \\
&\quad +2 \sum_{m < n} H_n(0) H_m(0) \left( \Re ( A_n A_m^\conj ) \cos( (n - m)\omega t_0) +\Im ( A_n A_m^\conj ) \sin( (n - m)\omega t_0)
\right)
\end{aligned}
\end{equation}
