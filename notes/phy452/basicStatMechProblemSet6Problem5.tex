...
=
z e^{-x} \sum_{s = 0}^\infty (-1)^s \lr{ \barz e^{-x} }^s,
\end{equation}
...
\paragraph{Integrating the pressure}
...
e^{-x(1 + s)} \\
&=
\frac{3 \theta e^{(s+1)/\theta}}{(s + 1)^2} K_2\lr{ \frac{s+1}{\theta } } \\
&=
...
Plotting this sum in \cref{fig:huang93:huang93Fig5sv} numerically to 10 terms, shows that we have a function that appears roughly polynomial in $\barz$ and $\theta$.

\imageFigure{huang93Fig5sv}{Pressure to ten terms in $\barz$ and $\theta$}{fig:huang93:huang93Fig5sv}{0.3}

\imageFigure{huang93Fig3sv}{Pressure summands}{fig:huang93:huang93Fig3}{0.3}
...
%
%Plotting the sum in \cref{fig:huang93:huang93Fig4sv} numerically (up to 10 terms, with no visible change after that for small $\barz$ (like 0.2)), we end up with something that looks fairly exponential
%\imageFigure{huang93Fig4sv}{Ten terms of pressure sum with $\barz = 1$}{fig:huang93:huang93Fig4sv}{0.3}

\paragraph{Integrating the number density by parts}
...
\barz^{s + 1} 
\int_{0}^\infty dx
\lr{ (\theta x + 1)^2 - 1}^{3/2} e^{-x(s + 1)}
=
3 \theta^2
\sum_{s = 0}^\infty
\binom{-2}{s}
\frac{\lr{ \barz e^{1/\theta} }^{s + 1}}{(s + 1)^2}
K_2\lr{ \frac{s+1}{\theta } }.
\end{dmath}
...
It's kind of odd seeming that this is quadradic in temperature.  Is there an error?

\paragraph{Integrating the energy by parts}
...
\sum_{s=0}^\infty \binom{-2}{s}
   \int_{0}^\infty dx \lr{(\theta x + 1)^2 - 1}^{3/2}
   \lr{
      (x - 1)\barz e^{-x} - \barz^2 e^{-2 x}
   } 
(\barz e^{-x})^s
...
and looks like \cref{fig:huang93:huang93Fig6sv}.  Series expansion shows that this hypergeometricU function has a $\theta^{3/2}$ singularity at the origin

\imageFigure{huang93Fig6sv}{Plot of HypergeometricU, and with $\theta^5$ scaling}{fig:huang93:huang93Fig6sv}{0.3}
...
It appears that I've picked up an extra factor of $\theta$ somewhere, but at least I've got the $2/3$ low density expression.  Given that I've Taylor expanded everything anyways around $\barz$ and $\theta$ this could likely have been done right from the get go, instead of dragging along the messy geometric integrals.

\paragraph{Low density result, take II}

I don't see where I've made the error that dragged in an extra factor of $\theta$.  Given that this end result was obtained by tossing all but the lowest order terms in $\theta$ and $\barz$, let's try that right from the get go.
...
=
\theta^{3/2} x^{3/2} 
2^{3/2} \lr{ 1 + \frac{\theta x}{2} }^{3/2}
= 
2 \sqrt{2} \theta^{3/2} x^{3/2} \lr{ 1 + \frac{\theta x}{2} }^{3/2}
\approx
...
\begin{equation}\label{eqn:huang93:1300}
\myBoxed{
P V = \frac{2}{3} U.
}
\end{equation}
