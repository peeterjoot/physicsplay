...
For a given density $\rho$, we can find the Fermi energy in the same way as we did for the non-relativisitic energies, with the exception that we have to integrate from a lowest energy of $\epsilon_0$ instead of $0$ (the energy at $\Bp = 0$).  That is
...
but before evaluating anything, let's make our change of variables for integration over all the system energies.  In spherical coordinates

\begin{dmath}\label{eqn:huang93:380}
d^3 (c \Bp)
= 2 \pi (c p)^2 d (c p) \sin\theta d\theta
= 2 \pi (c p)^2 \frac{d \epsilon}{d (cp)} d \epsilon \sin\theta d\theta.
\end{dmath}
...
= \frac{
\inv{ z^{-1} e^{\beta \epsilon} + 1 }
\epsilon \sqrt{ \epsilon^2 - \epsilon_0^2} d \epsilon \sin\theta d\theta
}
{
2 \int_{\epsilon_0}^\infty \inv{ z^{-1} e^{\beta \epsilon} + 1 }
...
{
   2 \int_{1}^\infty \inv{ z^{-1} e^{\beta \epsilon_0 x} + 1 }
   x \sqrt{ x^2 - 1} dx
}
= \frac{n \epsilon_0}{3}
\frac{
   \int_{0}^\infty
   \lr{ (y + 1)^2 - 1}^{3/2}
   \inv{ z^{-1} e^{\beta \epsilon_0 (y + 1)} + 1 }
   dy
}
{
   2 \int_{0}^\infty \inv{ z^{-1} e^{\beta \epsilon_0 (y + 1)} + 1 }
   (y + 1) \sqrt{ (y + 1)^2 - 1} dy
}.
\end{dmath}

%Now for the average energy difference from $\epsilon_0$.
