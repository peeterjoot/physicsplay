...
Solving for $\epsilon_{\mathrm{F}}/\epsilon_{\mathrm{F}}$ we have
...
\imageFigure{huang93Fig2}{Modified Bessel function of the second kind}{fig:huang93:huang93Fig2}{0.3}
...
(-1)^s
\frac{
\lr{ \barz e^{\kB T/\epsilon_0} }^{s + 1}
}{s + 1}
K_1\left( (s+1) \kB T \right).
\end{equation}

With the Bessel function dying off fast, but a product of a linear and exponential terms in front, its not at all obvious what the asymptotic behaviour of these summands are.  These are plotted in \cref{fig:huang93:huang93Fig3} to give us a bit of a clue.

\imageFigure{huang93Fig3}{Pressure summands $\frac{T k_B}{s + 1} e^{T k_B(s + 1)/\epsilon_0} K_1\left((s+1) T k_B\right)$, with $\epsilon_0 = 1$}{fig:huang93:huang93Fig3}{0.3}
