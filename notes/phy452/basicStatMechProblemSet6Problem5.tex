...
e^{-x(1 + s)} 
=
\frac{3 \theta}{(s + 1)^2} K_2\lr{ \frac{s+1}{\theta } },
\end{equation}
...
\paragraph{Busted attempt to integrate the number density by parts}

For the number density, it initially appears that we can evaluate the integral using integration from parts applied to \eqnref{eqn:huang93:730}
...
However, expanding in series as before gives us

\begin{equation}\label{eqn:huang93:920}
\frac{n}{n_0}
=
-
\theta
\sum_{s = 0} (-\barz)^s \int_{0}^\infty dx
\lr{ (\theta x + 1)^2 - 1}^{3/2} e^{-x s}
=
-
\theta
\sum_{s = 0} (-\barz)^s \int_{0}^\infty dx
...
This is no longer nicely behaved, since the $s = 0$ integral does not converge!  Is there an invalid assumption here, or is the expression for $n$ wrong?
