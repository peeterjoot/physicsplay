...
With $\epsilon_{\Bk} = \hbar^2 \Bk^2/2m$, we have $k_\alpha = \pi m_\alpha/L$, and can make an integral approximation of the sum
...
This is illustrated in \cref{fig:lecture15:lecture15Fig2}, where the smearing that occurs with increasing temperature is also indicated.
...
\footnote{\citep{kittel1980thermal} \S 7 calls $E_F$ as defined above the \underlineAndIndex{Fermi energy}, the energy at which all the lowest order orbitals are exactly filled.  A discussion of why this energy is being equated with the $T = 0$ potential energy can be found in \citep{pathriastatistical} \S 8.1.  Specifically, when the integral approximation is performed exactly, we see that at $T = 0$, the illustrated cutoff occurs at this $E_F = \mu(0)$ point.}
