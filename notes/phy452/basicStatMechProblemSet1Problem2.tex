...
To determine the limiting behaviour, we can utilize the Central limit theorem.  We first have to calculate the mean and the variance for the $N=1$ case.  The first two moments are
...
As a check we note that for $r = 1/2$ we have $8 r(1-r) = 2$ and $1 - 2r = 0$, so we get 

\begin{equation}\label{eqn:basicStatMechProblemSet1Problem2:250}
P_N(1/2, X) \rightarrow \inv{ \sqrt{2 \pi N }} \exp
\left(
- \frac{ X^2}{2 N }
\right).
\end{equation}

This is $1/2$ times \ref{eqn:basicStatMechProblemSet1Problem1:180} (also in the class notes).  I'd have expected an exact match, but it appears this Central limit theorem derived result has the correct unit normalization and the quoted result from class, also derived above, is not properly normalized.
}
