...
For the lens illustrated in figure (\ref{fig:modernOpticsProblemSet1Attempt1:modernOpticsProblemSet1Attempt1Fig1a}), use geometrical arguments to derive the image location formula

\imageFigure{modernOpticsProblemSet1Fig1a}{Thin paraxial lens with image in input and output conjugate planes}{fig:modernOpticsProblemSet1Attempt1:modernOpticsProblemSet1Attempt1Fig1a}{0.3} 

\begin{equation}\label{eqn:modernOpticsProblemSet1Attempt1:n}
\inv{s} + \inv{s'} = \inv{f}
\end{equation}
}
...
Because this describes a paraxial system, these angles approximate the tangents, so we can recast \ref{eqn:modernOpticsProblemSet1Attempt1:1090} and \ref{eqn:modernOpticsProblemSet1Attempt1:1150} as
...
\makeproblem{Demonstrate the equivalenance of Newton's lens focus formula to the inverse distance result shown above}{modernOpticsProblemSet1Attempt1:pr:2}{

Also referring to the figure where the distances $x$ and $x'$ were labelled, show that this is equivalent to
...
In the class notes, the magnification of the thin lens system described by \ref{eqn:modernOpticsProblemSet1Attempt1:1350} was given by

\begin{equation}\label{eqn:modernOpticsProblemSet1Attempt1:1370}
m = -\frac{s'}{s}.
\end{equation}

Other than the negation, this is a logical definition, the ratio of the output size with respect to the input size.  I'm guessing that this is defined as negated because the image is inverted.  From \ref{eqn:modernOpticsProblemSet1Attempt1:1270} we have

\begin{equation}\label{eqn:modernOpticsProblemSet1Attempt1:1390}
\frac{b}{a} = \frac{x'}{f}.
\end{equation}

Dividing the last two equalities in \ref{eqn:modernOpticsProblemSet1Attempt1:1210a} we have
...
and that figure (\ref{fig:modernOpticsProblemSet1Attempt1:modernOpticsProblemSet1Attempt1Fig1a}) with the distances as labeled describes the same system.  The remainder of the task is therefore algebraic.  We have
