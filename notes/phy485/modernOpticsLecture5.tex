...
\item Distance of observation $r$
\item Sources $r_s$
\end{itemize}

\begin{equation}\label{eqn:modernOpticsLecture5:30}
...
since this exponential could matter very much.  Recalling that $\BR = \Br - \Br'$ and expanding in Taylor series to second order \footnote{I've used $1/\Br$ here, which is a valid construction in Geometric Algebra. Assuming that this notational freedom is not familiar, this can just be thought of as a shorthand for $1/\Br = \rcap/r$.}, we have
...
= r \sqrt{ 1 + \frac{{\Br'}^2}{\Br^2} - 2 \inv{\Br} \cdot \Br' }
\approx 
r 
\left( 
1 + \inv{2} 
\left( \frac{{\Br'}^2}{\Br^2} - 2 \inv{\Br} \cdot \Br' \right)
-\inv{8} \left( \frac{{\Br'}^2}{\Br^2} - 2 \inv{\Br} \cdot \Br' \right)^2
+ \cdots
\right)
=
r 
\left( 
1 
+ \inv{2} \frac{{\Br'}^2}{\Br^2} - \inv{\Br} \cdot \Br' 
-\inv{8} \frac{{\Br'}^4}{\Br^4} 
-\inv{2} \left( \inv{\Br} \cdot \Br' \right)^2
+ \inv{2} \frac{{\Br'}^2}{\Br^2} \inv{\Br} \cdot \Br' 
+ \cdots
\right)
=
...
so that for $\theta \approx 0$, in the region illustrated in figures (\ref{fig:modernOpticsLecture5:modernOpticsLecture5Fig1}) and (\ref{fig:modernOpticsLecture5:modernOpticsLecture5Fig2}).
\imageFigure{figures/modernOpticsLecture5Fig1}{Obliquity factor}{fig:modernOpticsLecture5:modernOpticsLecture5Fig1}{0.4}
\imageFigure{figures/modernOpticsLecture5Fig2}{Obliquity factor}{fig:modernOpticsLecture5:modernOpticsLecture5Fig2}{0.2}
...
Referring to figure (\ref{fig:modernOpticsLecture5:modernOpticsLecture5Fig3}) we find
\imageFigure{figures/modernOpticsLecture5Fig3}{Defining k vectors}{fig:modernOpticsLecture5:modernOpticsLecture5Fig3}{0.2}
...
where $g(\Br')$ is an ``aperture'' function defined in the open portion as illustrated in figure (\ref{fig:modernOpticsLecture5:modernOpticsLecture5Fig4}).

\imageFigure{figures/modernOpticsLecture5Fig4}{Circular aperture}{fig:modernOpticsLecture5:modernOpticsLecture5Fig4}{0.2}
...
Calculating this for a circular pattern we find a sinc function, and for a square (?), we'll get a first order Bessel function.  (FIXME: was that right?).  We can deal with double slit by doing a convolution of a rectangle aperture with a pair of delta functions and then just multiply the Fourier transforms.
