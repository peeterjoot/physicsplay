...
\bcE_{\mathrm{L}} = E_{\mathrm{L}} e^{i \phi(t)}
\end{dmath}

(underscore L stands for laser).

\begin{dmath}\label{eqn:modernOpticsLecture21:140}
\Psi = \bcE_{\mathrm{L}} e^{-i \omega t} \times \mbox{spatial mode}
\end{dmath}

Random relative phase \(\theta\).  Two components of \(\bcE_{\mathrm{sp}}\)

\begin{enumerate}
\item Parallel to \(\bcE_{\mathrm{L}}\) changes amplitude, taken out by steady state operation of laser.
...
\bcE_{\mathrm{L}}' \sim \bcE_{\mathrm{L}} e^{i \delta \phi}
\end{dmath}
