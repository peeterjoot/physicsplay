...
From this we deduce the wave velocity is $c/\sqrt{\epsilon}$, and then find $n = c/v$ matches \ref{eqn:modernOpticsLecture2:5} above.
}
...
In \eref{modernOpticsLecture2:pr2} we show that 
...
From back to Maxwell's equation, in particular \ref{eqn:modernOpticsLecture2:190}, we have
...
Derive \ref{eqn:modernOpticsLecture2:390}. }
...
Now we use Eikonal equation \ref{eqn:modernOpticsLecture2:230} 
...
\makeproblem{Derive the Eikonal equation with Geometric Algebra}{modernOpticsLecture2:pr4}{ Express \ref{eqn:modernOpticsLecture2:140} in it's Geometric Algebra form, and figure out how \ref{eqn:modernOpticsLecture2:230} follows from that directly.
}
...
Good, this matches with \ref{eqn:modernOpticsLecture2:140} as derived in class.
...
Note that it almost looks like \ref{eqn:modernOpticsLecture2:1110b} could be observed directly from \ref{eqn:modernOpticsLecture2:1130a}.  However, at that point we hadn't actually proved that $\kcap$ was a unit vector, but had sneakily just used notation that implied it.

Having shown now that $\kcap$ is a unit vector, and because \ref{eqn:modernOpticsLecture2:1030b} is now no longer any different than the result for linear media with a phasor assumed to be of the form $F_0 e^{i \Bk \cdot \Bx - i \omega t}$, we must also have as the general solution to Maxwell's equation given our approximation
