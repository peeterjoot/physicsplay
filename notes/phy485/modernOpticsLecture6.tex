...
In the Fraunhofer limit (the far field) we found \cref{eqn:modernOpticsLecture5:270} that 
...
We use \footnote{Can be computed by integrating $\cos(\pi v^2/2) + i \sin(\pi v^2/2)$, or using the Gaussian integral result.  FIXME: am guessing that Gaussian here meant error function.}
...
Parametrically plotting these we get the \underline{Cornu Spiral} as plotted in \cref{fig:modernOpticsLecture6:modernOpticsLecture6Fig4}.

\imageFigure{figures/modernOpticsProblemSet2Problem2CornuSpiralPlainPlotFig4a}{Cornu Spiral}{fig:modernOpticsLecture6:modernOpticsLecture6Fig4}{0.4}
...
In general, for a barrier offset by $d$, and a value of $w$ that cooresponds to that, our \underline{Intensity} is
