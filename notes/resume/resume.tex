%
% Copyright © 2013 Peeter Joot.  All Rights Reserved.
% Licenced as described in the file LICENSE under the root directory of this GIT repository.
%
\input{../blogpost.tex}
\renewcommand{\basename}{WHAT}
\renewcommand{\dirname}{notes/FIXMEwheretodirname/}
%\newcommand{\dateintitle}{}
%\newcommand{\keywords}{}

\input{../peeter_prologue_print2.tex}

\beginArtNoToc

\generatetitle{Curriculum Vitae}
%\chapter{Curriculum Vitae}
%\label{chap:WHAT}
\paragraph{Address}\mbox{} \\
493 White's Hill Ave\par
Markham, Ontario\par
L6B 0J8\par
(416) 305-9441

\paragraph{Education}\mbox{ } \\

\begin{itemize}
\item BASc. Engineering Science ; Computer Engineering, University of Toronto
Conferred 1997
\item Selected physics courses. University of Toronto.
2010 - present. Quantum Physics I, Relativistic Electrodynamics, Quantum Physics II, Con-
tinuum Mechanics, Advanced Classical Optics, Basic Statistical Mechanics, (in progress) Con-
densed Matter Physics
\end{itemize}

\paragraph{Work history}\mbox{ } \\

1997-present. IBM Toronto Lab. Software development for DB2, including

\begin{itemize}
\item Implemented multithread support for DB2¿s Unix client.
\item DB2 Linux porting work and simultaneous reengineering of the Unix portions of our codebase for portability.
\item DB2 64-bit port. This was a project of massive scope, taking our codebase from it¿s 16-bit and 32-bit origins to its current multi-terrabyte consuming state. If you have the RAM we can now eat it.
\item Development and maintenance of DB2¿s internal mutex and atomic implementations, and associated memory ordering mechanisms.
\item Maintenance and implementation of the platform portions of stack collection and other post mortem facilities.
\item Lock-free reimplementation of DB2¿s reader-writer mutex. The performance of our original reader-writer mutex code sucked for a number of reasons (including but not restricted to an exclusive mutex used internally). This was a from scratch implementation where we used a single atomic to track the reader and writer linked lists (indirectly), the reader count, the writer held bit, and a writer reserved bit. This code has stood the test of time and remains one of my proudest products, not only not sucking for performance, but remaining correct despite the massive complexities and range of potential timing holes if done wrong.
\item Implementation of DB2¿s asynchronous IO abstraction layer. It is hard to believe that it was not that long ago that we did all our IO synchronously. This bit of code hides an impressive amount of operating system centric code from our high level development consumers, while squeezing maximum performance from each system.
\item Development and maintenance of many other aspects of DB2¿s operating system abstraction layer.
\item LeadofprojectbranchintegrationteamduringinternaltransitionfromCMVCVCStoclearcase.
\item Ad-hoc build tooling and makefile maintenance as required.
\item Technical owner of DB2¿s coding standards
\item DB2 pureScale project (distributed shared disk database): Implemented various duplexing, failover and reconstruct aspects of the communications between the DB2 engine and the shared buffer pool and lock manager component.
\end{itemize}

Work history prior to 1997 was primarily in construction trades.

\paragraph{Writing}\mbox{ } \\

\begin{itemize}
\item Exploring physics with Geometric Algebra
\item Miscellaneous Physics and Math Play
\item Classical mechanics
Course notes and problems from University of Toronto 2010 (taught by Prof Vatche Deyirmenjian) PHY356H1F Quantum Physics I
\item Course notes and problems from University of Toronto 2011 (taught by Prof Erich Poppitz)
PHY450H1S Relativistic Electrodynamics
\item Course notes and problems from University of Toronto 2011 (taught by Prof John E. Sipe)
PHY456H1F Quantum Physics II
\item Course notes and problems from University of Toronto 2012 (taught by Prof Kausik S. Das)
PHY454H1S Continuum Mechanics
\item Course notes and problems from University of Toronto 2012 (taught by Prof Joseph H. Thywissen) PHY485H1F Advanced Classical Optics
\item Course notes and problems from University of Toronto 2013 (taught by Prof Arun Paramekanti)
PHY452H1S Basic Statistical Mechanics
\item Course notes and problems from University of Toronto 2013 (taught by Prof Stephen Julian)
PHY487H1F Condensed Matter Physics
\item An attempt to illustrate differences between memory ordering and atomic access
\item A nice simple example of a memory barrier requirement
\item Timings and correctness issues for mutex release operations
\item (arxiv) Change of basis and Gram-Schmidt orthonormalization in special relativity
\item Relativistic origins of the Schrodinger equation
\item Spherical polar pendulum for one and multiple masses (using geometric algebra)
\item Lagrangian and Euler-Lagrange equation evaluation for the spherical N-pendulum problem
\end{itemize}

%- URLs got lost in cut and paste from the pdf that I had as backup after accidental deletion of .tex file.
% put these back in:
%https://sites.google.com/site/peeterjoot/math2009/gabook.pdf
%https://sites.google.com/site/peeterjoot/math2009/miscphysics.pdf
%https://sites.google.com/site/peeterjoot2/math2013/classicalmechanics.pdf
%https://sites.google.com/site/peeterjoot/math2010/phy356.pdf
%https://sites.google.com/site/peeterjoot/math2011/phy450.pdf
%https://sites.google.com/site/peeterjoot/math2011/phy456.pdf
%https://sites.google.com/site/peeterjoot2/math2012/phy454.pdf
%https://sites.google.com/site/peeterjoot2/math2012/phy485.pdf
%https://sites.google.com/site/peeterjoot2/math2013/phy452.pdf
%https://sites.google.com/site/peeterjoot2/math2013/phy487.pdf
%https://sites.google.com/site/peeterjoot/math2009/atomic.pdf
%https://sites.google.com/site/peeterjoot/math2010/atomicSimple.pdf
%http://sites.google.com/site/peeterjoot/math2010/mutexRelease.pdf
%http://arxiv.org/abs/1104.4829
%https://sites.google.com/site/peeterjoot/math2009/relwave.pdf
%https://sites.google.com/site/peeterjoot/math2009/multiPendulumSpherical2.pdf
%https://sites.google.com/site/peeterjoot/math2009/multiPendulumSphericalMatrix.pdf

See \href{https://sites.google.com/site/peeterjoot3/}{https://sites.google.com/site/peeterjoot3/} for the URLs above if this document has been printed.

%\EndArticle
\EndNoBibArticle
