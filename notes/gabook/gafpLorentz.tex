...
\section{ Covarient Lagrangian for a charged point particle? }
...
Now, is it correct that $\int \dot{p} \cdot ds$ is a correct calculation of energy relativistically?  On one hand, at least
in terms of this generalized potential it seems like a somewhat reasonable thing to do:

\begin{align*}
q \int \grad \left( A \cdot v/c \right) \cdot ds
&= q \sum \int \underbrace{\gamma^{\mu} \cdot \gamma_{\nu}}_{={\delta^{\mu}}_{\nu}} \frac{dx^{\nu}}{d\tau}\partial_{\mu} \left( A \cdot v/c \right) d\tau \\
&= q \sum \int \frac{dx^{\mu}}{d\tau}\frac{\partial \left( A \cdot v/c \right)}{\partial x^{\mu}} d\tau \\
&= q \int \frac{d \left( A \cdot v/c \right)}{d\tau} d\tau \\
&= q \left( A \cdot v/c \right) \\
\end{align*}

With $v = c\gamma_0$ in the particles rest frame this gives:

\begin{equation*}
q \left( A \cdot v/c \right) = q \phi
\end{equation*}

Could this be considered a covarient form of the potential:

If there is something to this then we have shown:
\begin{equation*}
\int \dot{p} \cdot ds
= q \int \grad \left( A \cdot v/c \right) \cdot ds
= q ( A \cdot v/c )
= -q \int \grad U \cdot ds = -(U_2 - U_1)
\end{equation*}

So it may be that we have the Lagrangian for a charged point particle in a electromagnetic field as follows:

\begin{align*}
\mathcal{L}
&= -mc^2\underbrace{\sqrt{1 - (\Bv/c)^2}}_{\gamma^{-2}} - q ( A \cdot v/c ) \\
&= -\frac{mc^2}{(v/c) \cdot \gamma_0} - q ( A \cdot v/c ) \\
\end{align*}

Looking again, I don't think this can be right, because I wouldn't expect an observer dependent term in the first energy component.

Another validation of the intuition that this guess isn't quite right comes from 
direct evaluation of that left hand integral:

\begin{equation*}
\int \dot{p} \cdot ds = m \int g_{\mu\nu} \dot{v}^{\mu} \dot{x}^{\nu} d\tau
\end{equation*}

compare to:

\begin{equation*}
\frac{d}{d\tau}(v \cdot v) = 2 \dot{v} \cdot v = 2 \sum \dot{v}^{\mu} \dot{x}^{\nu} g_{\mu\nu}
\end{equation*}

which gives:

\begin{equation*}
\int \dot{p} \cdot ds = m \inv{2} \int \frac{v \cdot v}{d\tau} d\tau = mv^2/2 = m c^2/2
\end{equation*}

This isn't anything recognizable, so I would guess the $\int \dot{p} \cdot ds$ isn't what what we want here.

It will be interesting to eventually figure out how all this fits together.
