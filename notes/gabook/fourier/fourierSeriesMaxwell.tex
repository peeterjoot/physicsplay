...
\begin{align}\label{eqn:fourier_series_maxwell:maxwell}
F &= \EE + i\HH \\
J &= (\rho + \Bj) \gamma_0 \\
\grad F &= 4 \pi J
\end{align}
...
\begin{align}\label{eqn:fourier_series_maxwell:vacuumMaxwell}
...
\begin{align}\label{eqn:fourier_series_maxwell:assumed}
...
\begin{align}\label{eqn:fourier_series_maxwell:undetermined}
F(\Bx,t) = \sum_{\Bk} 
...
\begin{align}\label{eqn:fourier_series_maxwell:bivectorSolNonGreens}
F(\Bx,t) &= \inv{V} \int \sum_{\Bk} \exp\left( i \Bk ct \right) \exp\left( i \Bk \cdot (\Bx' - \Bx) \right) F(\Bx', 0) d^3 x'
\end{align}

Or
\begin{align}\label{eqn:fourier_series_maxwell:bivectorSolution}
...
% k = kcappa g_0 |k|
% kcap g_0 = kcappa 
\begin{align}\label{eqn:fourier_series_maxwell:isItValid}
F(\Bx,t) 
...
That is
\begin{align}\label{eqn:fourier_series_maxwell:anticommutes}
e^{i\Bu} F &= 2 \sin\Bu \cdot (\EE - \HH) + F e^{-i\Bu}
\end{align}
...
\begin{align}\label{eqn:fourier_series_maxwell:energyMomentum}
U & 
\stackrel{?}{=}
...
\begin{align*}
F_{\Bk}(\Bx,t)
&= \inv{V} \int e^{ i \kcap \Abs{\Bk} ct + i \Bk \cdot (\Bx' - \Bx) } (\EE(\Bx', 0) + i\HH(\Bx', 0)) d^3 x'
\end{align*}
...
Multiplying this out with the initial time field $F(\Bx',0) = \EE(\Bx', 0) + i\HH(\Bx',0) = \EE' + i\HH'$ we can separate into grades.  

\begin{align*}
\exp&( i \Bomega t + i k \Delta x ) (\EE' + i\HH') \\
&= \cos( \Bomega t ) (\EE' \cos( k \Delta x ) -\HH' \sin( k \Delta x ) ) +  \sin( \Bomega t ) \cross ( \HH' \sin( k \Delta x ) - \EE' \cos( k \Delta x ) ) \\
&+i \cos( \Bomega t ) (\EE' \sin( k \Delta x ) + \HH' \cos( k \Delta x ) ) -i \sin( \Bomega t ) \cross (\EE' \sin( k \Delta x ) + \HH' \cos( k \Delta x ) ) \\
&-  \sin( \Bomega t ) \cdot (\EE' \sin( k \Delta x ) +\HH' \cos( k \Delta x ) ) \\
&+i( \sin( \Bomega t ) \cdot (\EE' \cos( k \Delta x ) - \HH' \sin( k \Delta x )) \\
\end{align*}
...
\begin{align*}
\inv{2} &\exp( i \Bomega t + i k \Delta x ) (\EE' + i\HH') \\
&= \cos( \Bomega t ) \EE' \cos( k \Delta x ) +  \sin( \Bomega t ) \cross \HH' \sin( k \Delta x ) \\
&+i \cos( \Bomega t ) \HH' \cos( k \Delta x ) -i \sin( \Bomega t ) \cross \EE' \sin( k \Delta x ) \\
&-  \sin( \Bomega t ) \cdot \EE' \sin( k \Delta x ) \\
&-i \sin( \Bomega t ) \cdot \HH' \sin( k \Delta x ) \\
&= (\EE' + i\HH') \cos( \Bomega t ) \cos( k \Delta x ) 
 -i \sin( \Bomega t ) \cross (\EE' + i \HH') \sin( k \Delta x ) \\
&-  \sin( \Bomega t ) \cdot (\EE'+i\HH) \sin( k \Delta x ) \\
\end{align*}
...
\inv{2} \exp( i \Bomega t + i k \Delta x ) (\EE' + i\HH') 
&= (\EE' + i\HH') \cos( \Bomega t ) \cos( k \Delta x )  \\
&-i \kcap \cross (\EE' + i \HH') \sin( \Abs{\Bomega} t ) \sin( k \Delta x ) \\
&-  \sin( \Bomega t ) \cdot (\EE'+i\HH') \sin( k \Delta x ) \\
\end{align*}
...
Now, from above, we can see that one way to reconcile this grade requirement is to require both $\kcap \cdot \EE' = 0$, and $\kcap \cdot \HH' = 0$.
How can such a requirement make sense given that $\Bk$ ranges over all directions in space, and that both $\EE'$ and $\HH'$ could conceivably 
