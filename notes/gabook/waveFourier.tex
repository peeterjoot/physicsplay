...
\author{Peeter Joot}
\date{ Jan 26, 2009.  Last Revision: $Date: 2009/01/27 03:26:57 $ }
...
This is a bit different seeming than the unit wave number dot product form, but lets see if it works.  We form
...
which sure enough equals zero, indicating that a correct guess about the delta function action of the integral kernel found 
via this Fourier transform technique.  Pretty cool.
...
\begin{align*}
\inv{({2\pi})^3} &\IIinf \exp\left( i k (x-u) + i m (y-w) + i n (z-r) \pm i \sqrt{k^2 + m^2 + n^2} v t \right) dk dm dn \\
&\sim 
\delta( x -u \pm vt )
\delta( y -w \pm vt )
\delta( z -r \pm vt )
...
\begin{align*}
\psi(x,y,z,t) &= \phi( x \pm vt, y \pm vt, z \pm vt ) \\
\end{align*}

There are also hints here of the form we see in the retarded time solutions to the charge and current density of the non-statics Maxwell wave equations under the Lorentz gauge.  That's interesting, and inspires an attempt to tackle that next.
