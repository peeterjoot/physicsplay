...
\author{Peeter Joot}
\date{ Feb 18, 2009.  Last Revision: $Date: 2009/02/21 05:13:37 $ }
...
Having done this, this integral isn't really any more well defined.  With a Rigor police on holiday, let's assume we want the
...
\begin{align*}
{\psi}(\Bx, t) 
&= \IIinf \IIinf \IIinf \IIinf \rho(\Bx - \Bx', t - t') 
%G(\Bx', t') 
u(t') \frac{c }{4\pi \Abs{\Bx'}} \left( \delta(\Abs{\Bx'} - c t') - \delta(\Abs{\Bx'} + c t') \right) 
d^3 x' dt' \\
\end{align*}
...
{\psi}_{-}(\Bx, t) &= \IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx'}} \rho(\Bx - \Bx', t - \Abs{\Bx'}/c) d^3 x'  \\
{\psi}_{+}(\Bx, t) &= -\IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx'}} \rho(\Bx - \Bx', t + \Abs{\Bx'}/c) d^3 x' 
\end{align}

It looks like my initial interpretation of the causal nature of the unit step in the original functional form wasn't really right.  In this form
we have two specific solutions to the equation.  One is due to retarded contributions (causal), and the other advanced, and the original unit step
did not have that information by itself.  Now, these unit step functions have been carelessly dropped here.  Intuitively I see that the remaining
impact of those is that we sum over previous times only for the retarded solution, and future times only for the advanced solution.  That statement
is a bit handwavyish (would have to think it though better to properly justify it), but is consistent with statements seen elsewhere, such as in 
\cite{feynman1963flp}.

It's been a couple years since seeing those for the
first time in Feynman's text, and wondering how on earth somebody would come up with those, and here we have it, albeit in a somewhat ad-hoc 
non-rigorous fashion.

\subsection{ Finishing up. }

Observe that \ref{eqn:almostThere} is not quite in the form that the retarded potentials are usually found.  For that we need one more change of
variables
\begin{align}
%{\psi}_{-}(\Bx, t) &= -\IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx -\Bu}} \rho(\Bu , t - \Abs{\Bx - \Bu}/c) d^3 u  \\
%{\psi}_{+}(\Bx, t) &= \IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx -\Bu}} \rho(\Bu , t + \Abs{\Bx - \Bu}/c) d^3 u 
{\psi}_{-}(\Bx, t) &= -\IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx -\Bx'}} \rho(\Bx' , t - \Abs{\Bx - \Bx'}/c) d^3 x'  \\
{\psi}_{+}(\Bx, t) &= \IIinf \IIinf \IIinf \frac{c }{4\pi \Abs{\Bx -\Bx'}} \rho(\Bx' , t + \Abs{\Bx - \Bx'}/c) d^3 x' 
\end{align}

But put this way, it appears that I gained a factor of $-1$ somewhere along the way.  Don't see it now.  Maybe later with fresh eyes.
