...
\chapter{Revisit Stokes derivation.}
...
\caption{Two variable parametrization of \R{n} parallelogram}\label{fig:parallelogram}
\end{figure}
...
\caption{Three variable parametrization of \R{n} parallelepiped}\label{fig:parallelopiped}
\end{figure}
...
\begin{equation}\label{eqn:stokesR:toprove2}
\int_{\partial V} F \cdot d^{k-1}\Bx 
= \int_V du_1 du_2 \cdots du_k \sum \PD{u_i}{F} \cdot (I \Br^{u_i})
\end{equation}
...
The remainder of the proof is to verify that the expression \ref{eqn:stokesR:curldot} matches the differential form in equation \ref{eqn:stokesR:toprove2}.
...
After nicely arranging the $m_i$ indexes to match equation \ref{eqn:stokesR:curldot}, the partials don't match.  Perhaps about a change of variables:
\begin{align*}
m_1 &= n_i \\
m_k &= n_1 \\
m_{k-1} &= n_2 \\
...
\section{Messy proof of zero sum in equation \ref{eqn:stokesR:toprove} }
