...
\date{ March 18, 2009.  Last Revision: $Date: 2009/03/20 01:41:03 $ }
...
Starting with a vector approach, we can express an orthonormal set of lab frame basis vectors $\{e_\mu\}$ in terms of the particle rest frame coordinates $\{\gamma_\mu\}$.  In terms of a Lorentz transform matrix ${\Lambda_\mu}^\nu$ this is

\begin{align}\label{eqn:Lambda}
e_\mu &= {\Lambda_\mu}^\nu \gamma_\nu
\end{align}

An inverse transformation, say, ${\Pi_\mu}^\nu$ can also be defined
\begin{align}\label{eqn:ILambda}
\gamma_\mu &= {\ILambda_\mu}^\nu e_\nu
\end{align}

These transformation matrixes can be expressed in terms of the pairs of frame vectors by taking dot products using the reciprocal pairs
$e^\mu \cdot e_\nu = {\delta^\mu}_\nu$, $\gamma^\mu \cdot \gamma_\nu = {\delta^\mu}_\nu$ 

\begin{align*}
e_\mu \cdot \gamma^\nu &= {\Lambda_\mu}^\alpha \gamma_\alpha \cdot \gamma^\nu = {\Lambda_\mu}^\alpha {\delta_\alpha}^\nu \\
\gamma_\mu \cdot e^\nu &= {\ILambda_\mu}^\alpha e_\alpha \cdot e^\nu = {\ILambda_\mu}^\alpha {\delta_\alpha}^\nu 
\end{align*}

This provides the Lorentz transform matrix and its inverse in terms of the pairs of frame vectors and their reciprocals 
\begin{align}
{\Lambda_\mu}^\nu &= e_\mu \cdot \gamma^\nu  \\
{\ILambda_\mu}^\nu &= \gamma_\mu \cdot e^\nu
\end{align}

From the defining relationships \ref{eqn:ILambda}, and \ref{eqn:Lambda}, we also have

\begin{align*}
e_\mu \cdot e^\nu &= {\Lambda_\mu}^\alpha \gamma_\alpha \cdot e^\nu \\
\gamma_\mu \cdot \gamma^\nu &= {\ILambda_\mu}^\alpha e_\alpha \cdot \gamma^\nu
\end{align*}

This provides the inverse relationships explicitly in terms of the index summation identities

\begin{align}\label{eqn:summation}
{\delta_\mu}^\nu 
&= {\Lambda_\mu}^\alpha {\ILambda_\alpha}^\nu = {\ILambda_\mu}^\alpha {\Lambda_\alpha}^\nu 
\end{align}

Relationships between the coordinates of a vector are also desired.  Writing
...
we want to, as in the GA case, express $y$ in terms of the Lorentz transformation between the two frames.  Suppose we write
...
Where $\LL(\gamma_\nu)$ is the Lorentz transformation \ref{eqn:ILambda}
that maps the basis vector $\gamma_\nu \rightarrow e_\nu$.

\begin{align*}
y^\alpha e_\alpha \cdot e^\mu &= x^\beta {\ILambda_\beta}^\nu e_\nu \cdot e^\mu \\
\end{align*}

which is
\begin{align}
y^\mu &= {\ILambda_\beta}^\mu x^\beta 
\end{align}

Employing the summation identity \ref{eqn:summation} 

\begin{align}
y^\alpha 
{\Lambda_\alpha}^\mu 
&= {\ILambda_\beta}^\alpha {\Lambda_\alpha}^\mu x^\beta  \\
&= {\delta_\beta}^\mu x^\beta  \\
\end{align}

So the inverse relationship for the rest frame coordinates is

\begin{align}
x^\mu  &= {\Lambda_\alpha}^\mu y^\alpha 
\end{align}

Observe that the coordinates transform in an inverted fashion in comparison to the transformation of the basis vectors.
