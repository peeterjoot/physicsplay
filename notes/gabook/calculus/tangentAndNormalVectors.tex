...
It may seem curious that we had to specify an Euclidean metric for the 3D case, but did not here.  That doesn't mean this is a metric free result.  Instead, the metric choice is built into the definition of the gradient \ref{eqn:tangentAndNormalVectors:460} and its associated reciprocal basis.  For example with a $1,3$ metric where $\gamma_0^2 = 1, \gamma_k^2 = -1$, we have $\gamma^0 = \gamma_0$ and $\gamma^k = -\gamma_k$.
