...
\begin{align}\label{eqn:vector_integral_relations:jacobianframe}
dA =
\begin{vmatrix}
\Be_1 \cdot \gamma_{1} & \Be_1 \cdot \gamma_{2} \\
\Be_2 \cdot \gamma_{1} & \Be_2 \cdot \gamma_{2}
...
\begin{align}\label{eqn:vector_integral_relations:jacobianarea}
...
\begin{align}\label{eqn:vector_integral_relations:areaintegral}
-\left(\grad \wedge \Bf\right) \cdot d\BA
&= \sum_{i<j} \left( \PD{x^{i}}{f_j} -\PD{x^{j}}{f_i} \right) \PD{(u,v)}{(x^i, x^j)} du dv \\
...
With this particular enumeration of the oriented areas this geometrically described is a ``left handed'' triple $\{ \PD{u}{\Br}, \PD{v}{\Br}, \PD{w}{\Br} \}$.
...
\begin{align}\label{eqn:vector_integral_relations:volumeareaAlmostDone}
\implies
\oiintctrclockwise F \cdot d\BA
&=
-\iiint du dv dw
