...
I will not reproduce or try to
...
that when thinking about how to prove this myself it did not occur to me.

\section{Explicit expansion for a scalar function}

Now, despite the $a \cdot \grad$ notation being unfamiliar seeming, the 
end result is not.  Explicit expansion of this for a vector to scalar mapping
...
I had seen that before the connection to the Hessian was not obvious.
...
and the function
this is applied to does not have to be a scalar function.
...
The simplest grade mixed multivector space is that of the complex numbers.  Let us write out the
...
but I did not understand the notation).

Let us do an initial comparison of the two.  We need a reminder
...
A gradient operator has been identified by factoring it out of the directional derivative.  Let us compare this to a
