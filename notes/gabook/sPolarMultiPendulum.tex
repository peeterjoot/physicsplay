...
\date{Oct 27, 2009}
\revisionInfo{$RCSfile: sPolarMultiPendulum.tex,v $ Last $Revision: 1.3 $ $Date: 2009/10/28 04:16:50 $}
...
% h = r(1+\cos\theta)
% \phi = m g h = m g r (1+\cos\theta)      %% then drop m g r constant term.
...
%For the multiple mass generalization only $R$ has to be changed, and it takes the block matrix form over columns $c$
%
%\begin{align}\label{eqn:sPolarMultiPendulum:zoo13}
%R &=
%\begin{bmatrix}
%{
%\left(\sum_{k=c}^N m_k\right) r_c
%\begin{bmatrix}
%j_c e^{j_c\theta_c} & 
%\sin\theta_c 
%\end{bmatrix}
