...
%\revisionInfo{$RCSfile: stokesNoTensor.tex,v $ Last $Revision: 1.10 $ $Date: 2009/10/22 02:07:20 $}
...
%However, we don't wish to use these Jacobians since we've seen how messy this makes things.
% ??? :
%Even when the volume element for the subspace was three dimensional, this was starting to get intractable.
...
( \grad \wedge F ) \cdot d^4 x 
=
\inv{(k-1)!} \epsilon^{ r s \cdots t u } \int \PD{F}{da_{m_d}} \cdot 
\left( 
\PD{a_r}{x} \wedge \PD{a_s}{x} \wedge \cdots \wedge \PD{a_t}{x} \right)
da_1 da_2 \cdots da_k}
\end{align}

Here the indexes have the range $\{r, s, \cdots, t, u\} \in \{1, 2, \cdots k\}$.  This is really Stokes theorem in its full glory.  Observe that in this Geometric algebra form, the one forms $dx_1, \cdots $dx_k$ are nothing more abstract that plain old vector differential elements (we would call them one forms in the formalism of differential forms).
