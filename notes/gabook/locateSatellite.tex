...
Was playing around with what is probably traditionally a spherical trig type problem using geometric algebra (locate satellite position using angle measurements from two well separated points).  Origin of the problem was just me looking at my Feynman Lectures introduction where there is a diagram illustrating how triangulation could be used to locate "Sputnik" and thought I'd try such a calculation, but in a way that I thought was more realistic.

\imageFigure{figures/satellite}{Satellite location by measuring direction from two points}{fig:satellite}{0.4}
...
&= \underbrace{\cos(\psi/2)\cos(\lambda/2)}_{a_0} \\
&\qquad + \underbrace{-\Be_{32}\sin(\psi/2)\cos(\lambda/2) - \Be_{31} \cos(\psi/2) \sin(\lambda/2)}_{A} \\
&\qquad + \underbrace{\Be_{21} \sin(\psi/2) \sin(\lambda/2)}_{B}
\end{align*}
