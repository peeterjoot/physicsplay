...
from the GA axioms (that is an easier proof without the mass of indexes and determinant products.) 
...
This is derived from equation \ref{eqn:reciprocal_frame:bladerejection}, after noting that
...
(\Bu_k \wedge \BB) \cdot \BB
&= 
\myMathWithDescription{(\Bx \BB)}{$\Bx = \Bu_k - (\Bu_k \cdot \BB) \cdot \inv{\BB}$}{1}
\cdot \BB \\
&= \gpgradeone{
\Bx\BB \BB
} \\
...
\begin{align*}
\BI \Ba 
&= \BI \cdot \Ba + \myMathWithDescription{\BI \wedge \Ba}{$=0$}{1} \\
&= (-1)^{k-1}\Ba \cdot \BI \\
&= (-1)^{k-1}\Ba \BI \\
\end{align*}
