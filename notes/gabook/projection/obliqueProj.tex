...
\[
\Bx \wedge \dcap + \alpha \myMathWithDescription{\dcap \wedge \dcap}{$=0$}{1} = \beta \pcap \wedge \dcap
\]
\[
\implies
...
Let us start at the same place as in equation \ref{eqn:oblique_proj:solveForprojLineUnit}, except that we know we can discard the unit vectors and work with any vectors in the projection directions:
...
\begin{equation*}
\Bx \wedge P + \alpha \BD \wedge P = \sum_{i=1}^k \beta_i \myMathWithDescription{\BP_i \wedge P}{$=0$}{1}
\end{equation*}
...
\begin{align*}
\Bx \cdot \BP^i
&= \myMathWithDescription{\Be \cdot \BP^i}{$=0$}{1} + \sum \beta_j \BP_j \cdot \BP^i \\
