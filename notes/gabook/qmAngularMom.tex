...
\date{July 27, 2009}
\revisionInfo{$RCSfile: qmAngularMom.tex,v $ Last $Revision: 1.1 $ $Date: 2009/07/28 04:21:24 $}
...
The choice to use the pseudoscalar for this imaginary seems a logical one and the end result is a pure bivector respresentation of angular momentum operator
...
The form of (\ref{eqn:ang}) suggests a more direct way to extract the angular momentum operator from the Hamilitonian (i.e. from the Laplacian).  Bohm uses the spherial polar representation of the Laplacian as the starting point.  Instead let's project the gradient itself in a specific constant direction $\Ba$, much as we can do to find the polar form angular velocity and acceleration components.
...
So we have for the Laplacian a reprentation in terms of projection and rejection components 
