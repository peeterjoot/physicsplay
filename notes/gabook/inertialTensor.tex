...
\end{equation}\label{eqn:kvector_orthogonal_decomp}

The dot product is required since the general product of two bivectors has grade-0, grade-2, and grade-4 terms.
...
The inverse in (\ref{eqn:kvector_orthogonal_decomp}) can be removed by reversion, and for a grade-r blade this sum of projective terms then becomes:

\begin{equation}
f(B) = (-1)^{r(r-1)/2} \frac{1}{\sigma_i \sigma_i^\dagger}\sum_i (f(B) \cdot \sigma_i) \cdot {\sigma_i}
\end{equation}
...
\end{equation}\label{eqn:kvector_orthonormal_decomp}
...
\end{equation}\label{eqn:inertia_tensor_as_dotdotdot}

As pointed out in the text this is symmetric.  That can't be more clear than in (\ref{eqn:inertia_tensor_as_dotdotdot}).
