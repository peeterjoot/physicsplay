...
\date{Sept 13, 2009}
\revisionInfo{$RCSfile: nuclearInteraction.tex,v $ Last $Revision: 1.5 $ $Date: 2009/09/15 04:35:27 $}
...
First order of business is an expression for a point charge current density four vector.  Following Jackson \cite{jackson1975cew} with a switch to SI units and vector notation we can apparently employ an arbitrary parameterization for the four-vector particle trajectory $R = R^\mu \gamma_\mu$, as measured in the observer frame, and write

\begin{align}\label{eqn:nuclearInteraction:boo7}
J(X) = q \int d\lambda \frac{dX}{d\lambda} \delta^4 (X - R(\lambda))
\end{align}
...
\begin{align*}
J(X) 
&= q \int dt \frac{dX}{dt} \delta^4 (X - R(t)) \\
&= q \int dt \frac{d}{dt} (c t \gamma_0 + \gamma_k X^k)\delta^4 (X - R(t)) \\
&= q \int dt \frac{d}{dt} (c t + \Bx)\delta^4 (X - R(t)) \gamma_0 \\
&= q \int dt (c + \Bv)\delta^4 (X - R(t)) \gamma_0 \\
&= q \int dt' (c + \Bv(t'))\delta^3 (\Bx - \Br(t')) \delta(t' - t) \gamma_0 \\
...
FIXME: maybe not quite right?  Should the split of the fourvolume delta function be $\delta^4(X - R(t) = \delta^3{\Bx - \Br(t)} \delta( ct' - ct )$.  That's going to add a multiplicative factor to the $dt'$ integral evaluation I think?
...
0 &= \int d^4 x \left( \grad F - \frac{e}{\epsilon_0 c}\int d\tau \frac{dX}{d\tau} \left( Z \delta^4 (X - R_p(\tau)) - \delta^4 (X - R_e(\tau)) \right) \right)
\end{align}
...
0 &= \int_{\text{spacetime volume around only the proton}} d^4 x \left( \grad F_p - \frac{Z e}{\epsilon_0 c} \int d\tau \frac{dX}{d\tau} \delta^4 (X - R_e(\tau)) \right) \\
0 &= \int_{\text{spacetime volume around only the electron}} d^4 x \left( \grad F_e + \frac{e}{\epsilon_0 c} \int d\tau \frac{dX}{d\tau} \delta^4 (X - R_e(\tau)) \right)
\end{align}
