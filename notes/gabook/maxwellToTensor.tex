...
\author{Peeter Joot}
\date{ Sept 7, 2008.  Last Revision: $Date: 2008/09/11 13:18:52 $ }
...
Any raising or lowering of indexes, whether it be in the partials or the basis vectors correspondes to a multiplication by a $(\gamma_{\alpha})^2 = \pm 1$ value, so doing this twice cancels out $(\pm 1)^2 = 1$.
...
Similarily our partial derivatives can be raised or lowered since they are just derivatives in terms of one of the choices of coordinates
...
uses different conventions (at the time of this writing), but both claim a $-+++$ metric, so I think one is wrong.  I'd naturally favour the
...
Now, the trivector part of this equation doens't seem like it is worth much consideration
...
For each of the four trivectors that span the trivector space the coefficents of those trivectors must all therefore equal zero.  The duality set
...
associated with the four vector space, and it will be convienent to use an "index-up" pseudoscalar $i=\gamma^{0123}$.   This will still anticommute with any of the $\gamma^{\mu}$ vectors.
...
Which is exactly the remainging four equations of Maxwell's equation in standard tensor form
...
With the density expanded completely in terms of potentials we are now close to ready to attempt to evaluate the Lagrangian equations or do the 
variational exersize (perhaps as Feynman did it for the electrostatic case in the Lectures) and see that this recovers the field equations.
