...
\date{ October 22, 2008.  Last Revision: $Date: 2008/10/23 12:38:25 $ } % Deleting this command produces today's date.
...
subsequent results apply to spatial rotation as well.  Due to the negative spatial signature ($(\gamma_i)^2=-1$) used here, for a spatial rotation $\alpha$ will represent a rotation in the negative sense with respect to the unit bivector $\acap$.
...
\ref{eqn:boostresultspacetime}
above (ie: taking space time indexes in \ref{eqn:coords}) is the conserved quantity for a boost, and 
the second vector result in \ref{eqn:rotation}
for purely spatial indexes is the conserved quantity for spatial rotation.
