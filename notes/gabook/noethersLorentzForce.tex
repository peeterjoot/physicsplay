...
\date{ October 22, 2008.  Last Revision: $Date: 2008/10/23 04:39:30 $ } % Deleting this command produces today's date.
...
\section{ Noethers applied to covariant Lorentz force Lagrangian. } 

%The Hamiltonian was used above to extract $v^2$ invariance from the Lorentz force Lagrangian under changes of proper time.

Here we consider Noether's theorem applied to the covariant form of the Lorentz force Lagrangian.
For proper velocity $v$, four potential $A$, and positive time metric signature $(\gamma_0)^2 = 1$, that is
...
Consider change with respect to the rapidity factor $\alpha$

\begin{align}\label{eqn:lorentzboosted}
\PD{\alpha}{\LL'} = \frac{d}{d\tau} \left( \PD{\alpha}{x'} \cdot \grad_{v'} \LL \right)
\end{align}

The boost spacetime plane $\acap$ could also be considered a parameter in the transformation, but to use that or the combination of the
two we need the multivector form of Noethers.  These notes were in fact an attempt to get a feeling for the scalar case as lead up to
that so this is an exersize for later.
...
x \wedge \pdot = qA \wedge v/c + q/c \dot{A} \wedge x 
\end{align}
