...
From \autoref{eqn:fourierMaxVac:8}, the energy density can now be formed
...
The grade selection used here doesn't change the result since we already have a scalar, but will just make it convenient to filter out any higher order products that will cancel anyways.  Integrating over the volume element and taking advantage of the orthogonality relationship \autoref{eqn:fourierMaxVac:6}, the exponentials are removed, leaving the energy contained in the volume $V = \lambda_1 \lambda_2 \lambda_3$
...
\subsection{First reduction of the Hamiltonian.}

Let's take the products involved in sequence one at a time, and evaluate
...
If these are not real quantities, part of the followup will be to apply a real selection operation.  The expectation is to obtain a Hamiltonian for the field that has the structure of harmonic oscillators, where the middle two products would have to be zero or sum to zero or have real parts that sum to zero.  The first is expected to contain only products of $\Abs{{\dot{A}_\Bk}^m}^2$, and the last only products of $\Abs{{A_\Bk}^m}^2$.
...
\frac{\epsilon_0 V}{2 c^2} \dot{\BA}_\Bk \cdot \conjugateStar{\dot{\BA}_\Bk}.
\end{align}
...
\frac{2 \epsilon_0 V \pi k_m}{c \lambda_m} \Real \Bigl( i \conjugateStar{(A_\Bk^0)} \dot{A_\Bk}^m \Bigl)
=
\frac{\epsilon_0 V}{c} \Real \Bigl( i \conjugateStar{(A_\Bk^0)} \Bk \cdot \dot{\BA}_\Bk \Bigl).
\end{align}
...
Integration and orthogonality gives
