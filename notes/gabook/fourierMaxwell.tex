...
\date{ Jan 29, 2009.  Last Revision: $Date: 2009/02/24 14:56:33 $ }
...
\subsubsection{ Attempt at evaluating the Green's function. }

The Green's function that we convolve with in \ref{eqn:oneDimResult}
is a rather complex looking function.  As seen later in 
\cite{PJpoisson} it was possible to evaluate a 3D variant of such an integral in
ad-hoc methods to produce a form in terms of retarded time and advanced time delta
functions.  Try such a treatment here too.  Given the 3D result, it is expected
that we will get an integral involving $g(x \pm vt)$ terms.
...
\cite{PJpoisson} for an evaluation of this integral (in an ad-hoc
