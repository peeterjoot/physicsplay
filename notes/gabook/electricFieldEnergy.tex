...
\author{Peeter Joot}
\date{ Jan 03, 2009.  Last Revision: $Date: 2009/02/13 06:03:37 $ }
...
\begin{align}\label{eqn:magneticComponents}
...
From \ref{eqn:magneticComponents} we can calculate the field in terms of our potential directly
...
\begin{align}\label{eqn:poyntingConservation}
...
and now take \ref{eqn:poyntingConservation} and integrate over a (small)
volume
...
Now recall that when the time variation of the Poynting was studied in \cite{PJpoyntingRate} we had what looked like the Lorentz force 
...
associated with the Maxwell equation.  We've also seen in various exersizes that the Lorentz force could be obtained from the action

\begin{align*}
S = \int \left(\inv{2} m (\frac{dx}{d\tau})^2 + q A \cdot \frac{dx}{c d\tau}\right) d\tau
\end{align*}
...
fundamental.  We have one equation at the root of both (and that equation is probaby quite close to the Maxwell field Lagrangian), probably
