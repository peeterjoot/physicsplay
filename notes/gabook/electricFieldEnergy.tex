...
\date{ Jan 03, 2009.  $RCSfile: electricFieldEnergy.tex,v $ Last $Revision: 1.12 $ $Date: 2009/06/11 16:45:58 $ }
...
Once I've got all these down I think I'll be ready to revisit Bohm's Rayleigh-Jeans law treatment in \cite{bohm1989qt}.  Unfortunately, each time I try perusing some 
...
Feynman's energy discussion of the magnetic field for a constant current loop (magnetostatics), is not so easy to follow.  He considers the dipole moment of a small loop, obtained by comparison to previous electrostatic results (that I'd have to go back and read or re-derive) and some
subtle seeming arguments about the mechanical vs. total energy of the system.

\subsection{Biot Savart. }

As an attempt to understand all this, let's break it up into pieces.  First, is calculation of the field for a current loop.  Let's also use 
...
\begin{align}\label{eqn:electric_field_energy:magneticComponents}
...
From \ref{eqn:electric_field_energy:magneticComponents} we can calculate the field in terms of our potential directly
...
Now we can take the curl of $\Bj$, also 
writing this magnetic field $F$ in it's dual form $F = i c \BB$, we see that the curl of our static current density vector is zero:
...
Much shorter derivations are possible than this one which was essentially done from first principles.  The one in
\cite{doran2003gap}, which also uses the STA formulation, is the shortest I've ever seen,
utilizing a vector Green's function for the Laplacian.  However, that requires understanding the geometric calculus chapter of that book, 
which is a battle for a different day.
...
\begin{align}\label{eqn:electric_field_energy:poyntingConservation}
...
and now take \ref{eqn:electric_field_energy:poyntingConservation} and integrate over a (small)
volume
...
That's a funny looking term though?  Where would we see momentum integrated over time in classical mechanics?

\begin{align*}
\int m v dt = m x
\end{align*}

Let's look at the dimensions of all the terms in the conservation equation.  We've identified the $\Bj \cdot \BE$ term with energy density, and 
should see this
...
Good.  That's what should have been the case.
...
Okay, that's good too, since we were calling $U$ energy density.  Now for the Poynting term we have
...
We've got the spatial vector Lorentz force in the second term, and now have an idea what this $-\Bj \cdot \BE$ term is in the energy momentum 
vector.  It isn't a random thing, but an intrinsic part (previously ignored) of the covariant Lorentz force.

Now recall that when the time variation of the Poynting was studied in \ref{chap:PJpoyntingRate} we had what looked like the Lorentz force 
components in all the right hand side terms.  Let's reiterate that here, putting all the bits together
