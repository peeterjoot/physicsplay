...
\begin{align}\label{eqn:lorentzFboost:LorentzBoost}
L(x) &= 
e^{-\alpha \vcap/2}
x
e^{\alpha \vcap/2}
...
&=
\left( \myMathWithDescription{\EE_\parallel' -\Vcap (\EE_\parallel' \cdot \Vcap)}{$\EE_\parallel' - \EE_\parallel'$}{1} \right)
+\EE_\perp'
-\gamma \left( \myMathWithDescription{\EE_\parallel - \Vcap (\EE_\parallel' \cdot \Vcap)}{$\EE_\parallel - \EE_\parallel$}{2} \right)
-\gamma \EE_\perp
+ \gamma (\HH_\perp \cross \BV/c) \\
\end{align*}
...
\begin{align*}
0 &=
% \frac{\gamma}{c^2} \left( d\Bx \cross (\EE' \cross \BV) + \BV ((\EE' - \EE) \cdot d\Bx) \right)
%+ \frac{d\Bx}{c} \cross (\HH -\HH') 
{d\Bx} \cross \left(
