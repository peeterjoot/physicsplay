...
Assuming
that for now (but come back to this and think it through), and writing \(\bcE = \EE_\parallel + \EE_\perp\) for the projective
...
{
\left( 
\EE_\parallel' -\Vcap (\EE_\parallel' \cdot \Vcap)
\right)
}{\(\EE_\parallel' - \EE_\parallel'\)} 
+\EE_\perp'
-\gamma 
...
\begin{aligned}
\EE_\perp' &= \gamma \left( \EE_\perp + \frac{\BV}{c} \cross \bcH_\perp \right) \\
\EE_\parallel' &= \EE_\parallel
\end{aligned}
\end{equation}
...
Contrast this to the result for \(\EE_\perp'\) in the first of \eqnref{eqn:lorentzFboost:finalResultsForPower}.  It differs only by
