...
b_{ij}
&= 
\BB \wedge (\Ba_1 \wedge \cdots \check{\Ba_i} \cdots \check{\Ba_j} \cdots \wedge \Ba_k) 
\frac{ (-1)^{i+j-1} }
{(\Ba_1 \wedge \cdots \wedge \Ba_k) } \\
&=
\BB \cdot \left(\Ba^j \wedge \Ba^i\right) \\
\end{align*}
...
%(it's easy enough to confirm for $k=3$ for example).
