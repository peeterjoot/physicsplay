...
Moving on to the charge and current dependent vector terms of Maxwell's equation, we want express equation \ref{eqn:stokesMax:maxwellv} as a spacetime curl so that we can apply stokes to it.
...
&= \grad \cdot (I D) \\
&= \gpgradeone{ \grad I D } \\
&= -\gpgradeone{ I (\myMathWithDescription{\grad \cdot D}{1 vector}{1} + \myMathWithDescription{\grad \wedge D}{3 vector}{2}) } \\
&= - I (\grad \wedge D) \\
\end{align*}
...
&=
\inv{\epsilon_0}
\myMathWithDescription{{\gamma_0}^2 {\gamma_1}^2}{$=-1$}{1}
\myMathWithDescription{{\gamma_2}^2 {\gamma_3}^2}{$=(\pm 1)^2$}{2}
