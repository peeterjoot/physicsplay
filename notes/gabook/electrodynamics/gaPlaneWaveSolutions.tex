...
%
%\input{../blogpost.tex}
%\renewcommand{gaPlaneWaveSolutions}{gaPlaneWaveSolutions}
...
We want dimensions of $1/L$ for the derivative operator on the RHS of \ref{eqn:gaPlaneWaveSolutions:130}, so we divide through by $\sqrt{\mu\epsilon} I$ for

\begin{equation}\label{eqn:gaPlaneWaveSolutions:130b}
-I \inv{\sqrt{\mu\epsilon}} \spacegrad \BB = \sqrt{\mu\epsilon} \PD{t}{\BE}.
\end{equation}

This can now be added to \ref{eqn:gaPlaneWaveSolutions:110} for
...
We now seek the constraints on $\Bk$, $\omega$, and $F_0$ that allow $\tilde{F}$ to be a solution to \ref{eqn:gaPlaneWaveSolutions:150}
...
Inserting the exponential form of our assumed solution \ref{eqn:gaPlaneWaveSolutions:190} we find
...
Our assumed solution must also satisfy the first order system \ref{eqn:gaPlaneWaveSolutions:230}
...
Writing out all the geometric products, noting that $I$ commutes with all of $\kcap$, $\BE_0$, and $\BB_0$ and employing the identity $\Ba \Bb = \Ba \cdot \Bb + \Ba \wedge \Bb$ we have
...
Grouping the columns and using $\Ba \wedge \Bb = I \Ba \cross \Bb$ again we have a set of constraints resulting from the first order system
\begin{subequations}
...
This and \ref{eqn:gaPlaneWaveSolutions:290} describe all the constraints on our phasor that are required for it to be a solution.  Note that only one of the two cross product equations in \label{eqn:gaPlaneWaveSolutions:405} are required because the two are not independent.  This can be shown by crossing $\kcap$ with \ref{eqn:gaPlaneWaveSolutions:430} and using the identity

\begin{equation}\label{eqn:gaPlaneWaveSolutions:490}
\Ba \cross (\Ba \cross \Bb) = - \Ba^2 \Bb + a (\Ba \cdot \Bb).
\end{equation}

One can find easily that \ref{eqn:gaPlaneWaveSolutions:430} and \ref{eqn:gaPlaneWaveSolutions:450} provide the same relationship between the $\BE_0$ and $\BB_0$ components of $F_0$.  
...
Had we been clever enough this could have been deduced directly from the \ref{eqn:gaPlaneWaveSolutions:330} directly, since we require a product that is killed by left multiplication with $\kcap - 1$.  Our complete plane wave solution to Maxwell's equation is therefore given by 
