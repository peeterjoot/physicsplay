...
\begin{aligned}
F = \FF e^{ i(\Bk \cdot \Bx - \omega t) } = (\bcE + I\bcB/\sqrt{\mu\epsilon}) e^{ i(\Bk \cdot \Bx - \omega t) }
\end{aligned}
\end{equation}
...
Now, lets apply this to the field with \(\FF = \bcE + I\bcB/\sqrt{\mu\epsilon}\).  To avoid dragging around the \(\sqrt{\mu\epsilon}\) factors, let us also temporarily
