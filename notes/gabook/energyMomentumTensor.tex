...
\date{ Jan 01, 2009.  $RCSfile: energyMomentumTensor.tex,v $ Last $Revision: 1.15 $ $Date: 2009/06/11 16:45:58 $ }

\section{Expanding out the stress energy vector in tensor form. }

\cite{doran2003gap} defines (with $\epsilon_0$ omitted),
...
\begin{align}\label{eqn:energy_momentum_tensor:firstPartDone}
...
which is
\begin{align}\label{eqn:energy_momentum_tensor:secondPart}
((\grad \wedge A) \wedge \gamma_\mu) \cdot (\grad \wedge A) \cdot \gamma^\nu
&= F^{\nu \alpha} F_{\mu \alpha} + \inv{2} F_{\alpha \beta} F^{\beta \alpha} {\delta_{\mu}}^\nu
%F^{\nu \beta} F_{\mu \beta} +\inv{2} F_{\alpha \beta} F^{\beta \alpha} {\delta_{\mu}}^\nu
\end{align}

The final combination of results \ref{eqn:energy_momentum_tensor:firstPartDone}, and
\ref{eqn:energy_momentum_tensor:secondPart} gives
...
\begin{align}\label{eqn:energy_momentum_tensor:messyTensor}
{T_\mu}^{\nu}
&=
\epsilon_0 \left(
...
In \ref{chap:PJpoynting}, the electrodynamic energy density $U$ and momentum flux density vectors were related as follows

\begin{align}\label{eqn:energy_momentum_tensor:fromPoyntingNotes}
...
Let's compare these to \ref{eqn:energy_momentum_tensor:messyTensor}, which gives
\begin{align*}
{T_0}^{0}
...
Okay, good.  This checks 4 of the elements of \ref{eqn:energy_momentum_tensor:messyTensor} against the explicit $\BE$ and $\BB$ based representation of $T(\gamma_0)$ in \ref{eqn:energy_momentum_tensor:fromPoyntingNotes}, leaving only 6 unique elements in the remaining parts of the (symmetric) tensor to verify.
...
\ref{eqn:energy_momentum_tensor:fromPoyntingNotes}.  In particular, writing the last of those as
...
Although done in \ref{chap:PJpoynting}, for
...
\begin{align}\label{eqn:energy_momentum_tensor:conservation}
...
Equation \ref{eqn:energy_momentum_tensor:conservation} is the explicit spacetime expansion of the equivalent relativistic equation
