...
\title{NFCM Exercise 8.4.  Legendre Polynomials} % Declares the document's title.
\author{Peeter Joot \quad peeter.joot@gmail.com}         % Declares the author's name.
\date{ Feb 4, 2008.  Last Revision: $Date: 2009/02/22 15:35:07 $ }
...
This exercise is to find the first couple terms of the Legendre polynomial
expansion of 
...
Using the Taylor formula one can derive a recurrance relation for these that makes the calculation a bit
simpler
...
Using the recurrance relation one can calculate $P_3$ for example.
...
- 2 \Bx^2 \Ba^2(\Bx \cdot \Ba)) \\
&=
(1/3) ({\Ba \cdot \Bx}) (
\frac{5}{2}(3(\Ba \cdot \Bx)^2 -\Ba^2\Bx^2)
- 2 \Bx^2 \Ba^2) \\
&=
({\Ba \cdot \Bx}) ( \frac{5}{2}((\Ba \cdot \Bx)^2 ) - 3/2 \Bx^2 \Ba^2) \\
&=
...
This is still pretty labourious to calculate, especially because of not having a closed form recurrance
...
This is nice geometrically since the directional dependence of this term on the colinearity and 
