...
\title{ Application of Stokes Integrals to Maxwell's Equation. }
\author{Peeter Joot}
\date{ Sept 26, 2008.  Last Revision: $Date: 2008/10/02 04:24:10 $ }
...
detailed in \cite{PJStokes1}.  Background of interest can also be found
in \cite{DenkerWire}, which contained the core statement of
...
Or
\begin{equation}\label{eqn:maxwellint}
\oiintctrclockwise F \cdot (d^2\Bx I) = \int \frac{J}{\epsilon_0 c} \cdot (d^3\Bx I)
\end{equation}

This is the integral form of the vector part of Maxwell's equation \ref{eqn:maxwell}.
This doesn't look terribly familiar, but we aren't used to
...
taking $J = j^1 \gamma_1$.  The exersize will be to compute the integrals of equation \ref{eqn:maxwellint}.
...
methods from \cite{PJStokes2} since algebraically enumerating the surfaces
should make life easier.  The two Gauss's law results above were done without this, which wasn't too bad for a purely spatial volume, but with spacetime
...
component, thus for a rectanglar prism integration spacetime volume let
...
again to get exactly that form.  This really just undoes the fact that the partials to used as a convience enumerate
...
will produce the same result.  Of course more care is required to handle the more abstract symbolic indexes since a nice
hardcoded number is easier.
On the other hand the usual dodge, employing freedom to orient the coordinate system along the
$\gamma_1$ direction makes the more general algebraic approach a less interesting exersize.

\subsection{ Faraday's law.}

We have five of the eight Maxwell's equations.  Gauss's law for electrostatics
from the vector part of equation \ref{eqn:maxwellv}, integrating over a spatial
volume, and the Maxwell-Ampere equation from the same, integrating over
a spacetime volume.  Gauss's law for magnetostatics from the trivector part
of \ref{eqn:maxwellv}, integrating over a spatial volume.  This suggests
...
traditional integral form of maxwells equations are
obtained directly from the STA maxwells equation
...
\begin{equation}\label{eqn:summaryMaxwell}
\grad F = J/\epsilon_0 c.
\end{equation}
...
Duality relations were used to put the vector parts of equation \ref{eqn:summaryMaxwell} into a form that Stokes can be applied to.  This gives us
...
produces a combined electric and magnetic field form of a Faraday's law and Gauss' magnetostatics law that doesn't look terribly familiar

\begin{equation}\label{eqn:summaryGaussMandFaraday}
\oiintclockwise \BE \cdot d^2\Bx = - c \oiintclockwise \BB \cdot (d^2\Bx I),
\end{equation}

but integration of this using a spatial volume produces the familiar Gauss's magnetostatic law
...
Integration and summation of the same trivector parts in \ref{eqn:summaryGaussMandFaraday} over each of the possible three spacetime volumes gives us Faraday's law
...
Doing all this with just the volume integrals would possibly have been simpler.  That said, as an exersize to validate the generalized Stokes equation
...
\cite{PJMaxwellProj}, will produce the various traditional vector differential equations.  Of course those can be used once found to develop the integral relations,
but here we get them all in one shot.
%The old 1966 Britianica article uses the integral forms
...
In \cite{PJMaxwell2}, an earlier treatment of Maxwell's equations in a GA context, I used
...
\cite{PJMaxwellProj}, and produce the various traditional vector
differential forms of Maxwell's equations.  Alternatively, as in
\cite{PJMaxwellTensor}, we can extract the traditional tensor
...
\cite{PJSrLagrangian}, to
find the Lorentz force law in STA form using the Euler-Lagrange equations,
and finally in \cite{PJMaxwellLagrangian} where the STA
...
\cite{HestenesFormsGA}.
I re-encountered that paper after writing up these notes.  I'd seen it before but
those parts that cover (tersely) the same material as above didn't make much sense until I had independently worked it all
out in detail myself.
Perhaps I'm dense, but I find that many academic papers are ironically not very good at all for learning from!

I believe these residue/green's function ideas both relate to the
Biot-Savart law, as mentioned in
\cite{HestenesFormsGA}, \cite{doran2003gap}, and
\cite{DenkerWire}.  All of those are either too terse or have details missing
