...
\author{Peeter Joot}
\date{ Sept 26, 2008.  Last Revision: $Date: 2008/10/01 16:26:47 $ }
...
in \cite{DenkerWire}, and \cite{DenkerMaxwell}.
...
FIXME: intially guessed that this one comes from integrating the current over a volume that excludes the current contributions (unlike Maxwell-Ampere above).  wrong: looks like it's from the trivector parts.  writeup.

\section{ Summary and Followup. }

In the treatment of these notes, we get directly to the the
traditional integral form of maxwells equations using the
bivector Stokes equation, and various spacetime integration volumes.

FIXME: summarize equations here, and describe.
...
such as in
FIXME: cite: GAFP, but understanding that takes a lot more work.
...
Goldstein (FIXME:cite) has an interesting treatment of a combined Lagrangian for both
the Lorentz force law and the field equations (using spatial delta functions).
Minimization of the action for that Lagrangian with respect to the potential
...
text like \cite{studentsGuide} would be any easier with a combined field as
