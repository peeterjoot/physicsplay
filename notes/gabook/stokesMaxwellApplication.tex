...
\date{ Sept 26, 2008.  $RCSfile: stokesMaxwellApplication.tex,v $ Last $Revision: 1.16 $ $Date: 2009/06/15 00:48:32 $ }
...
The second of these, equation \ref{eqn:stokesMax:maxwellt}, we can apply Stokes to
directly:
...
On the surface this doesn't look like a familiar identity.  It is in fact Gauss's law for magneto-statics, which will be shown
later.
...
of Maxwell’s equation, we want express equation \ref{eqn:stokesMax:maxwellv} as a
...
This is the integral form of the vector part of Maxwell's equation \ref{eqn:stokesMax:maxwell}.
...
from \ref{eqn:stokesMax:maxwell}, by first extracting the differential vector
...
front surface (see: \ref{chap:PJStokes1})
we have
...
Return now to equation \ref{eqn:stokesMax:gaussmagnetostatics}, which resulted from considering the trivector part of Maxwell's equation
...
taking $J = j^1 \gamma_1$.  The exercise will be to compute the integrals of equation \ref{eqn:stokesMax:maxwellint}.
...
methods from \ref{chap:PJStokes2} since algebraically enumerating the surfaces
...
from the vector part of equation \ref{eqn:stokesMax:maxwellv}, integrating over a spatial
...
of \ref{eqn:stokesMax:maxwellv}, integrating over a spatial volume.  This suggests
...
Duality relations were used to put the vector parts of equation \ref{eqn:stokesMax:summaryMaxwell} into a form that Stokes can be applied to.  This gives us
...
produces a combined electric and magnetic field form of a Faraday's law and Gauss' magneto-statics law that doesn't look terribly familiar
...
Integration and summation of the same trivector parts in \ref{eqn:stokesMax:summaryGaussMandFaraday} over each of the possible three spacetime volumes gives us Faraday's law
...
\ref{chap:PJMaxwellProj}, will produce the various traditional vector differential equations.  Of course those can be used once found to develop the integral relations,
...
In \ref{chap:PJMaxwell2}, an earlier treatment of Maxwell's equations in a GA context, I used
...
\ref{chap:PJMaxwellProj}, and produce the various traditional vector
differential forms of Maxwell's equations.  Alternatively, as in
\ref{chap:PJMaxwellTensor}, we can extract the traditional tensor
...
\ref{chap:PJSrLagrangian}, to
find the Lorentz force law in STA form using the Euler-Lagrange equations,
and finally in \ref{chap:PJMaxwellLagrangian} where the STA
