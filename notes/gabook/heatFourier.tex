...
\author{Peeter Joot}
\date{ Jan 19, 2009.  Last Revision: $Date: 2009/01/21 03:32:17 $ }
...
\begin{align*}
w(x) &= \inv{\sqrt{2\pi}} \IIinf A(s) e^{2 \pi i x s } ds \\
\end{align*}
...
\begin{align*}
A(s) &= \inv{\sqrt{2\pi}} \IIinf w(x) e^{-2 \pi i s x } dx \\
&= \hat{w}(s)
\end{align*}
...
\begin{align*}
u(x,t) &= \inv{\sqrt{2\pi}} \IIinf \hat{w}(s) e^{ -4 \pi^2 s^2 t/ \kappa } e^{2 \pi i x s } ds  \\
\end{align*}

\subsection{ With Green's function? }

If we put in the integral for $\hat{w}(s)$ explicitly and switch the order as was done with the Fourier series will we get a similar result?   Let's try

\begin{align*}
u(x,t) 
&= \inv{\sqrt{2\pi}} \IIinf \left( \inv{\sqrt{2\pi}} \IIinf w(u) e^{-2 \pi i s u } du \right) e^{ -4 \pi^2 s^2 t/ \kappa } e^{2 \pi i x s } ds  \\
&= \inv{\sqrt{2\pi}} \IIinf du w(u) \inv{\sqrt{2\pi}} \IIinf e^{ -4 \pi^2 s^2 t/ \kappa } e^{2 \pi i (x - u) s } ds  \\
\end{align*}
