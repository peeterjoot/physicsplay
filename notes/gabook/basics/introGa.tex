...
\begin{align*}
a b = \myMathWithDescription{\inv{2}(a b + b a)}{$a \cdot b$}{1} + \myMathWithDescription{ \inv{2} ( a b - b a ) }{$a$ something $b$}{2}
\end{align*}
...
Here $\sgn(\pi(\cdots))$ is the sign of the permutation of the indexes.  While we have not gotten yet to products of more than two vectors it is helpful to know that the wedge product will have a place in such a general product.   An equation like \ref{eqn:intro_ga:wedge} makes a lot more sense after writing it out in full for a few specific cases.  For two vectors $a_1$ and $a_2$ this is
...
&= 
\myMathWithDescription{\sum_{\mu} a^\mu b^\mu 
- \sum_{\alpha} a^\alpha b^\alpha }{$=0$}{1}
+ \sum_{\mu \ne \nu} a^\mu b^\nu e_\mu e_\nu 
...
connection is is not obvious.  In particular the use of \ref{eqn:intro_ga:scalarPlusIcross} and the angle relations, one can easily
