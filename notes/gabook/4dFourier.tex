...
FIXME: In \chapcite{PJpoisson} I worked through how to evaluate such an integral (expanding on a too brief treatment found in \cite{byron1992mca}).  To apply such a technique here, where our Green's function has precisely the same form as the Green's function for the Poisson's equation, a way to do the equivalent of a spherical polar parametrization will be required.  How would that be done in 4D?  Have seen such treatments in \cite{flanders1989dfa} for hypervolume and surface integration, but they didn't make much sense then.  Perhaps they would now?
...
Here also we have to be careful to keep the Green's function on the right hand side of $A$ since they won't generally commute.
