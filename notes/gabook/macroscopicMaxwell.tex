...
\date{ May 28, 2009.  Last Revision: $Date: 2009/05/29 01:49:38 $ }
...
\begin{align}\label{eqn:MaxwellMixedFields}
...
and in turn can write the macroscopic Maxwell equations \ref{eqn:MaxwellMixedFields}
...
For the remainding terms we have $\spacegrad \wedge \BP, i\BM \in \Span\{\gamma_a \gamma_b\}$, and $\gamma_0 \spacegrad \wedge (iM) \in \Span{ \gamma_1 \gamma_2 \gamma_3}$, so between the three of these we have a (Dirac) trivector, so it would be reasonable to write
...
\begin{align*}
\gpgradezero{ \grad \grad F } &= \gpgradezero{ \grad \frac{4\pi}{c} J + 4\pi T }  \\
