...
However, in the form of equation \ref{eqn:pseudoinversetimesmatrixsum} this is not too hard.
...
This allows us to write the matrix of equation \ref{eqn:pseudoprojmatsum}
as
...
Now suppose we have a vector $n$ that is perpendicular to all vectors $v_i$.  We can then write:
...
In block matrix form for all $v_i$ that is:

\[
[\transpose{v_i} n]_i = [\transpose{v_i}]_i n = \transpose{V} n = 0
\]

This is why we say that $n$ is in the null space of $\transpose{V}$,
$N(\transpose{V})$ not $N(V)$.  One perhaps could say this is in the null
or perpendicular space of the set $\{v_i\}$, but our use of columns as
...
Since $\transpose{V} n = 0$ for any $n \in N(V)$ we also have

\[
V \transpose{V} n = 0
\]

Thus the first property required of this matrix as a projection operation
is satisfied.

Now we have seen that $V \transpose{V}$ is not an identity matrix, although
it can have blocks of identity matrixes along the diagonal in some cases.
However, we can examine the action of this matrix on any vector that lies in the
span of the set $\{v_i\}$.  By linearity it is sufficient to do this calcuation 
for a particular $v_i$:
...
&= \sum_{k=1}^{r} v_k \delta_{ki} \\
&= v_i \\
\end{align*}

This completes the verification that $V \transpose{V}$ is a projection
onto the span of the columns of $V$.  Looking back, since the columns
of $V$ are of full rank we could also have said this directly using our
previous projection matrix equation

\[
\Proj_V = V (\transpose{V} V)^{-1} \transpose{V}
\]

The central product term to invert is:

\[
\transpose{V} V = [ \transpose{v_i} v_j ]_{ij} = [ \delta_{ij} ]_{ij} = I_{r,r}
\]

Therefore, in this particular case where the column vectors are orthonormal,
our projection matrix can be expressed using the two exterior terms alone:

\begin{equation}\label{eqn:projVr}
\Proj_V = V (\transpose{V} V)^{-1} \transpose{V} = V \transpose{V}
\end{equation}
...
not the $V \in M^{n,r}$ of equation \ref{eqn:projVr}.
