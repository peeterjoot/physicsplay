...
\begin{equation}\label{eqn:projectionwitheucliandot}
p = U \alpha = U
{{
\begin{bmatrix}
u_i \cdot u_j
...
matrix calculation with respect to a general complex dot product:
...
The problem is the same as above.  We want to repeat the derivation of equation \ref{eqn:projectionwitheucliandot}, but be more
careful with ordering of terms since we now using a non-commutative dot (inner) product.
