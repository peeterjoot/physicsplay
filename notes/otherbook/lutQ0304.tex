...
%\date{ March dd, 2009.  Last Revision: $Date: 2009/06/04 01:21:04 $ }

\begin{document}

Attempt to decode a fragment of 
\href{http://en.wikipedia.org/wiki/Frame_fields_in_general_relativity}{Static observers in Schwarzschild vacuum, fro the Frame field article.}
...
What's the gradient notation mean?  Is it the gradient from Doran/Lasenby?  There for a vector in a basis $\vec{e}_\mu$, or the dual basis $\sigma^\mu$ is
...
Perhaps such a gradient is related to this GR frame gradient?  Let's try applying that to $\vec{e}_0$
