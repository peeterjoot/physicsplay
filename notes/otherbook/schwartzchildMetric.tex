...
\date{ October 2, 2008.  Last Revision: $Date: 2008/10/05 20:46:44 $ } % Deleting this command produces today's date.
...
%Parameterization with $\lambda = c\tau$, where $ds = c d\tau$ is an element of event arc length this is
%
%\begin{align*}
%1 &= \left(\frac{dt}{d\tau}\right)^2 \left( 1 - (v/c)^2 \right) \\
%\delta \tau 
%&= \left( 1 - (v/c)^2 \right)^{-1/2} \delta t \\
%&= \gamma \delta t \\
%\end{align*}
...
%Again, the metric tensor and its inverse is formed by the matrix of dot products.
%
%\begin{align*}
%g_{\mu\nu} &= e_\mu \cdot e_\nu \\
%g^{\mu\nu} &= e^\mu \cdot e^\nu
%\end{align*}
%
%These are both neccessarily symmetric, but there is no requirement for diagonality.  The 
