...
\keywords{Navier-Stokes, PHY454H1S, PHY454H1, Mathematica}
...
For a cup size of $R = 5 \text{cm}$, stir radius of $s = 3 \text{cm}$, stir depth $d = 2 \text{cm}$, and angular velocity $\Omega = 2 \pi \text{rad}/\text{s}$, we can compute (\href{https://raw.github.com/peeterjoot/physicsplay/master/notes/phy454/coffeeCupWithBottom.cdf}{phy454/coffeeCupWithBottom.cdf}) some terms of this series
...
\imageFigure{coffeeCupWithBottomFig2}{Vector field plot of the velocity field below the stir depth.}{fig:coffeeCupWithBottom:coffeeCupWithBottomFig2}{0.3}
...
\imageFigure{coffeeCupWithBottomFig1}{Radial velocity dependence at the stir height.}{fig:coffeeCupWithBottom:coffeeCupWithBottomFig1}{0.3}
...
\begin{equation}\label{eqn:coffeeCupWithBottom:600}
\PD{t}{u} = 
\nu
...
\begin{equation}\label{eqn:coffeeCupWithBottom:620}
\frac{T'}{T} = -\nu \frac{\alpha^2}{R^2} =
\nu
...
\begin{equation}\label{eqn:coffeeCupWithBottom:640}
T \propto e^{-\alpha^2 \nu t/R^2},
\end{equation}
...
\begin{subequations}
\begin{equation}\label{eqn:coffeeCupWithBottom:660}
\inv{r R} ( r R' )' -\frac{1}{r^2} = -\frac{\lambda_i^2}{R^2}
\end{equation}
\begin{equation}\label{eqn:coffeeCupWithBottom:680}
...
\begin{equation}\label{eqn:coffeeCupWithBottom:700}
\begin{aligned}
u(r, z, t)
&=
s \Omega \sum_{\lambda_i^2 > \alpha^2} c_{i \alpha} J_1(\lambda_i r/R) \sinh\left( \sqrt{\lambda_i^2 - \alpha^2} \frac{z}{R} \right) e^{-\alpha^2 \nu t/R^2} \\
&+s \Omega \sum_{\lambda_i^2 < \alpha^2} d_{i \alpha} J_1(\lambda_i r/R) \sin\left( \sqrt{\alpha^2 -\lambda_i^2} \frac{z}{R} \right) e^{-\alpha^2 \nu t/R^2}
\end{aligned}
\end{equation}

with a boundary value condition

\begin{equation}\label{eqn:coffeeCupWithBottom:720}
\begin{aligned}
\sum_{i=1}^\infty 
\frac{
\int_0^1 w J_1 (\lambda_i w) dw
...
J_1(\lambda_i r/R) \frac{\sinh(\lambda_i z/R)}{\sinh(\lambda_i d/R) } 
&=
\sum_{\lambda_j^2 > \alpha^2} c_{j \alpha} J_1(\lambda_j r/R) \sinh\left( \sqrt{\lambda_j^2 - \alpha^2} \frac{z}{R} \right) \\
&+\sum_{\lambda_j^2 < \alpha^2} d_{j \alpha} J_1(\lambda_j r/R) \sin\left( \sqrt{\alpha^2 -\lambda_j^2 } \frac{z}{R} \right) 
\end{aligned}
\end{equation}

It's not clear to me how to solve this, and I think that I'd have to go back and actually understand the theory from \cite{sagan1989boundary} used to solve the steady state problem above for $C_i$ (i.e. I don't know where the result I used came from, only that it works, so when I'm faced with a requirement to generalize it, I'm out of luck).

It looks like it would be reasonable to drop the hyperbolic terms, and insist that

\begin{equation}\label{eqn:coffeeCupWithBottom:740}
(\alpha^2 - \lambda_j^2) \frac{d}{R} = 2 \pi j,
\end{equation}

or

\begin{equation}\label{eqn:coffeeCupWithBottom:760}
\alpha^2 2 \pi j \frac{R}{d} + \lambda_j^2.
\end{equation}

leaving us to find $d_j = d_{j\alpha}$ such that

\begin{equation}\label{eqn:coffeeCupWithBottom:720b}
\sum_{i=1}^\infty 
\frac{
\int_0^1 w J_1 (\lambda_i w) dw
}{
...
J_1(\lambda_i r/R) \frac{\sinh(\lambda_i z/R)}{\sinh(\lambda_i d/R) } 
=
\sum_{j = 1}^\infty d_j J_1(\lambda_j r/R) \sin 2 \pi j \frac{z}{d}.
\end{equation}

If this matches at any $r$, then it must also match at $r = s$, so we'd have

\begin{equation}\label{eqn:coffeeCupWithBottom:720c}
\sum_{i=1}^\infty 
\frac{
\int_0^1 w J_1 (\lambda_i w) dw
}{
...
J_1(\lambda_i s/R) \frac{\sinh(\lambda_i z/R)}{\sinh(\lambda_i d/R) } 
=
\sum_{j = 1}^\infty d_j J_1(\lambda_j s/R) \sin 2 \pi j \frac{z}{d}.
\end{equation}

Solving this Fourier problem will give us a match at $z = 0$, and $r = 0, s, R$.  Will that match for all $r,z$, or do we have to retain the hyperbolic terms and do something fancier?
...
I'd like to go back and understand the Sturm Liouville theory in \cite{sagan1989boundary} that I used the result from to solve the steady state problem.  Thinking about what I've been using, we basically been using a weighted inner product, so that if we are looking for a fit for $x \in [0,1]$ of
...
\sqrt{\alpha^2 - \lambda_j^2} \frac{d}{R} = \pi j,
\end{equation}

or

\begin{equation}\label{eqn:coffeeCupWithBottom:880}
\frac{\alpha^2}{R^2} = \frac{\pi^2 j^2}{d^2} + \frac{\lambda_j^2}{d^2},
\end{equation}
...
=
\sum_{j = 1}^\infty d_j J_1(\lambda_j x) \sin\left( \pi j y \right).
\end{equation}

Observing that 

\begin{equation}\label{eqn:coffeeCupWithBottom:920}
\int_0^1 \sin(\pi j y) \sin(\pi k y) dy = \inv{2} \delta_{j k}.
\end{equation}

we can multiply both sides by $x J_1(\lambda_k x) \sin ( \pi m y )$ and integrating over the unit square we have
...
\int_0^1 x J_1(\lambda_i x) J_1( \lambda_k x) dx \int_0^1 \sin( \pi m y ) \frac{\sinh(\lambda_i y d/R)}{\sinh(\lambda_i d/R) } dy \\
&=
\sum_{j = 1}^\infty d_j \int_0^1 x J_1(\lambda_j x) J_1( \lambda_k x) dx \int_0^1 \sin( \pi m y ) \sin\left( \pi j y \right) dy.
\end{aligned}
\end{equation}
...
\int_0^1 x J_1^2(\lambda_i x) \delta_{i k} \int_0^1 \sin( \pi m y ) \frac{\sinh(\lambda_i y d/R)}{\sinh(\lambda_i d/R) } \\
&=
...
\int_0^1 \sin( \pi k y ) \frac{\sinh(\lambda_k y d/R)}{\sinh(\lambda_k d/R) }
=
...
d_k = 2 \int_0^1 \sin( \pi k y ) \frac{\sinh(\lambda_k y d/R)}{\sinh(\lambda_k d/R) } dy
=
\frac{2 \pi k (-1)^{k+1} \sinh \left(\frac{d \lambda_k}{R}\right)}{\frac{d^2 \lambda_k^2}{R^2}+ k^2\pi^2}
\end{equation}
...
\Bu&(r, z, t) \\
&= 
\frac{2 \Omega s}{\pi} \phicap \sum_{j = 1}^\infty 
(-1)^{j+1}
\frac{j }{\frac{d^2 \lambda_j^2 }{R^2 \pi^2}+ j^2}
\sinh \left(\frac{d \lambda_j}{R}\right)
\exp\left( -\left( \frac{\lambda_j^2}{R^2} + \frac{\pi^2 j^2}{d^2} \right) \nu t\right)
J_1 \left(\frac{\lambda_j r}{R}\right) 
\sin\left( \frac{\pi j z}{d} \right) 
.
\end{aligned}
\end{equation}

It's interesting that we end up with products of the orthonormal Bessel and trig functions when we started with a requirement for Bessel times hyperbolic trig functions.  We can likely utilize a similar Fourier decompostion of the hyperbolic trig functions to solve the steady state problem above the stir point, then solve for all our Fourier coefficients using this technique.  That will obviously be a harder problem, but one that looks at least feasible.  Regardless, we have to start at the bottom of the cup and work our boundary value conditions up from there.  

We have a result that appears to validate the claim in \cite{acheson1990elementary} about the importance of the bottom of the cup in this problem.  Let's look at the magnitude of the viscous boundary layer term and see if we can estimate approximately when the spin down is mostly complete.
