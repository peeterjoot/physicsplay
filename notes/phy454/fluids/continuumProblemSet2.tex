...
% 
\begin{Exercise}[
title={Steady rectilinear blood flow},
label={problem:fluids:blood}
]
...
\Question{Show that the Navier-Stokes equation reduces to
...
\label{problem:fluids:bloodReduceNS}
\Question{Show that the velocity profile of the fluid inside the artery is a parabolic profile.}
\label{problem:fluids:bloodVelocityProfile}
\Question{What is the maximum speed of the fluid?  Draw the velocity profile to show where the maximum speed occurs inside the artery.}
\label{problem:fluids:bloodMaxSpeed}
\Question{If due to smoking etc., the viscosity of the blood gets doubled, then what should be the new pressure gradient to be maintained by the `heart' to keep the liquid flux through the artery at the same level as the non-smoking one?}
\label{problem:fluids:bloodViscositySmoking}
\end{Exercise}

\begin{Answer}[ref={problem:fluids:blood}]

\ref{problem:fluids:bloodReduceNS} 
%\subsubsection{Solution.  Part 1.  Navier-Stokes equation for the system.}
...
%\subsubsection{Solution.  Part 2.  Velocity profile.}
\ref{problem:fluids:bloodVelocityProfile}For the velocity profile, integrating \ref{eqn:continuumProblemSet2:140} twice, we have
...
%\subsubsection{Solution.  Part 3.  Maximum speed of the flow.}

\ref{problem:fluids:bloodMaxSpeed}
...
\pdfTexFigure{figures/continuumProblemSet2Fig1r2.pdf_tex}{Velocity profile for 1D constant pressure gradient steady state flow.}{fig:continuumProblemSet2:continuumProblemSet2Fig1}{0.3}

%\subsubsection{Solution.  Part 4.  Effects of viscosity doubling.}
\ref{problem:fluids:bloodViscositySmoking}
...
\begin{Exercise}[
title={simple shearing flow},
label={problem:fluids:simpleShearing}
]
...
\Question{Derive the velocity profile of the two fluids.}
\label{problem:fluids:simpleShearing1}
\Question{Calculate the maximum speed.}
\label{problem:fluids:simpleShearing2}
\Question{Calculate the mean speed.}
\label{problem:fluids:simpleShearing3}
\Question{Calculate the flux (the volume flow rate.)}
\label{problem:fluids:simpleShearing4}
\Question{Calculate the tangential force (per unit width) $F_x$ on the strip $0 \le x \le L$ of the wall $y = -h$.}
\label{problem:fluids:simpleShearing5}
\Question{Calculate the tangential force (per unit width) $F_x^0$ on the strip $0 \le x \le L$ at the interface $y = 0$ by the top fluid on the lower fluid.}
\label{problem:fluids:simpleShearing6}
\end{Exercise}

\begin{Answer}[ref={problem:fluids:}]

\ref{problem:fluids:simpleShearing1} Starting with the velocity profile derivation for the two fluids, we set up coordinates as in figure (\ref{fig:continuumProblemSet2:continuumProblemSet2Fig2}).  Our steady flow for layers $1$ and $2$ has the form
...
\pdfTexFigure{figures/continuumProblemSet2Fig2r2.pdf_tex}{Two layer flow induced by moving wall.}{fig:continuumProblemSet2:continuumProblemSet2Fig2}{0.4}
...
\imageFigure{figures/continuumProblemSet2Fig3}{Two layer shearing flow with water over mercury.}{fig:continuumProblemSet2:continuumProblemSet2Fig3}{0.2}

\ref{problem:fluids:simpleShearing2} We are now ready to calculate the maximum speed.
...
\ref{problem:fluids:simpleShearing3} Now we calculate the mean speed.
...
\ref{problem:fluids:simpleShearing4} Next is the calculation of the flux.
...
\ref{problem:fluids:simpleShearing5} For the tangential force on the wall, we see from \ref{eqn:continuumProblemSet2:600} the tangential components of our traction vectors are
...
\ref{problem:fluids:simpleShearing6} Finally, for the tangential force on the lower fluid, we note that due to constant nature of the tangential component of the traction vector shown above, the force per unit width of the upper fluid acting on the lower fluid, is also given by \ref{eqn:continuumProblemSet2:740}.
\end{Answer}

\begin{Exercise}[
title={simple shearing flow with constant pressure gradient},
label={problem:fluids:simpleShearingPressureGrad}
]
...
\imageFigure{figures/continuumProblemSet2Fig4}{Superposition of constant pressure gradient and shear flow solutions ($\mu^{(1)} > \mu^{(2)}$).}{fig:continuumProblemSet2:continuumProblemSet2Fig4}{0.3}

\imageFigure{figures/continuumProblemSet2Fig5}{Superposition of constant pressure gradient and shear flow solutions ($\mu^{(1)} < \mu^{(2)}$).}{fig:continuumProblemSet2:continuumProblemSet2Fig5}{0.3}
...
The figures above are kind of rough.  It's not actually hard to solve the system above.  After some simplification, I find using Mathematica \href{http://github.com/peeterjoot/physicsplay/blob/master/notes/phy454/mathematica/problemSetIIQ3exactSolution.cdf}{problemSetIIQ3exactSolution.cdf} the following solution

\begin{align}\label{eqn:continuumProblemSet2:840}
...
Plotting this with sliders or animation in Mathematica is a fun way to explore visualizing this.  The results vary widely depending on the various parameters.  Such a Mathematica notebook can be found in \href{https://raw.github.com/peeterjoot/physicsplay/master/notes/phy454/mathematica/problemSetIIQ3PlotWithManipulate.cdf}{problemSetIIQ3PlotWithManipulate.cdf}.  Here are animations with variation of the pressure gradient for $v_1 = 0$, $h_1 = h_2$, showing the superposition of the shear and channel flow solutions

\begin{itemize}
\item With $\mu^{(1)} > \mu^{(2)}$.  See \youtubehref{2xVoFAL9XGA}.
%figure \ref{fig:continuumProblemSet2:continuumProblemSet2Animation1} (or 

\item With $\mu^{(2)} > \mu^{(1)}$.  See \youtubehref{FJekyGf6XJw}.
%figure \ref{fig:continuumProblemSet2:continuumProblemSet2Animation2} (or 
\end{itemize}
...
%figure (\ref{fig:continuumProblemSet2:continuumProblemSet2Fig6mu1Gtmu2})
\imageFigure{figures/continuumProblemSet2Fig6mu1Gtmu2}{$\mu^{(1)} > \mu^{(2)}$}{fig:continuumProblemSet2:continuumProblemSet2Fig6mu1Gtmu2}{0.3}

%figure (\ref{fig:continuumProblemSet2:continuumProblemSet2Fig6mu2Gtmu1})
\imageFigure{figures/continuumProblemSet2Fig6mu2Gtmu1}{$\mu^{(2)} > \mu^{(1)}$}{fig:continuumProblemSet2:continuumProblemSet2Fig6mu2Gtmu1}{0.3}
