...
% 
% 
\makeproblem{Couette flow of a viscous incompressible fluid}{problem:fluids:couetteFlow}{
A problem from the 2011 phy1530 problem set 2.  Consider the steady flow between two long cylinders of radii $R_1$ and $R_2$, $R_1 > R_1$, rotating about their axes with angular velocities $\Omega_1$, $\Omega_2$.  Look for a solution of the form, where $\phicap$ is a unit vector along the azimuthal direction:
...
\label{problem:fluids:couetteFlow:1}
\Question{Write out the Navier-Stokes equations and find differential equations for $v(r)$ and $p(r)$.  You should find that these equations have relatively simple solutions, i.e.,
...
\label{problem:fluids:couetteFlow:2}
\Question{Fix the constants $a$ and $b$ from the boundary conditions.  Determine the pressure $p(r)$}
\label{problem:fluids:couetteFlow:3}
\Question{Compute the friction forces that the fluid exerts on the cylinders, and compute the torque on each cylinder.  Show that the total torque on the fluid is zero (as must be the case)}
} % end problem
...
\imageFigure{figures/couetteFlowFig1}{Couette flow configuration}{fig:couetteFlow:couetteFlowFig1}{0.3}

\ref{problem:fluids:couetteFlow:1}
%\unnumberedSubsection{Navier-Stokes and resulting differential equations}
...
Let us first verify that the incompressible condition \ref{eqn:couetteFlow:90} is satisfied for the presumed form of the solution we seek.  We have
...
Good.  Now let us write out the terms of the momentum conservation equation \ref{eqn:couetteFlow:70}.  We have got
...
for which we have been told to expect that \ref{eqn:couetteFlow:50} is a solution (and it has the two integration constants we require for a solution to a homogeneous equation of this form).  Let us verify that we have computed the correct differential equation for the problem by trying this solution
...
\ref{problem:fluids:couetteFlow:2}
%\unnumberedSubsection{Fixing the constants}
...
\ref{problem:fluids:couetteFlow:3}
%\unnumberedSubsection{Friction and torque}
...
\imageFigure{figures/couetteFlowFig2}{Couette flow plot}{fig:couetteFlow:couetteFlowFig2}{0.2}
%figure (\ref{fig:couetteFlow:couetteFlowFig3}).
\imageFigure{figures/couetteFlowFig3}{Couette flow plot}{fig:couetteFlow:couetteFlowFig3}{0.2}
%figure (\ref{fig:couetteFlow:couetteFlowFig4}).
\imageFigure{figures/couetteFlowFig4}{Couette flow plot}{fig:couetteFlow:couetteFlowFig4}{0.2}
%figure (\ref{fig:couetteFlow:couetteFlowFig5}).
\imageFigure{figures/couetteFlowFig5}{Couette flow plot}{fig:couetteFlow:couetteFlowFig5}{0.2}
