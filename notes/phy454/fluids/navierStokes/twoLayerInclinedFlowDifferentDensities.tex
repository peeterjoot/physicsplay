...
Our problem is illustrated in figure (\ref{fig:twoLayerInclinedFlow:twoLayerInclinedFlowFig1}) with a plane set at angle $\alpha$, fluid depths of $h^{(1)}$ and $h^{(2)}$ respectively, and viscosities $\mu^{(1)}$ and $\mu^{(2)}$.  We will write $H = h^{(1)} + h^{(2)}$.  We have a pair of Navier-Stokes equations to solve
...
To try this out numerically, I have created a Mathematica worksheet \href{https://raw.github.com/peeterjoot/physicsplay/master/notes/phy454/mathematica/twoLayerInclinedFlowDifferentDensities.cdf}{twoLayerInclinedFlowDifferentDensities.cdf}.
