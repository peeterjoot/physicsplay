%
% Copyright © 2016 Peeter Joot.  All Rights Reserved.
% Licenced as described in the file LICENSE under the root directory of this GIT repository.
%
%----------------------------------------------------------------------------------------
%
% Prof. decoder ring:
%
% antigreat      == integrate
% ambeeguis      == ambiguous
% eikenvector    == eigenvector
% esmal          == small
% sine "of a" x  == sin(x)
% beta suba x    == beta_x
% constrain      == constraint
% havea expr     == have expr (example: havea e^x), plug this expression for beta -> plug this expression for abeta.
% isa            == is : f_{c1} isa bigger than f_{c2}
% den            == then
% ispectra       == spectrum
% smood          == smooth
% togeder        == together
% 
%----------------------------------------------------------------------------------------
\part{Lecture notes}
   %\chapter{Electromagnetic fields}
   \input{emtLecture1.tex}
   \input{emtLecture2.tex}
   \section{Problems}
      \input{continuityDisplacement.tex}
      \input{griffithsEM2_7.tex}
      \input{emtproblemSet1Problem1.tex}
      \input{emtproblemSet1Problem2.tex}
      \input{emtproblemSet1Problem3.tex}
      \input{emtproblemSet1Problem4.tex}
      \input{emtproblemSet1Problem5.tex}
      \input{emtproblemSet1Problem6.tex}
      %\input{emtproblemSet1Appendix.tex}
      \input{emtproblemSet2Problem2.tex}
      \input{emtproblemSet2Problem3.tex}
      \input{emtproblemSet2Problem5.tex}

   \input{emtLecture3.tex}
   \section{Problems}
      \input{emtproblemSet2Problem1.tex}
      \input{emtproblemSet2Problem4.tex}
      \input{dipoleMoment.tex}
      \input{dipolePotential.tex}

   \input{emtLecture4.tex}
      \section{Problems}
      \input{magneticMomentJackson.tex}
      \input{vectorAreaGriffiths.tex}
      \input{emtproblemSet3Problem1.tex}
      \input{emtproblemSet3Problem2.tex}
      \input{emtproblemSet3Problem3.tex}
      \input{emtproblemSet3Problem4.tex}
      %\input{emtProblemSet3Appendix.tex}
      \input{emtproblemSet4Problem4.tex}
      \input{magneticFieldFromMoment.tex}

   \input{emtLecture5.tex}
      \section{Problems}

      \input{emtproblemSet4Problem1.tex}
      \input{emtproblemSet4Problem2.tex}
      \input{emtproblemSet4Problem3.tex}
      %\input{poynting.tex}
      \input{emtproblemSet7Problem1.tex}
      \input{poyntingTimeHarmonic.tex}

   \input{emtLecture6.tex}
      \section{Problems}

      \input{emtproblemSet5Problem1.tex}
      \input{emtproblemSet5Problem2.tex}
      \input{emtproblemSet5Problem3.tex}

   \chapter{Druid model}
      \input{druid.tex}
%      FIXME: transcribe handwritten notes that were mostly skipped over in class?
      \section{Problems}

%\chapter{conductivity} % transcribe?  This was part of L7
   \input{emtLecture7.tex}
      \section{Problems}
         \input{emtproblemSet6Problem1.tex}

   \chapter{Wave equation solutions}

In class, we walked through splitting up the wave equation into components, and separation of variables.  I didn't take notes on that.

Winding down that discussion, however, was a mention of phase and group velocity, and a phenomina called superluminal velocity.  This latter is analogous to quantum electron tunneling where a wave can make it through an aperature with a damped solution \( e^{-\alpha x} \) in the aperature interval, and sinuoidal solutions in the incident and transmitted regions as sketched in \cref{fig:L7:L7Fig1}.  The time \( \tau \) to get through the aperature is called the tunnelling time.

\imageFigure{../../figures/ece1228-emt/L7Fig1}{Superluminal tunneling.}{fig:L7:L7Fig1}{0.3}

      \section{Problems}
          \input{emtproblemSet6Problem2.tex}
          \input{emtproblemSet6Problem3.tex}

   \chapter{Wave equation solutions}
      \input{emtLecture8.tex}
      \section{Problems}
         \input{emtproblemSet7Problem2.tex}
         \input{emtproblemSet7Problem3.tex}

   \chapter{Quadrupole expansion}
      \input{emtLecture8Quad.tex}
      \input{momentCoeffiecients.tex}
      \section{Problems}
         \input{dipoleFromSphericalMoments.tex}

   \chapter{Fresnel relations}
      \input{emtLecture10.tex}
      \input{twoInterfaceNormal.tex}
      \section{Problems}
         \input{fresnelSumAndDifferenceAngleFormulas.tex}
         \input{emtproblemSet8Problem1.tex}
         \input{emtproblemSet8Problem2.tex}
         \input{emtproblemSet8Problem3.tex}
         \input{emtproblemSet9Problem1.tex}
         %\input{emtproblemSet9Problem2.tex}
   \chapter{Gauge freedom.}
      \section{Problems}
         \input{emtproblemSet9Problem3.tex}
