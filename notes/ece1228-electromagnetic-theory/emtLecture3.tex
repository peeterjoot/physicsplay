...
An equal charge dipole configuration is sketched in 
...
When an electric field is applied to a volume, bound charges are induced on the surface of the material, and bound charges induced in the volume.  Both of these are related to the polarization \( \BP \).

F5

For example in a capacitor using glass as a dielectric, as sketched in
...
In particular, when the electron cloud density of a material is not symmetric

F4

then we have a dipole configuration in each atom.  When the atom is symmetric, by applying an electric field, a dipole configuration can be created.
...
Looking back to the FIXME:cref potentials in their volume and surface charge density forms, we see that 
