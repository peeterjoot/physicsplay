...
FIXME: the following attempt to capture what was discussed in class is confused and needs to be rethought.

It will be left to homework to show, using the continuity equation and Gauss's law that 
Inside a conductor, the charges distribute in such a way that there is no electric field inside the medium.  This has some interesting consequences.
%It wasn't clear if the following was an attempt to justify this, or use this:
...
If this is true for any geometry, then it must be that \( \BE_1 \) is zero within the conductor.  There is nothing in this equation that prevents non-zero time independent magentic fields inside a conductor.  
