...
To deterine the constraints on \( \beta_\txtz \) it can be observed that the fields obey the wave equation (or Helmholtz equation in the frequency domain), which follows from the expansion of the curl of the curl
...
so the constraint on the dispursion rate is
...
\citep{doran2003gap}
vector product \( \BA \BB = \BA \cdot \BB + I \BA \cross \BB \), which also applies to the gradient 
...
Operation with the gradient from the left gives
