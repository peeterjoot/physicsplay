...
Assuming normal incidence, we can solve the system assuming waves of the form sketched in \cref{fig:twoInterfaces:twoInterfacesFig2}.
...
Here, as done in class, I've utilized the freedom to include arbitrary constant phase factors in the input and output wave functions.  The boundary conditions at \( z = d \) are
...
The frequency dependence is carried by \( k_i = (\omega/c) n_i \).  In the one interface problem, that completely kills any explicit frequency dependence, but we have such dependence here, carried by the phase factor \( e^{-j k_2 d } \).  That is

%\begin{dmath}\label{eqn:emtproblemSet8Problem2:240}
\boxedEquation{eqn:emtproblemSet8Problem2:240}{
T(\omega)
=
\frac{4 \mu_1 \mu_2 n_1 n_2 e^{-j \omega n_2 d/c}}{(\mu_2 n_1 + \mu_1 n_2)^2 - (\mu_2 n_1 - \mu_1 n_2)^2 e^{-2 j \omega n_2 d/c} }.
}
%\end{dmath}
