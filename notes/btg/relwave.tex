...
The introduction in (\cite{french1998iqp}) is accessible to engineering undergrads and attempts to ``just make plausible'' the one dimensional specialization of Schr\"{o}dinger's equation for a particle in a potential.  The essence of their argument relies on energy conservation, and for quick reference (without the discussion of the text) can be found in (\cite{wikiSchH}).

Calling many of the plausibility introductions derivations is justifiably debatable and is probably the origin of statements like ``Schr\"{o}dinger's equation cannot be derived'' (\cite{hyperphysicsSch}), or 
Feynman: quote about cannot understand.
Perhaps agreeing with this statements some texts do not much more than present the equation and start calculating.  For example in (\cite{liboff2003iqm}) the equation is presented rather abstractly as an operatorizing of the Hamiltonian.  This requires an identifications $\Bp \sim -i \hbar \spacegrad$, something harder to make plausible in a classical context.  As a method of developing comfort with many of the concepts of the theory calculation is definitely required.

Bohm's Quantum Theory (\cite{bohm1989qt}) stands out as a rather meticulously sequenced exposition of many of the concepts and the mathematics behind Quantum theory.  Among other things, the momentum operator identification pointed out above is justified nicely utilizing Fourier transform arguments.  However, with a cost of nine chapters and two hundred pages to get to the starting point of many other introductory Quantum texts this one is not for the impatient.

Pauli's Wave Mechanics text (\cite{pauli2000wm}) contains an approach considerably different from most introductory texts, and performs a small momentum approximation 
.  Pauli performs a non-relativistic approximation 
This can be done, and Bohm's.  treatment, a rather meticulously crafted
 however, is not for the impatient, 

The commentary (origin unknown, citations missing) in wikipedia's Klien-Gordon page (\cite{wikiKG}), indicate the Schr\"{o}dinger's equation as we first see it in introductory texts was arrived at utilizing a non-relativistic approximation.  Not many texts appear to use this historical approach to introduce the subject, probably because of the desire to avoid requiring special relativity.  is an exception.


FIXME: write something short here, perhaps introducing both the Fourier
notation, and the wave equations we want to solve, and a point form
approach showing the high level route to be followed.

\section{Relativity prerequistes.}

In these notes the space time trajectory of a particle will be represented as the pair of locally observable quantities or a coordinate column vector representation of the same form
...
Utilizing this Lorentz distance invariant length vectors can be constructed by scaling unit four vectors.  The example of importance here is that of proper velocity, the unit four vector for a particle’s worldline evolution (FIXME: what would this derivative be called?)

\frac{dX}{dt} = \left(c, \frac{d\Bx}{dt}\right)

The invariant length is

\frac{dX}{dt} = c^2 - \left(\frac{d\Bx}{dt}\right)^2

So the proper velocity is

FIXME.


FIXME: reintroduce a subset of my original Lorentz invariants discussion.
...
-\inv{c^2} \int A(\Bk) \left( {\omega^2} - c^2 \Bk^2 \right)\exp\left( i \Bk \cdot \Bx - i \omega t \right) d^3 k
\end{align}
