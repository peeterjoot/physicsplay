...
We have \( 2 < 8.8 f \) for frequencies above 1 \si{GHz}, which is probably good enough to apply the low loss approximations.
...
The 10 \si{GHz} impedance is almost exactly the same as the low-loss approximation.
...
For lower frequencies, where the wavelength is larger, this is actually a fairly significant attenuation.  For the same respective frequencies we have
...
Let's see how the 
low-loss line approximation fares for both the real (\(\alpha\)) and imaginary (\(\beta\)) components of \( \gamma \)? For the same frequencies, with the \( \omega \) in \si{GHz} units, are

\begin{equation}\label{eqn:uwavesproblemSet1Problem2:260}
\begin{aligned}
\gamma &= 0.377 + 0.5305 j \omega \\
\gamma &= 0.3789 + 0.5279 j \omega \\
\gamma &= 0.3792 + 0.5274 j \omega \\
\gamma &= 0.3794 + 0.5271 j \omega.
\end{aligned}
\end{equation}

These compare well to the low-loss approximations using \( Z_0 = \sqrt{L/C}, \alpha = (R/Z_0 + G Z_0)/2, \beta = \omega \sqrt{LC} \)

\begin{equation}\label{eqn:uwavesproblemSet1Problem2:280}
\begin{aligned}
\gamma &= 0.3795 + 0.527 j \omega \\
\gamma &= 0.3795 + 0.527 j \omega \\
\gamma &= 0.3795 + 0.527 j \omega \\
\gamma &= 0.3795 + 0.527 j \omega .
\end{aligned}
