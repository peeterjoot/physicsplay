...
\imageFigure{../../figures/ece1236/deck7Fig1}{Single stage impedance matching.}{fig:deck7:deck7Fig1}{0.15}
...
I asked what business do we have in assuming that \( Z_\txtL/Z_0 \) is near unity?  The answer was that it isn't but suprisingly it works out well enough despite that.  As an example, consider \( Z_0 = 100 \Omega \) and \( R_\L = 50 \Omega \).  The exact expression

\frac{Z_\L - Z_0}{Z_\L + Z_0} 
= \frac{100-50}{100+50} 
= 
...
\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:820}
\Delta \Gamma = \frac{ (Z + \Delta Z) - Z }{(Z + \Delta Z) + Z} = \frac{\Delta Z}{2 Z}
\end{equation}

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:840}
\Delta Z \rightarrow 0
\end{equation}

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:860}
d\Gamma = \frac{dZ}{2 Z} = \inv{2} \frac{d (\ln Z/Z_0)}{dz} dz
\end{equation}
...

Hence as we did for multisection transformers, associate \( \Delta \Gamma \) with \( e^{- 2j \beta z} \) as sketched in 

F

assuming small reflections (i.e. \( Z(z) \) is a slowly varying function, ``adiabatic''  as the term in optics. Then

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:n}
\Gamma(\omega) = \int_0^L e^{ -2 j \beta z} d\Gamma
\end{equation}

\section{Exponential taper}

Let
\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:620}
Z(z) = Z_0 e^{a z}, \qquad 0 < z < L
\end{equation}

subject to
\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:640}
\begin{aligned}
Z(0) &= Z_0 \\
Z(L) &= Z_0 e^{a z} = Z_\txtL,
\end{aligned}
\end{equation}

which gives

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:660}
\ln \frac{Z_\txtL}{Z_0} = a L,
\end{equation}

or
\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:680}
a = \inv{L} \ln \frac{Z_\txtL}{Z_0}
\end{equation}

Also 

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:700}
\frac{d}{dz} \ln \frac{Z_\txtL}{Z_0} = \frac{d}{dz} \ln e^{a z}
\end{equation}

so

\begin{equation}\label{eqn:uwavesDeck7MultisectionTransformersCore:720}
\frac{d}{dz} (a z) = a.
\end{equation}

Hence 

\begin{dmath}\label{eqn:uwavesDeck7MultisectionTransformersCore:740}
\Gamma(\omega) 
...
\item  ...
\item The electrical length \( \beta L \), is ...
\end{enumerate}
...
\begin{dmath}\label{eqn:uwavesDeck7MultisectionTransformersCore:760}
\beta L = \pi,
\end{dmath}

or
\begin{dmath}\label{eqn:uwavesDeck7MultisectionTransformersCore:780}
\frac{\omega_c}{v_\phi} L = \pi
\end{dmath}

where \( \omega_c \) is the cutoff frequency.  This gives

\begin{dmath}\label{eqn:uwavesDeck7MultisectionTransformersCore:800}
...
Price to pay for this is that the zero is at \( 2 \pi \) so we have to make it twice as long to get the ripple down.
