...
Starting with \eqnref{eqn:distributions:startingPtFirstOrder} for now, we can form a functional pairing with a test function \(\psi\) in the Schwartz function space \(\Sw\) by integration.  That is
...
Suppose instead of the differential \eqnref{eqn:distributions:firstOrderAdvancedWave}, we couple this entire equation using integration to a function in \(\Sw\).  That is
...
Now let us switch gears and make the functional nature of \(\hat{f}\) explicit.  For \(\psi(\Bk) \in \Sw\), we want solutions
of
