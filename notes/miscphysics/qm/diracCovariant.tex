...
It is slightly tempting to re-derive this, with the inclusion of the $\hbar$ and $c$ factors.  However, a bit of play using the Geometric Algebra (GA) operators discussed in \cite{doran2003gap} proves more productive.  This text introduces a purely GA formalism for the Dirac equation which I will not use here.  Instead I will use the more conventional Feynman slash notation so that a four vector with coordinates $a^\mu$ is written with its basis as
...
The bigger question of why \ref{eqn:diracCovariant:520} is the usual transformation remains.  I do not know the answer for that one.  It is common in Lagrangian transformation discussions for the Klein-Gordon equation, and I am guessing it is something better justified in places I have not yet read.
