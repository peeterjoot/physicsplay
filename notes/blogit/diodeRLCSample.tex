...
First evaluating the derivatives, and then evaluating the result at each of the Nykvist times \( t_k \), yields a \( (2 N + 1) \times 1 \) matrix equation
...
\end{bmatrix}}_{k \in [-N, N]}
\end{equation}

%The time dependence in the linear terms is nicely taken of by this transformation to the frequency domain.  This 
%
%
%However, we have a fairly messy structure with sums of Fourier components instead of the nice Fourier component vectors that we see in \S A.4 of \citep{giannini2004NonlinearMicrowaveCircuitDesign}.  
...
a block matrix form of \cref{eqn:diodeRLCSample:80} is now close
...
What is desired here is to factor out a matrix of the form \( {\begin{bmatrix} {\BV^{(1)}}^\T & {\BV^{(1)}}^\T & \cdots \end{bmatrix}}^\T \).  How can this be done?

%
%FIXME: this is where the errors occur.  Went from \( t \) to \( t_k \), but shoudl have an array here over all the values of \( k \).
%
