...
We've been using the fact that a period function
...
Now let's express the explicit form for the Fourier coefficent $V_\BG$ so that we can compute the Fourier representation for some periodic potentials for some numerical experimentation.  In particular, let's think about what it meant to integrate over a unit cell.  Suppose we have a parameterization of the points in the unit cell
...
\imageFigure{fourierReciprocalUnitCellFig1}{Unit cell}{fig:fourierReciprocalUnitCell:fourierReciprocalUnitCellFig1}{0.3}
...
Picking the $u$ integral of this integrand as representitive, we have when $r = r'$
