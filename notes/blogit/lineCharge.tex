...
The limit of this potential \( a = -\Delta/2 \rightarrow -\infty, b = \Delta/2 \rightarrow \infty \) doesn't exist in any strict sense.  If we are cavilier about the limits, as in
...
For has a PV limit of \cref{eqn:lineCharge:40} at \( z = 0 \), and also for the finite case, has the \( \zcap \) field component that was obtained when the field
...
I think that were things go wrong is that the integral of \cref{eqn:lineCharge:180} does not apply to charge distributions that are not finite on the infinite ball \( z \in [-\infty, \infty] \).  That solution was obtained by utilizing an all-space Green's function, and the boundary term in that Green's analysis was assumed to tend to zero.  That isn't the case when the charge distribution is \( \lambda_0 \delta( z ) \).
