...
\newcommand{\keywords}{Discrete Fourier transform, PHY487}
...
For decoupling trial solutions of lattice vibrations we have what appears to be a need for the use of a discrete Fourier transform.  This is described briefly in \citep{haykin1994cs}, but there's no proof of the inversion formula.  Let's work through that detail.
...
Observe that $k' - k \in [-N+1, N-1]$, and neccessarily takes on just integer values.  We have terms of the form $\sin \pi m$, for integer $m$ in the numerator, always zero.  In the denominator, the sine argument is in the range
...
\imageFigure{discreteFourierRangeFig1}{Sine with integer multiples of $\pi$ omitted}{fig:discreteFourierRange:discreteFourierRangeFig1}{0.2}
...
\begin{dmath}\label{eqn:discreteFourierTransform:160}
\myBoxed{
\begin{aligned}
...
\begin{dmath}\label{eqn:discreteFourierTransform:200}
\myBoxed{
