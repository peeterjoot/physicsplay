...
\beginArtNoToc
\generatetitle{ECE1254H Modeling of Multiphysics Systems.  Lecture 14: XXX.  Taught by Prof.\ Piero Triverio}
%\chapter{XXX}
...
Assuming that matrices \( G, C \) are constant, and \( \Delta t \) is fixed, a matrix inversion can be avoided, and a single LU decomposition can be used.  For \( N \) sampling points (not counting \( t_0 = 0 \)), \( N \) sets of backward and forward subsitutions will be required to compute \( x_1 \) from \( x_0 \), and so forth.
...
+ \Delta t B u(t_n) 
\end{equation}
\item This is an explicit method
\item This can be cheap but instable.
\end{itemize}
...
\imageFigure{../../figures/ece1254/lecture14Fig2}{Trapazoidal derivative approximation}{fig:lecture14:lecture14Fig2}{0.3}
...
Applicaiton to \cref{eqn:multiphysicsL14:40} for \( t_{n-1}, t_n \) respectively gives
...
This is
\begin{itemize}
\item an implicit method
\item ...
\item ... BE, for the same computational cost
\end{itemize}
