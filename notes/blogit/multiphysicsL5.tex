...
However, even with strict dominance, we can have trouble with ill posed (pertubative) systems.
...
\section{Exploring uniqueness and existance}
...
With a linear dependency an additional solution, given solution \( \overbar{x} \) is \( \overbar{x}^1 = \overbar{x} + \alpha y \).  This becomes relavant for numerical processing since for a system 
...
\section{Pertubation and norms}

Consider a pertubation to the system \( M \overbar{x} = \overbar{b} \)
...
\Abs{ \overbar{x} }_1 = \sum_i \Abs{ x_i }
\end{dmath}

\item \(L_2\) norm

\begin{dmath}\label{eqn:multiphysicsL5:280}
\Abs{ \overbar{x} }_2 = \sqrt{ \sum_i \Abs{ x_i }^2 }
\end{dmath}

\item \(L_\infty\) norm

\begin{dmath}\label{eqn:multiphysicsL5:300}
\Abs{ \overbar{x} }_\infty = \max_i \Abs{ x_i }.
\end{dmath}
\end{itemize}
...
\Abs{ M } = \max_{ \Abs{\overbar{x}} = 1 } \Abs{ M \overbar{x} },
\end{dmath}

and the 2-norm is

\begin{dmath}\label{eqn:multiphysicsL5:360}
\Abs{ M }_2 = \max_{ \Abs{\overbar{x}}_2 = 1 } \Abs{ M \overbar{x} }_2.
\end{dmath}
