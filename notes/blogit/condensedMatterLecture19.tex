...
Last time we noted that we can't use plain Bloch waves to model this, but must introduce a wave packet

F1

centered on some $\Bk$, moving with group velocity \index{group velocity}
...
Here $m^\conj$ is the \textAndIndex{effective mass}.  Keep in mind that the Fermi surface is often not spherical as in
...
Wave vector of wave package advances steadily, provided we ignore scattering (for now).
...
the \textAndIndex{effective mass tensor}, which represents ``resistance'' to applied force.  For example
