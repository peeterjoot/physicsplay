...
and formally make the identification $E = 3 \omega \hbar/2 = (1 + 1/2) \omega \hbar$, this isn't a normalizable wavefunction, and has no physical relavence, unless we set $B = 0$.
...
The question of probabily per unit length is much easier asked of a pure state, for which we have just a single contributor
...
\begin{align*}
\text{Probabilty that particle is found between -0.5 and 0.5}
&=
...
&= 
\inv{ \sqrt{\pi} }
\int_{-{\alpha/2}}^{\alpha/2} e^{-\xi^2} dx 
= 
\sqrt{\frac{2}{\pi}} \erf({\alpha/2}) \\
%%%
\inv{ 2 \sqrt{\pi} }
\int_{-{\alpha/2}}^{\alpha/2} H_1^2(\xi) e^{-\xi^2} d\xi 
&= 
\frac{2}{ \sqrt{\pi} }
\int_{-{\alpha/2}}^{\alpha/2} \xi^2 e^{-\xi^2} dx 
= 
...
&= 
\inv{ 2 \sqrt{\pi} }
\int_{-{\alpha/2}}^{\alpha/2} (\xi^2 -1)^2 e^{-\xi^2} dx 
= 
\frac{3}{8} \erf({\alpha/2}) + \frac{1}{8 \sqrt{\pi}} \alpha e^{-\alpha^2/4} (1 - \alpha^2/2)
\end{align*}

To get a feel for this observe that we have $m_e/\hbar = 8 637.99381 s / m^2$, so for an electron, unless $\omega$ is very small our parameter $\alpha = \sqrt{m \omega/\hbar}$ is fairly large.  This means that we can make the approximations $\erf(\alpha/2) \approx 1$, and $e^{-\alpha^2} \approx 0$.  This gives us probabilities per unit length for the interval around $x = 0$ as
