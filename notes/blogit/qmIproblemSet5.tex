...
For such a potential, within the harmonic well, a general solution of the form

\begin{equation}\label{eqn:qmIproblemSet5:19}
u(x,t) = \sum_n H_n(\xi) \Bigl(A_n e^{-\xi^2/2} + B_n e^{\xi^2/2} \Bigr) e^{-i E_n t/\hbar},
\end{equation}

is possible since normalization would not prohibit non-zero $B_n$ values in that situation.  The problem would become one of matching the boundary value constraints.
...
We cannot answer the question for the probability that the particle is found at the specific $x=0$ position at $t=t_0$ (that probability is zero in a continuous space), but we can answer the question for the probability that a particle is found in an interval surrounding a specific point at this time.  We could calculate the average probability in the unit interval $[-1/2,1/2]$ surrounding the point $x = 0$.  It would also be possible to calculate the average probability for an increasingly small interval, and could define this as the probability per unit length for an interval surrounding $x = x_0$ as

\begin{equation}\label{eqn:qmIproblemSet5:17}
P = \text{Probability per unit length} = 
\lim_{\epsilon \rightarrow 0} \inv{\epsilon} \int_{x_0 - \epsilon/2}^{x_0 + \epsilon/2} \Abs{ \Psi(x, t_0) }^2 dx = \Abs{\Psi(x_0, t_0)}^2
\end{equation}

So the probability per unit length is just the probability density itself, evaluated at the point of interest.  For the impure state of \ref{eqn:qmIproblemSet5:20} we have for the probability density
...
It is interesting that the time dependence of the probability per unit length only has the time dependence for a mixed state.
...
This is zero for odd $n$.  For even $n$ is appears that $H_n(0)^2$ may equal $2^n$ (this is true at least up to n=4).  If that's the case we have for even energy states an $x=0$ probability per unit length value of $\Abs{u(0,t_0)}^2 = \frac{\alpha}{\sqrt{\pi} n!}$.

\subsubsection{Probability for the non-normalizable state earlier in the problem statement.}

It is possible, albeit odd (but consistent with the lack of details in the problem statement), that this question was asking for the probability per unit length for the non-normalizable $E_1$ wavefunction $u(x) = B \xi e^{\xi^2/2}$.  Since normalization requires $B=0$, that probability density is simply zero (or undefined, depending on one's point of view).
