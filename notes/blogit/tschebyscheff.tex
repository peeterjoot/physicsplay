...
\lr{ 1 - x^2} \frac{d}{dx} T_n(x) + n x T_n(x) - n T_{n-1}(x)
=
\sin^2\theta 
\lr{
-\frac{1}{\sin\theta} \frac{d}{d\theta}}
\cos( n \theta ) + n \cos\theta \cos( n \theta ) - n \cos( (n-1)\theta )
=
-\sin\theta (-n \sin(n \theta)) + n \cos\theta \cos( n \theta ) - n \cos( (n-1)\theta ).
\end{dmath}

Applying the cosine addition formula \cref{eqn:tschebyscheff:540} gives

\begin{dmath}\label{eqn:tschebyscheff:560}
n \lr{ \sin\theta \sin(n \theta) + \cos\theta \cos( n \theta ) } - n 
\lr{
...
\cref{fig:Chebychev:ChebychevFig1}.
