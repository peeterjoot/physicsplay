...
%\usepackage{media9}
\chapter{Spin down of coffee in a bottomless cup.}

\label{chap:bottomlessCoffee}
%\useCCL
\blogpage{http://sites.google.com/site/peeterjoot2/math2012/bottomlessCoffee.pdf}
\date{Apr 24, 2012}
\gitRevisionInfo{bottomlessCoffee}
\keywords{PHY454H1S, PHY454H1, coffee, Navier-Stokes, spin}
...
Here's a variation of a problem outlined in \S 2 of \cite{acheson1990elementary}, which looked at the time evolution of fluid with initial rotational motion, after the (cylindrical) rotation driver stops, later describing this as the spin down of a cup of tea.  I'll work the problem in more detail than in the text, and also make two refinements.
...
We'll work in cylindrical coordinates following the conventions of figure (\ref{fig:coffeeCupDiagram:coffeeCupDiagramFig1}).

\imageFigure{bottomlessCoffeeFig1}{Fluid flow in nested cylinders.}{fig:coffeeCupDiagram:coffeeCupDiagramFig1}{0.3}
...
Let's first verify that this meets the non-compressible condition that eliminates the $\mu \spacegrad (\spacegrad \cdot \Bu)$ term from Navier-Stokes
...
\begin{align}\label{eqn:bottomlessCoffee:290}
A R_1 + \frac{B}{R_1} &= \Omega R_1 \\
A R_2 + \frac{B}{R_2} &= 0,
\end{align}

or
\begin{align}\label{eqn:bottomlessCoffee:310}
...
Checking \cite{abramowitz1964handbook} (9.1.1) we see that this can be put into the standard form of the Bessel equation if we eliminate the $\lambda$ term.  We can do that writing $z = r \lambda$, $\mathcal{R}(z) = R(z/\lambda)$ and noting that $r d/dr = z d/dz$ and $r^2 d^2/dr^2 = z^2 d^2/dz^2$, which gives us

\begin{equation}\label{eqn:bottomlessCoffee:450}
0 = z^2 \frac{d^2 \mathcal{R}}{dr^2} + z \frac{d \mathcal{R}}{dr} + \mathcal{R} \left( z^2 - 1 \right).
\end{equation}

The solutions are

\begin{equation}\label{eqn:bottomlessCoffee:470}
\mathcal{R}(z) = J_{\pm 1}(z), Y_{\pm 1}(z).
\end{equation}
...
Our boundary value constraint is that we require this to match \ref{eqn:bottomlessCoffee:370} at $t = 0$.  Let's write $R_2 = R$, $R_1 = a R$, $z = r/R$, so that we are working in the unit circle with $z \in [0, 1]$.  Our boundary problem can now be expressed as
...
Looking at \S 2.7 of \cite{sagan1989boundary} it appears the solutions for $c_i$ can be obtained from

\begin{equation}\label{eqn:bottomlessCoffee:570}
c_i = \frac{
\int_0^1 r\phi(z) J_1(\lambda_i z) dz}{
\int_0^1 r J_1^2(\lambda_i z) dz},
\end{equation}

where $\lambda_i$ are the zeros of $J_1$.

To get a feel for these, a plot of the first few of these fitting functions is shown in figure (\ref{fig:bottomlessCoffee:bottomlessCoffeeFig3}).

\imageFigure{bottomlessCoffeeFig3}{First four zero crossing Bessel functions $J_1( \lambda_i z)$}{fig:bottomlessCoffee:bottomlessCoffeeFig3}{0.2}

Using Mathematica in \href{https://raw.github.com/peeterjoot/physicsplay/master/notes/phy454/bottomlessCoffee.cdf}{bottomlessCoffee.cdf}, these coefficents were calculated for $a = 0.6$.  The $n = 1, 3, 5$ approximations to the fitting function are plotted with a comparision to the steady state velocity profile in figure (\ref{fig:bottomlessCoffee:bottomlessCoffeeFig2}).

\imageFigure{bottomlessCoffeeFig2}{Bessel function fitting for the steady state velocity profile for $n = 1, 3, 5$.}{fig:bottomlessCoffee:bottomlessCoffeeFig2}{0.2}
