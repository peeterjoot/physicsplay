...
%\usepackage{media9}
\chapter{Continuum mechanics review.}

\label{chap:continuumReview}
%\useCCL
\blogpage{http://sites.google.com/site/peeterjoot2/math2012/continuumReview.pdf}
\date{Apr 21, 2012}
\gitRevisionInfo{continuumReview}
\keywords{strain, displacement vector, stress, constitutive relation, Lame parameters, shear modulus, Cauchy tetrahedron, Uniform hydrostatic compression, Bulk modulus, Uniaxial stress, Young's modulus, Poisson's ratio, Compatibility condition,P-waves, S-waves, wave equation, displacement potentials, phasor, Love wave, Rayleigh wave}
...
Review of key ideas and equations from the class.
...
\begin{subequations}
\begin{equation}\label{eqn:continuumReview:50}
(d\Bx')^2 - (d\Bx)^2
= 2 e_{ij} dx_i dx_j
\end{equation}
\begin{equation}\label{eqn:continuumReview:70}
e_{ij}
=
\inv{2}
\left( 
...
\begin{equation}\label{eqn:continuumReview:90}
e_{ij}
=
\inv{2}
\left( 
...
\subsubsection{Unresolved: Relating displacement and position by strain}
...
\begin{equation}\label{eqn:continuumReview:110}
u_i = e_{ij} x_j.
\end{equation}

When we get to fluid dynamics we perform a linear expansion of $du_i$ and find something similar

\begin{equation}\label{eqn:continuumReview:530}
dx_i' - dx_i = du_i = \PD{x_k}{u_i} dx_k = e_{ij} dx_k + \omega_{ij} dx_k
\end{equation}

where

\begin{equation}\label{eqn:continuumReview:550}
\omega_{ij} = \inv{2} \left( \PD{x_i}{u_j} +\PD{x_j}{u_i} \right).
\end{equation}

Except for the antisymmetric term, note the structural similarity of \ref{eqn:continuumReview:110} and \ref{eqn:continuumReview:530}.  Why is it that we neglect the vorticity tensor in statics?
...
%\end{align*}
%
%This is easily seen to be the case, recovering \ref{eqn:continuumReview:90} by taking derivatives of \ref{eqn:continuumReview:110}, plus an assumption that $e_{ij}$ is symmetric.

\subsubsection{Diagonal strain representation.}
...
\begin{equation}\label{eqn:continuumReview:130}
...
\begin{equation}\label{eqn:continuumReview:150}
...
\begin{equation}\label{eqn:continuumReview:230}
...
The cylindrical tensor coordinates of \ref{eqn:continuumReview:210} follow from 
\ref{eqn:continuumReview:230} nicely taking $\ncap = \rcap, \phicap, \zcap$ in turn.
...
\begin{equation}\label{eqn:continuumReview:510}
...
\begin{equation}\label{eqn:continuumReview:250}
...
\begin{equation}\label{eqn:continuumReview:270}
...
\begin{equation}\label{eqn:continuumReview:330}
...
\begin{equation}\label{eqn:continuumReview:350}
...
\begin{equation}\label{eqn:continuumReview:370}
\sigma_{ij} = -p e_{ij}.
\end{equation}

With $e_{ii} = (dV' - dV)/dV = \Delta V/V$, we formed the Bulk modulus $K$ with the value

\begin{equation}\label{eqn:continuumReview:390}
...
\begin{equation}\label{eqn:continuumReview:410}
...
\begin{equation}\label{eqn:continuumReview:430}
...
\begin{equation}\label{eqn:continuumReview:450}
\mu = \frac{E}{2(1 + \nu)}
\end{equation}

\begin{equation}\label{eqn:continuumReview:470}
\lambda = \frac{E \nu}{(1 - 2 \nu)(1 + \nu)}
\end{equation}

\begin{align}\label{eqn:continuumReview:490}
...
\begin{equation}\label{eqn:continuumReview:590}
...
\begin{equation}\label{eqn:continuumReview:610}
\rho \PDSq{t}{\Bu} = (\lambda + \mu) \spacegrad (\spacegrad \cdot \Bu) + \mu \spacegrad^2 \Bu.
\end{equation}

\subsubsection{P-waves}

Operating on \ref{eqn:continuumReview:610} with the divergence operator, and writing $\Theta = \spacegrad \cdot \Bu$, a quantity that was our relative change in volume in the diagonal strain basis, we were able to find this divergence obeys a wave equation

\begin{equation}\label{eqn:continuumReview:630}
...
Similarly, operating on \ref{eqn:continuumReview:610} with the curl operator, and writing $\Bomega = \spacegrad \cross \Bu$, we were able to find this curl also obeys a wave equation

\begin{equation}\label{eqn:continuumReview:650}
...
\begin{equation}\label{eqn:continuumReview:670}
\frac{C_L}{C_T} = \sqrt{\frac{ \lambda + 2 \mu}{\mu}} = \sqrt{ \frac{\lambda}{\mu} + 2}.
\end{equation}

\subsubsection{Scalar and vector potential representation.}

Assuming a vector displacement representation with gradient and curl components

\begin{equation}\label{eqn:continuumReview:690}
...
\begin{equation}\label{eqn:continuumReview:710}
\spacegrad
\left(
...
\begin{subequations}
\begin{equation}\label{eqn:continuumReview:730}
\rho \PDSq{t}{\phi} - (\lambda + 2\mu) \spacegrad^2 \phi = 0
\end{equation}
\begin{equation}\label{eqn:continuumReview:750}
\rho \PDSq{t}{\BH} - \mu \spacegrad^2 \BH
= 0.
\end{equation}
\end{subequations}
...
\begin{equation}\label{eqn:continuumReview:810}
...
\begin{equation}\label{eqn:continuumReview:830}
...
\begin{subequations}
\begin{equation}\label{eqn:continuumReview:850}
e_{ij} = \inv{2} \left(
\PD{x_j}{u_i} +
\PD{x_i}{u_j} \right).
\end{equation}
\begin{equation}\label{eqn:continuumReview:870}
...
\begin{equation}\label{eqn:continuumReview:890}
...
\begin{subequations}
\begin{equation}\label{eqn:continuumReview:n}
...
\begin{subequations}
\begin{equation}\label{eqn:continuumReview:910}
\Bomega = \spacegrad \cross \Bu
\end{equation}
\begin{equation}\label{eqn:continuumReview:930}
...
\begin{equation}\label{eqn:continuumReview:950}
