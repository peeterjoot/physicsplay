...
Comparing to \ref{eqn:qmIproblemSet4:155a} from the attractive delta function completes the problem.  The conclusion is that when the finite well is narrowed with $a \rightarrow zero$, also letting $V_0 \rightarrow \infty$ such that the absolute area of the well $g = (2 a) V_0$ is maintained, the finite potential well produces exactly the attractive delta function wave function and associated bound state energy.
...
r^2 \sin\theta 
{r'}^2 \sin\theta' 
R_{lm}(r) Y_{lm}^\conj(\theta, \phi)
\bra{ r \theta \phi} A(R) \ket{ r' \theta' \phi'}
R_{lm}(r') Y_{lm}(\theta', \phi') \\
\end{align*}
...
As a consistency check, observe that with $A(r) = 1$, this integral evaluates to 1 according to equation (8.274) in the text, so we can think of $(r R_{lm}(r))^2$ as the radial probability density for functions of $r$.
...
The last integral evaluates to $6$, leaving
\begin{equation}\label{eqn:qmIproblemSet4:220}
1/ \bra{nlm} R \ket{nlm}
=
\frac{ 4 a_0 }{ Z } = n^2 \frac{a_0}{Z}
\end{equation}
...
\\
&=
\inv{24} \left( \frac{ Z }{ a_0 } \right)^3
\int_0^\infty
du 
u
e^{-u}.
...
This last integral has value $1$, and we have the second part of the computational task complete
\begin{equation}\label{eqn:qmIproblemSet4:225}
\bra{nlm} 1/R \ket{nlm} = \inv{24} \left( \frac{ Z }{ a_0 } \right)^3
\end{equation}

The question of whether or not \ref{eqn:qmIproblemSet4:220}, and \ref{eqn:qmIproblemSet4:225} are equal is answered.  HOWEVER, \ref{eqn:qmIproblemSet4:225} does not look dimensionally correct!
