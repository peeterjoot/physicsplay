...
\begin{itemize}
\item 
\href{https://raw.github.com/peeterjoot/matlab/master/ece1254/ps1/generateResistorGridNetlist.m}{generateResistorGridNetlist.m}
\item 
\href{https://raw.github.com/peeterjoot/matlab/master/ece1254/ps1/withPivotLU.m}{withPivotLU.m}
\item 
\href{https://raw.github.com/peeterjoot/matlab/master/ece1254/ps1/tests/ps2a.mat}{tests/ps2a.mat},
\end{itemize}

where the last is the driver.  We aren't lucky enough in this problem to end up with system equations \( G \Bx = \Bb \) that can be \( L U \) factored without pivots, and have to use the more general (and slower) factorization routine that generates the factors \( G = L' U\), where \( P L' \) is lower triangular, and \( P \) is a permutation matrix.  To solve such a system we do a forward substitution to solve for \( \By \) in
...
% n(i,j)
For a system that has the random current sources specified in \cref{tab:multiphysicsProblemSet1Problem2:1}, the voltage surface plot for this network is plotted with surf in \cref{fig:ps1p2a:ps1p2aFig1}.
...
\imageFigure{../../figures/ece1254/ps1p2aFig1}{50 node resistor network plot}{fig:ps1p2a:ps1p2aFig1}{0.3}

%This plot is a more erratic looking than I would have expected, which makes me wonder if I have made an error in the code that constructs the system equations from the netlist.

\makeSubAnswer{}{multiphysics:problemSet1:2b}

A plot of the cpu time vs node number is given in \cref{fig:ps1p2b:ps1p2bFig3}.

\imageFigure{../../figures/ece1254/ps1p2bFig3}{Cpu times for range of grid sizes}{fig:ps1p2b:ps1p2bFig3}{0.3}
...
For grid sizes \( n \in \{ 4,8,12,16,20,24,28,32,36,40,44 \} \), we find

\begin{equation}\label{eqn:multiphysicsProblemSet1Problem2:n}
t(n) = 197 \times 10^{-6} n^{2.5}
\end{equation}

This linear fit of the log log plot of timings vs grid sizes is plotted along with raw data in \cref{fig:ps1p2c:ps1p2cFig4}.  The corresponding power fit and the original timing data is plotted in \cref{fig:ps1p2c:ps1p2cFig5}.

\imageFigure{../../figures/ece1254/ps1p2cFig4}{Log log plot of timings vs grid sizes and its linear fit}{fig:ps1p2c:ps1p2cFig4}{0.3}

\imageFigure{../../figures/ece1254/ps1p2cFig5}{Power law fit with origina timing data}{fig:ps1p2c:ps1p2cFig5}{0.3}

\makeSubAnswer{}{multiphysics:problemSet1:2d}

Notice that the linear fit is not actually a good match for large \( n \), but we have time \( O(2.4) \) for the \( L U \) decomposition for small \( n \).  For large \( n \) the sparsity of the system generates a large amount of fill in work.
}
