...
\begin{subequations}
\label{eqn:multiphysicsProblemSet3b:230}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:240}
\ddt{w^1(t)} + \alpha w^1 = h^1 u(t)
\end{equation}
...
These must be supplemented with the boundary value constraints

\begin{equation}\label{eqn:multiphysicsProblemSet3b:500}
\begin{aligned}
w^1 - w^2 &= 0 \\
w^N - w^{N+1} &= 0.
\end{aligned}
...
After substitution into \cref{eqn:multiphysicsProblemSet3b:230}, the system with boundary value constraints is

\begin{subequations}
\label{eqn:multiphysicsProblemSet3b:520}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:540}
\ddt{w^2(t)} + \alpha w^2 = h^1 u(t)
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:560}
\ddt{w^{2}(t)}   
+ \alpha w^{2} = h^{2} u(t)
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:580}
\ddt{w^3(t)}
-
\inv{\lr{\Delta x}^2} 
\lr{ 
...
+ \alpha w^{3}(t) = h^3 u(t)
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:600}
\ddt{w^i(t)}
-
\inv{\lr{\Delta x}^2} 
\lr{ 
...
+ \alpha w^{i}(t) = h^i u(t), \qquad i \in [4, N]
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:620}
\ddt{w^{N}(t)} + 
\frac{w^N - w^{N-1}}{\lr{\Delta x}^2}
+ \alpha w^{N} = h^{N+1} u(t).
\end{equation}
...
FIXME: need a better way.  Introduce currents?

To put into the form of \cref{eqn:multiphysicsProblemSet3b:60}, let
\begin{subequations}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:300}
\BG = 
\begin{bmatrix}
\alpha \phantom{ - N^2} &               &               &               &                 &        \\
\phantom{2} N^2         & \alpha - N^2  &               &               &                 &        \\
-N^2                    & 2 N^2         & \alpha -N^2   &               &                 &        \\
                        & -N^2          & 2 N^2         & \alpha -N^2   &                 &        \\
                        &               & \qquad\ddots  &               &                 &        \\
                        &               & -N^2          & 2 N^2         & \alpha -N^2     &        \\
                        &               &               & -N^2          & \phantom{2} N^2 & \alpha \phantom{ -N^2} \\
\end{bmatrix}
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:480}
\BB =
\begin{bmatrix}
h^1 &     &        &            &           \\
    & h^2 &        &            &           \\
...
\end{bmatrix}
\end{equation}
\begin{equation}\label{eqn:multiphysicsProblemSet3b:360}
\BC = \BI, \qquad
\Bx =
\begin{bmatrix}
w^1 \\
...
The vector \( \Bx \) represents a vector of temperatures at each of the sampling points.

FIXME: have to incorporate boundary value constraints.
...
The frequency response plotting function was implemented in \matlabFuncPath{PlotFreqResp}{ps3b:PlotFreqResp.m}().
