...
Note that Snell's law can also be expressed in terms of this critical angle, allowing for the solution of the transmittion angle in a convinient way
...
The propagation is chanelled along the x axis, but the propagation into the second medium decays exponentially (or unphysically grows exponentially), only getting into the surface a small amount.
...
For normal media where \( n_i > 0, n_t > 0 \), only the positive solution is physically relavent, which is
