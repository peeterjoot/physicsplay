...
The field plots are conceptually the simplest, since that vector parametrizes a surface.  Any such radial field with magnitude \( f(r, \theta, \phi) \) can be plotted in Mathematica in the \( \phi = 0 \) plane at \( r = r_0 \), or in 3D (respectively, but also at \( r = r_\)) with code like \cref{eqn:chapter2Notes:600}
