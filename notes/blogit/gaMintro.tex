...
k^2 
&= \Be_2 \Be_3 \Be_2 \Be_3 \\
&= -\Be_3 \myMathWithDescription{\Be_2 \Be_2}{$=1$}{1} \Be_3 \\
&= -\Be_3 \Be_3 \\
&= -1.
\end{align*}
...
&=
\Be_2 \cos\theta + \myMathWithDescription{\Be_2 \Be_2}{$=1$}{1} \Be_3 \sin\theta.
\end{align*}
...
\begin{align}\label{eqn:gaMintro:mp60}
...
\begin{align}\label{eqn:gaMintro:mp170}
\Bt^2 = 
\Br^2 \cos^2 \theta
+\Bs^2 \cos^2 \alpha
...
The squared speed of the unit polar vector by multiplying \ref{eqn:gaMintro:mp180} with \ref{eqn:gaMintro:mp190}, like so
...
As noted in \ref{eqn:mp80}, we can express the unit spherical vector using geometric algebra as
...
\dot{\rcap}^2 = 
\dot{\theta}^2 \myMathWithDescription{\Be_1 e^{i\phi} e^{j \theta} e^{-j\theta} e^{-i \phi} \Be_1 }{$=1$}{1}
+ S_\theta^2 \dot{\phi}^2 \myMathWithDescription{\Be_2 e^{i \phi} e^{-i \phi} \Be_2 }{$=1$}{2}
+ 
\dot{\theta} 
\dot{\phi}
S_\theta
