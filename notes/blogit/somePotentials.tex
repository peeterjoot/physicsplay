...
Suppose we could also validly argue that this tends to $\ln(1) = 0$, and the difference of square roots could also be cancelled.  Then we would be left with just
...
This differs from the expected result by a factor of $-2$, and we've had to do some very fishy root taking to even get that far.  Employing l'H\hat{o}pital's rule (or letting Mathematica attempt to evaluate the limit), we get infinities for the difference of logarithm term.
