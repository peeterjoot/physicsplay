...
We find that the average of the Heisenberg position operator evolves in time in exactly the same fashion as position in the classical Harmonic oscillator.
...
=
\frac{\Hbar}{ 2 m \omega}
\lr{ z^2 + {(z^\conj)}^2 + z^\conj z + z^\conj z + 1}  \ket{z}
=
\frac{\Hbar}{ 2 m \omega}
\lr{ z + z^\conj }^2
\frac{\Hbar}{ 2 m \omega}.
...
In this Hamiltonian the vector \( \Bp \) is called the canonical momentum, the momentum conjugate to position in phase space, and the particle velocity is \( \Bv = \lr{ \Bp - q \BA }/m \).
...
\Hhat = \inv{2 m} \lr{ \hat{\Bp} - q \hat{\BA}(\Br, t) }^2 + q \phi(\Br, t).
\end{dmath}
...
FIXME: subst to show invariance of the magnetic and electric fields.
