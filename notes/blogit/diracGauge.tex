...
but this has to be expressed in terms of $\BP$.  Having found the form of the Hamiltonian for the interaction case, it is easily verified that \ref{eqn:diracGauge:90} is the required form
...
\begin{align}\label{eqn:diracGauge:200}
\mathcal{E}   &\rightarrow \mathcal{E} - e\phi \\
...
+\left(\frac{\mathcal{E}}{c} - \frac{e}{c} \phi \right) 
- \gamma_0 m c \\
\end{align*}
...
The motivation for this appears to be that 

\begin{equation}\label{eqn:diracGauge:n}
(\pslash + m c) \gamma_0 \gamma_0 (\pslash - m c) 
=
(\pslash + m c) (\pslash - m c) 
= \inv{c^2} \left( \mathcal{E} - \frac{e}{c} \phi \right)^2
 -\left( \Bp - \frac{e}{c} \BA \right)^2 - (m c)^2 + \cdots,
\end{equation}

which is the Klein-Gordon equation, plus cross terms to be determined.
