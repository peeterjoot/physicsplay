...
\paragraph{At point (a)} we have ${E_{k }^\nought} = {E_{k -G}^\nought}$, and can get a rough idea of the separation by further dropping $C_{k - G}, C_{k + G}, C_{k + 2G}$ terms, so that the system to solve is
...
\paragraph{At point (b)} we have ${E_{k - G}^\nought} = {E_{k +G}^\nought}$, and dropping all but the $C_{k - G}, C_{k + G}$ terms gives us
...
This implicit function has two discrete solutions, as illustrated by representitive plots in \cref{fig:ps7p2c:ps7p2cFig1}.  This could clearly be solved numerically.

\imageFigure{ps7p2cFig1}{implicit function curves for delta-function-potential energy}{fig:ps7p2c:ps7p2cFig1}{0.3}

None of this helps with an estimate of the gap at the crossing points.  Considering point (a) as representitive, let's make a crude approximation of \eqnref{eqn:condensedMatterProblemSet7Problem2:640} as
