...
The system is observable if \( \forall t \), the initial state \( \Bx(t_0) \) can bre determined knowing \( \Bu(t), \By(t), \qquad t \in [t_0, t_1] \).
...
The system is controlable if and only if the controllability Grammian \( \BW_c > 0 \).
}
...
if \( \Bx_1 \) is an eigenvector of \( \BW_c \) with eigenvvalue \( \lambda_1 \), then this reduces to
...
The purpose of considering observability and controllability is to help determine what modes can be safely discarded to reduce the model order.  A state that cannot be achived without excessive energy cost is not likely to be of interest.

While the explicit form of the controlability Gramian has not been given, it becomes of less interest since it is possible to make a change of variables, such that the controllability and observability Gramians become equal and diagonal.

\begin{dmath}\label{eqn:multiphysicsProblemSet3b:240}
\begin{aligned}
...
Using these provides a balanced method to detemine the most important modes to retain.  
