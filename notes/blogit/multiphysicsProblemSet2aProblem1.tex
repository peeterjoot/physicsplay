...
The Gershgorin circles and eigenvalues were plotted with \textbf{partH}(0).  
Plots for \( R = 0.1, 1, 10 \) can be found respectively in \cref{fig:gershgorinPlotR0_1:gershgorinPlotR0_1Fig6}, \cref{fig:gershgorinPlotR1:gershgorinPlotR1Fig8}, and \cref{fig:gershgorinPlotR10:gershgorinPlotR10Fig7}.
...
The Gershgorin circles and eigenvalues with tridiagonally preconditioned \( G \) were plotted with \textbf{partH}(1).  
Plots for \( R = 0.1, 1, 10 \) can be found respectively in \cref{fig:gershgorinPlotPreconditionedR0_1:gershgorinPlotPreconditionedR0_1Fig9}, \cref{fig:gershgorinPlotPreconditionedR10:gershgorinPlotPreconditionedR10Fig11}, and \cref{fig:gershgorinPlotPreconditionedR1:gershgorinPlotPreconditionedR1Fig10}.
...
The imaginary component of the eigenvalues observed in these plots show that the transformation \( G \rightarrow P^{-1/2} G P^{-1/2} \) was not a symmetric-matrix preserving transformation.  It was effective in rescaling the eigenvalues of the problem, especially for \( R = 0.1 \).  Having seen that the original non-symmetric (voltage source) nodal matrix system was not solvable using CG, I wonder if it is it possible that the destruction of the symmetric nature of the matrix made the iteration converge more slowly?  Note that the tridiagonal preconditioner used in this case was nothing more fancy than the original matrix \( G \) with all non-tridiagonal terms zeroed.
