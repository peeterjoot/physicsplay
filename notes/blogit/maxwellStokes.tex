...
where \( \grad = \gamma^\mu \partial_\mu \) is the spacetime gradient, \( F = \BE + I c \BB \) is the electromagnetic field bivector, and \( J = (c\rho, \BJ) \) is the four (vector) current.
...
This can clearly be further simplified by using the Lorentz gauge, where \( \grad \cdot A = 0 \).  However, the aim for now is to try applying Stokes theorem to Maxwell's equation.  The dual form \eqnref{eqn:maxwellStokes:100} has the curl structure required for the application of Stokes.  Suppose that we evaluate this curl over the three parameter volume element \( d^3 x = \gamma_0 \gamma_1 \gamma_2 dx^0 dx^1 dx^2 \)
...
This uses the distibution identity \( A_s \cdot (a \wedge A_r) = (A_s \cdot a) \cdot A_r \) which holds for blades \( A_s, A_r \) provided \( s > r > 0 \).  Observe that only the component of the gradient that lies in the tangent space of the three volume manifold contributes to the integral.  That means that we can apply Stokes theorem

For the surface area element, we can write
\begin{dmath}\label{eqn:maxwellStokes:140}
d^2 x = \sum_{\mu \ne 3} [FIXME: the non-form part here.]\gamma^\mu 
\end{dmath}
...
\int_{\partial V} d^2 x \wedge F 
= \frac{1}{c \epsilon_0} \int_V d^3 x \wedge J.
\end{dmath}
