...
and for \( m \ne n \) we can pick up the electric field components
\begin{dmath}\label{eqn:maxwellStokes:n}
(\gamma_m \gamma_n ) \cdot \partial_\mu G
=
\lr{ \gamma_m \gamma_n } \cdot \partial_\mu \lr{
\inv{2} \epsilon^{r s t} \gamma_r \gamma_s E^t 
}
\end{dmath}
...
\inv{I} d^2 x \wedge F = 
- \evalbar{E^3}{\Delta z} dy dx 
- \evalbar{E^2}{\Delta y} dx dz 
- \evalbar{E^1}{\Delta x} dz dy 
\end{aligned}
\end{dmath}

Putting everything together, and letting the volume element tend to zero (essentially undoing the Stokes integration) we have
\begin{dmath}\label{eqn:maxwellStokes:420}
\begin{aligned}
c \int_V dx dy dt \lr{ - \inv{c} \PD{t}{E^3} + c \lr{ \PD{x}{B^2} - \PD{y}{B^1} } } &= c \int_V dx dy dt \inv{c \epsilon_0} J^3 \\
c \int_V dx dz dt \lr{ \inv{c} \PD{t}{E^2} + c \lr{ \PD{x}{B^3} - \PD{z}{B^1} } } &= -c \int_V dx dy dt \inv{c \epsilon_0} J^2 \\
c \int_V dy dz dt \lr{ -\inv{c} \PD{t}{E^1} + c \lr{ \PD{y}{B^3} - \PD{z}{B^2} } } &= c \int_V dx dy dt \inv{c \epsilon_0} J^1 \\
\int_V dx dy dz \lr{ - \PD{x}{E^1} - \PD{y}{B^2} - \PD{z}{B^3} } &= -\int_V dx dy dz \inv{\epsilon_0} \rho,
\end{aligned}
\end{dmath}

or
\begin{dmath}\label{eqn:maxwellStokes:440}
\begin{aligned}
...
These are the Ampere-Maxwell equation and Gauss's law respectively.
} % answer
