...
\spacegrad \wedge \BB = j \omega \epsilon_0 \mu_0 \BE I
\end{dmath}
...
\cref{eqn:phasorDualMaxwellsGA:420} shows that the divergence of the dual electric field is zero.  It it therefore possible to find a potential \( \BF \) such that

\begin{dmath}\label{eqn:phasorDualMaxwellsGA:460}
-\BE I = \inv{\epsilon_0}\spacegrad \wedge \BF.
\end{dmath}

FIXME: everything after here only partially dual transformed.

Substituting this into the dual-Maxwell-Faraday \cref{eqn:phasorDualMaxwellsGA:360} gives

\begin{dmath}\label{eqn:phasorDualMaxwellsGA:480}
\spacegrad \wedge \lr{ \BE + j \omega \BF } = 0.
\end{dmath}

This relation is a bivector identity with zero, so will be satisfied if 

\begin{dmath}\label{eqn:phasorDualMaxwellsGA:500}
\BE + j \omega \BF = -\spacegrad \phi_m,
\end{dmath}

for some scalar \( \phi_m \).  Unlike the \( \BB I = \spacegrad \wedge \BF \) solution to \cref{eqn:phasorDualMaxwellsGA:420}, the grade of \( \phi_m \) is fixed by the requirement that \( \BE + j \omega \BF \) is unity (a vector), so
...
Substitution of \cref{eqn:phasorDualMaxwellsGA:500} and \cref{eqn:phasorDualMaxwellsGA:460} into Ampere's law \cref{eqn:phasorDualMaxwellsGA:380} gives

\begin{equation}\label{eqn:phasorDualMaxwellsGA:520}
\begin{aligned}
-\spacegrad \cdot \lr{ \spacegrad \wedge \BF } &= \mu_0 \BM + j \omega \epsilon_0 \mu_0 \lr{ -\spacegrad \phi_m -j \omega \BF } \\
...
\spacegrad^2 \BF + k^2 \BF = -\mu_0 \BM - \spacegrad \lr{ \spacegrad \cdot \BF + j \frac{k}{c} \phi_m }.
\end{dmath}
...
\( A^\mu = \lr{\phi_m/c, \BF} \), and expanding the Lorentz gauge condition

\begin{dmath}\label{eqn:phasorDualMaxwellsGA:580}
0 
= \partial_\mu \lr{ A^\mu e^{j k c t}}
= \partial_a \lr{ A^a e^{j k c t}} + \inv{c}\PD{t}{} \lr{ \frac{\phi_m}{c} e^{j k c t}}
...
A = A^\mu \gamma_\mu = \lr{ \phi_m/c, \BF }
\end{equation}
...
J = J^\mu \gamma_\mu = \lr{ c \rho_m, \BM },
\end{equation}
\end{subequations}
...
c \rho_m - \BM 
= 
\gamma_0 \lr{ \gamma_0 c \rho_m - \gamma_0 \gamma_a \gamma_0 J^a }
= 
\gamma_0 \lr{ \gamma_0 c \rho_m + \gamma_a J^a }
= 
...
\grad \wedge A 
=
\lr{ \gamma^a \partial_a + \gamma_0 j k } \wedge \lr{ \gamma_0 \phi_m/c + \gamma_b A^b }
=
- \sigma_a \partial_a \phi_m/c + \gamma^a \wedge \gamma_b \partial_a A^b - j k \sigma_b A^b
=
- \sigma_a \partial_a \phi_m/c + \sigma_a \wedge \sigma_b \partial_a A^b - j k \sigma_b A^b
