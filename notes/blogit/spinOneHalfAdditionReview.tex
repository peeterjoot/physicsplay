...
where $S$ takes on the values of the total spin for the (possibly composite) spin operator.  Thinking about the spin operators in their matrix representation, it's not obvious to me that we can just add the total spins, so that if $\BS_1$ and $\BS_2$ are the spin operators for two respective particle, then the total system has a spin operator $\BS = \BS_1 + \BS_2$ (really $\BS = \BS_1 \otimes I_2 + I_2 \otimes \BS_2$, since the respective spin operators only act on their respective particles).
