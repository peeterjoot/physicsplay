...
\beginArtNoToc
\generatetitle{PHY1520H Graduate Quantum Mechanics.  Lecture 13: Time reversal (cont.).  Taught by Prof.\ Arun Paramekanti}
...
\ket{\tilde{\Psi}} = \Theta \ket{\Psi(0)}
\end{dmath}

\begin{dmath}\label{eqn:qmLecture13:40}
...
lhs
\begin{dmath}\label{eqn:qmLecture13:100}
\Theta^{-1} \lr{ 1 - i \frac{\hat{H}}{\Hbar} \delta t } \ket{\Psi(0)},
\end{dmath}
...
but since \( \ket{\Psi(0)} \) is any state, this is
...
and \( \Theta \ket{\Psi_n} \) is an eigenstate with energy \( E_n \).
...
How about the angular momentum operators
...
\paragraph{Spin 1/2 (Fermions).  Attempt I.}

Consider two spin states \( \ket{\uparrow}, \ket{\downarrow} \).  What should the action of the time reversal operator on such a state be.  Let's (incorrectly) start by supposing that the time reversal operator effects on this is
...
\item The action on a spin operator with properties
...
Justify by recalling that

\begin{equation}\label{eqn:qmLecture13:520}
\begin{aligned}
\hat{S}_z \ket{\uparrow} &= \frac{\Hbar}{2} \ket{\uparrow} \\
\hat{S}_x \ket{\uparrow} &= \frac{\Hbar}{2} \ket{\downarrow} \\
\hat{S}_y \ket{\uparrow} &= \frac{i\Hbar}{2} \ket{\downarrow}
\end{aligned}
\end{equation}

\item Spin expectations

The expectations of the spin operators (without any application of time reversal) are

\begin{equation}\label{eqn:qmLecture13:540}
\begin{aligned}
\bra{\Psi} \hat{S}_x \ket{\Psi} &= \frac{\Hbar}{2} \lr{ a^\conj b + b^\conj a } \\
\bra{\Psi} \hat{S}_y \ket{\Psi} &= \frac{\Hbar}{2 i} \lr{ a^\conj b - b^\conj a } \\
\bra{\Psi} \hat{S}_z \ket{\Psi} &= \frac{\Hbar}{2} \lr{ \Abs{a}^2 - \Abs{b}^2 },
\end{aligned}
\end{equation}

so the time reversal action would be

\begin{equation}\label{eqn:qmLecture13:560}
\begin{aligned}
\bra{\Psi} \Theta^{-1} \hat{S}_x \Theta \ket{\Psi} &= \frac{\Hbar}{2} \lr{ b^\conj a + a b^\conj } \\
\bra{\Psi} \Theta^{-1} \hat{S}_y \Theta \ket{\Psi} &= \frac{\Hbar}{2 i} \lr{ b a^\conj - a b^\conj } \\
\bra{\Psi} \Theta^{-1} \hat{S}_z \Theta \ket{\Psi} &= \frac{\Hbar}{2} \lr{ \Abs{b}^2 - \Abs{a}^2 },
\end{aligned}
\end{equation}

We see that this is not right, because the sign for the x component has not been flipped (only the sign of the z component has).
...
{\hat{L}_x}
=
\hat{L}_y \antisymmetric{\hat{L}_y}{hatL_x}
+\antisymmetric{\hat{L}_y}{hatL_x} \hat{L}_y
+\hat{L}_z \antisymmetric{\hat{L}_z}{hatL_x}
+\antisymmetric{\hat{L}_z}{hatL_x} \hat{L}_z
= 
0.
\end{dmath}
...
Let's define
\begin{dmath}\label{eqn:qmLecture13:n}
\begin{aligned}
...
Because these are sums of \( \hatL_x, \hatL_y \) they must also commute
\begin{dmath}\label{eqn:qmLecture13:n}
\antisymmetric{\hat{\BL}^2}{\hatL_{\pm}} = 0.
\end{dmath}

We find
\begin{dmath}\label{eqn:qmLecture13:n}
\antisymmetric{\hat{L}_z}{\hatL_{\pm}} = \pm \Hbar \hatL_{\pm},
\end{dmath}

or
\begin{dmath}\label{eqn:qmLecture13:n}
\hat{L}_z \hatL_{+} - \hatL_{+} \hat{L}_z &= \Hbar \hatL_{+}
\hat{L}_z \hatL_{-} - \hatL_{-} \hat{L}_z &= -\Hbar \hatL_{-}
\end{dmath}
...
\begin{dmath}\label{eqn:qmLecture13:n}
\hatL_z \hatL_{+} \ket{a, b} 
= 
\Hbar \L_p \ket{a,b} + \L_p \hatL_z \ket{a,b}
= 
\Hbar \L_p \ket{a,b} + \Hbar b \L_p \ket{a,b}
=
\Hbar \lr{ b + 1 } \hatL_{+} \ket{a, b} 
\end{dmath}
...
\lr{ \hatL_x + i \hatL_y }
=
\lr{ \hat{\BL}^2 - \hatL_z^2 } + i \antisymmetric{\hatL_x}{\hatL_y}
=
\hat{\BL}^2 - \hatL_z^2 - \Hbar \hatL_z,
\end{dmath}
...
\begin{equation}\label{eqn:qmLecture13:n}
0 
\le \bra{a, b} \L_m \L_p \ket{a, b} 
= 
\bra{a, b} 
\lr{ \hat{\BL}^2 - \hatL_z^2 - \Hbar \hatL_z }
\ket{a, b} 
=
