...
This sum over all unique pairs is somewhat unweildly, so it can be adjusted by explicitly double counting with a corresponding divide by two
...
This used the Laplacian representation of the delta function \( \delta^3(\Bx) = -(1/4\pi) \spacegrad^2 (1/\Abs{\Bx}) \).  Backsubstitution gives
