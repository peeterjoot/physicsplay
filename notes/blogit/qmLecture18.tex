...
\item Take \( \lambda \) small, and do a series expansion.  This is \textAndIndex{perturbation theory}.
\end{enumerate}
...
where \( P_m = \Abs{a_m}^2 \), which has the structure of a probability coefficient once divided by \( \sum_m P_m \).  This average energy is a probability weighted average of the individual energy basis states.   One of those energies is the ground state energy \( E_1 \), so we necessarily have
...
FIXME: Nishant's notes showed these probabilities ordered so that \( P_1 \ge P_2 \ge P_3 \cdots \), as in \cref{fig:lecture18:lecture18Fig1}.
...
\paragraph{Example: particle in a \( [0,L] \) box.}
...
\paragraph{Example II: particle in a \( [-L/2,L/2] \) box.}
