...
The diffusion results in an ``attempt'' to flatten the distribution of the concentration as in 
...
This is the \underline{Maxwell-Boltzman} distribution, illustrated in 

F3

The concentration $c(v)$ has a probability distribution.

If we use this distribution, and calculate $\expectation{v^2}$ we can identify the $D/\eta$ factor.  This also happens to be the energy in terms of temperature (we can view this as a definition of the temperature for now).

\begin{equation}\label{eqn:basicStatMechLecture4:280}
\inv{2} m \expectation{\Bv^2} = \inv{2} \left( \frac{D}{\eta} \right) = \inv{2} _{k_{\mathrm{B}}} T
\end{equation}

FIXME: do this $\expectation{v^2}$ calculation.

Here

\begin{subequations}
\begin{equation}\label{eqn:basicStatMechLecture4:300}
_{k_{\mathrm{B}}} = \mbox{Boltzmann constant}
\end{equation}
...
\boxed{
\frac{D}{\eta} = \frac{_{k_{\mathrm{B}}} T}{m}
}
\end{equation}
...
Fick's law, and the continuity equation respectively take the forms
...
\PD{t}{} c(\Bv, t) + \spacegrad \cdot \Bj(\Bv, t) = 0
\end{equation}
\end{subequations}

Our steady state equation will end up with the solution
...
\inv{2} m \expectation{\Bv \cdot \Bv} = \frac{3}{2} _{k_{\mathrm{B}}} T
\end{equation}

FIXME: do this calcuation.
...
This has phase space trajectories as in
