...
\generatetitle{Some problems from Kittle Thermal Physics, chapter II}
%\chapter{Some problems from Kittle Thermal Physics, chapter II}
\label{chap:kittleCh2}
\section{Motivation}
...
from which we find the entropy \ref{eqn:kittelCh2:220} directly after taking logarithms.  The temperature is found directly
...
\begin{enumerate}
\item Where does $2 \expectation{s}/N$ come from?  If we calculate the expectatation of the spin excess, we find that it is zero
...
\item If $2 \expectation{s}$ has a non-zero value, then doesn't that make $\expectation{U}$ also zero?  It seems to me that $U$ in \ref{eqn:kittelCh2:340} is the energy of a system with spin excess $s$, and not any sort of average energy?
\end{enumerate}
...
A final rearrangement gives us the Planck result \ref{eqn:kittelCh2:80}.
} % makeanswer
