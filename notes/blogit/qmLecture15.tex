...
This has a block diagonal form that's sketched in
...
\paragraph{Agnular momentum operator in space representation}
...
We see that scaling \( \Br \rightarrow \alpha \Br \) doe not change this operator, allowing for an angular representation \( \hat{L}_k(\theta, \phi) \) that have the form
...
The equivalent matrix problem is
...
Since \( \hat{\BL} \) is a vector we expect to be able to add it as we add classical vectors

F3

When we have a potential that depends only on the difference in position \( V(\Br_1 - \Br_2) \)

F4

then we know from classical problems it is effective to work in center of mass coordinates
