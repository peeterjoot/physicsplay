...
\generatetitle{On Tai and Pereira's half beamwidth approximations}
%\chapter{On Tai and Pereira's half beamwidth approximations}
%\label{chap:taiAndPereira}

\section{Short horizontal electrical dipole}
...
\section{Arithmetic mean formula}
...
\section{E and H plane directivities}
\subsection{Problem}
...
\inv{D_1} =  \frac{\Abs{E_\theta}^2_\tmax}{2 \int_0^\pi \Abs{E_\theta(\theta, 0)}^2 \sin\theta d\theta}
\end{dmath}
\begin{dmath}\label{eqn:taiAndPereira:40}
\inv{D_2} =  \frac{\Abs{E_\phi}^2_\tmax}{2 \int_0^\pi \Abs{E_\theta(\theta, \pi/2)}^2 \sin\theta d\theta},
\end{dmath}
\end{subequations}
...
where these are derived by considering ``the asypmtotic expression for the directivity of an antenna with a rotationally symmetrical power pattern of the form \( U(\theta) = \cos^m \theta, \, \theta \in [0, \pi/2] \), with a large value of \( m \)''.
