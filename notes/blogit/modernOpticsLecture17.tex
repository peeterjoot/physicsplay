...
\imageFigure{modernOpticsLecture17Fig1}{Cavity}{fig:modernOpticsLecture17:modernOpticsLecture17Fig1}{0.3}
...
\Gamma_{\mathrm{sp}} = 10^{7} \mathrm{s}^{-1}
\end{dmath}

The spontaneous em rate out of cavity, and

\begin{dmath}\label{eqn:modernOpticsLecture17:100}
\Gamma_{\mathrm{st}} = 1 \mathrm{s}^{-1}
\end{dmath}

(the spontaneious emmision into cavity mode of interest).
...
\ddt{\expectation{n}} = \Gamma_{\mathrm{st}} N_2 ( 1 + \expectation{n} ) - \Gamma_{\mathrm{cav}} \expectation{n}
\end{dmath}
...
\imageFigure{modernOpticsLecture17Fig2}{Probability distribution}{fig:modernOpticsLecture17:modernOpticsLecture17Fig2}{0.3}
...
We aren't equipt to do this (covered in PHY2204), however, for the steady state solution, setting $dN_2/dt = 0$ we have for the atomic population
...
Plugging into the steady state ($d\expectation{n}/dt = 0$) photon population equation of \ref{eqn:modernOpticsLecture17:160} we have
...
This can be writen in a nicer way
...
Here $n_s$ is called the \underline{saturation photon number}, and $C$ is called the \underline{cooperation parameter}, a non-dimensional pump rate.  $R$ was the controllable quanity, the voltage of some such knob that we can vary.  The rest are fixed.

Our quadratic equation (taking the positive root to avoid unphysical negative photon numbers)
