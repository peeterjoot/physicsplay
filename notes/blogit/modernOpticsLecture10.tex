...
where $\delta \equiv \bar{\omega} \tau$.  Plotting the absolute value $\Abs{ \gamma_{12}}$ we have something like

FIXME: F2

Here $\alpha_{12}$ is the difference in the phase from the average.  As an illustration we may be considering a phase shift at one of the points:

FIXME: F3

Resulting in a non-zero $\alpha_{12}$ as in
...
The first case $\Abs{\gamma_{12}} = 1$ is the easiest case to deal with
...
We can get this in the lab, by introducing a filter like 
...
Consider two sources with no mutual coherence as in
...
It could be that we've scrambled up any possible fringes.  We'll eventually be dconsidering a spatially extended source (i.e. a filiment in a light bulb: FIXME: F9), and will deal with that by summing over a source distribution, and first need to know how to deal with a pair of sources.
...
\inv{\Delta k} = l_{text{tc}},
\end{dmath}

or
\begin{dmath}\label{eqn:modernOpticsLecture10:230}
\frac{\lambda}{\Delta \theta} = l_{text{tc}}.
\end{dmath}
