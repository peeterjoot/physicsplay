...
Taking dot products we see that the unknown coefficients can be specified in terms of $\Bu_\nought$ and $\Bu_\nought'$
...
Observe that the dot products of the form $(\Ba \cdot \Be) \Be$ have been put into projector operator form to group terms more nicely.  The solution can be thought of as a weighted projector operator working as a time evolution operator from the initial state.

\makeexample{Our example interaction revisited}{example:threeSpringLoop:2}{

Using the interaction example of \eqnref{eqn:threeSpringLoop:500} again, we can sum $\Be_{-3,1} \Be_{-3, 1}^\T + \Be_{-3,2} \Be_{-3, 2}^\T$ to find
...
The leading matrix is an orthonormal projector of the initial conditions onto the eigen subspace for $\lambda_n = -3$.
}
