...
Ungraded solutions to problem set 2 (I audited half the lectures for this course and intend to do the course problem sets if time allows after the fact.)

\makeproblem{Symmetries and conservation laws in external E.M. fields}{\basename:problem:1}{
...
\makesubproblem{Consider first the case of time-independent $\BA$ and $\phi$. Find the expression for the conserved energy, $\mathcal{E}$, of the particle.}{\basename:problem:1:1}
\makesubproblem{For external $\BA$ and $\phi$ dependent on time, find $d \mathcal{E}/dt$.}{\basename:problem:1:2}
\makesubproblem{Let now $\BA$ and $\phi$ be spatially homogeneous, i.e. $\Bx$-independent. Find the conserved momentum. Is it equal to the usual $m \Bv$?}{\basename:problem:1:3}
\makesubproblem{Consider motion in the field of an electrostatic source (creating an external static $\phi(\Bx)$). Find the angular momentum of the particle. Is it conserved for all $\phi(\Bx)$?}{\basename:problem:1:4}
}

\makeanswer{\basename:problem:1}{
\makesubanswer{Conserved energy}{\basename:problem:1:1}
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:410}
\boxed{
\mathcal{E} = \PD{\qdot_i}{\LL} \qdot_i -\LL 
}
\end{dmath}

so that the when the Lagrangian is independent of time $\mathcal{E}$ is conserved, and in general

\begin{dmath}\label{eqn:classicalMechanicsPs2:430}
\boxed{
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:491}
\boxed{
\mathcal{E} = 
\inv{2} m \Bv^2 + q \phi.
}
...
\makesubanswer{Find $d\mathcal{E}/dt$}{\basename:problem:1:2}

\paragraph{With direct computation}
There are two ways we can try this.  One is with direct computation of the derivative from \ref{eqn:classicalMechanicsPs2:490}
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:510}
\boxed{
\ddt{\mathcal{E}}
=
...
Doing it explictly as above is the hard way.  We can do it from the conservation identity \ref{eqn:classicalMechanicsPs2:430} instead
...
\ddt{\mathcal{E}} \stackrel{?}{=} q \Bv \cdot \BE,
\end{dmath}

but that's not what we got.  With $\mathcal{E} = m \Bv^2/2 + q \phi$, it appears that we get a similar result considering just the Kinetic portion of the energy

\begin{dmath}\label{eqn:classicalMechanicsPs2:550}
\inv{2} m \Bv^2 = \mathcal{E} - q \phi.
\end{dmath}
...
\ddt{} \left(
\inv{2} m \Bv^2 \right) = \ddt{} \left( \mathcal{E} - q \phi \right) = q \Bv \cdot \BE
\end{dmath}
...
\ddt{\mathcal{E}_{\text{kin}}} = q \Bv \cdot \BE,
\end{dmath}

where $\mathcal{E}_{\text{kin}} = \gamma m c^2$.  To first order, removing an additive constant, we have $\gamma m c^2 \approx m \Bv^2/2$, so everything checks out.

\makesubanswer{Conserved momentum}{\basename:problem:1:3}
...
we have $\PDi{\epsilon}{\Bx'} = \Ba$, so \ref{eqn:classicalMechanicsPs2:610} takes the form
...
\makesubanswer{Conserved angular momentum}{\basename:problem:1:4}
...
Evaluating the conservation identity \ref{eqn:classicalMechanicsPs2:770} at $\epsilon = 0$ we have
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:n}
...
Note that the dependency on Geometric Algebra in the Noether's argumend above can probably be eliminated by utilizing a rotational transformation of the form
...
\makeproblem{Integrals of motion in a helix potential}{\basename:problem:2}{
...
\makeanswer{\basename:problem:2}{ 
TODO.
}
\shipoutAnswer

\makeproblem{``Hidden'' symmetries and integrals of motion}{\basename:problem:3}{
...
Here $g$ is the magnetic charge, which can be found using the magnetic analogue of Gausses law: $4 \pi Q_{\text{magn}} = \int_{S^2} d\BSigma \cdot \BB = 4 \pi g$.  In this problem, do not start with a Lagrangian
...
\makesubproblem{Find the value of $c$ ensuring that $d \BD/dt = 0$ for solutions of the equations of motion.}{\basename:problem:3:1}
\makesubproblem{Show that the radius vector of the trajectory for a particle moving in the field of a monopole obeys $\dot{\rcap} \cdot \BD = c$, where $\dot{\rcap} = \Br/r$.}{\basename:problem:3:2}
...
surface when $g \rightarrow 0$? Is it surprising?}{\basename:problem:3:3}
\makesubproblem{Is energy conserved? Find the energy of the particle $\mathcal{E}$ as a function of $\Bv$, $\Br$, ... as appropriate to the case in hand.}{\basename:problem:3:4}
...
\makeanswer{\basename:problem:3}{ 
\makesubanswer{Value of $c$}{\basename:problem:3:1}
TODO.
\makesubanswer{Trajectory radius vector}{\basename:problem:3:2}
TODO.
\makesubanswer{Trajectory surface}{\basename:problem:3:3}
TODO.
\makesubanswer{Is energy conserved}{\basename:problem:3:4}
TODO.
}
\shipoutAnswer

\makeproblem{Symmetries and conservation laws}{\basename:problem:4}{
...
\makesubproblem{An infinite homogeneous plane}{\basename:problem:4:1}
\makesubproblem{An infinite homogeneous cylinder}{\basename:problem:4:2}
\makesubproblem{Two point particles}{\basename:problem:4:3}
\makesubproblem{An infinite homogeneous prism}{\basename:problem:4:4}
\makesubproblem{A homogeneous cone}{\basename:problem:4:5}
\makesubproblem{Three point particles}{\basename:problem:4:6}
\makesubproblem{A homogeneous torus}{\basename:problem:4:7}
...
\makeanswer{\basename:problem:4}{ 
\makesubanswer{An infinite homogeneous plane}{\basename:problem:4:1}
TODO.
\makesubanswer{An infinite homogeneous cylinder}{\basename:problem:4:2}
TODO.
\makesubanswer{Two point particles}{\basename:problem:4:3}
TODO.
\makesubanswer{An infinite homogeneous prism}{\basename:problem:4:4}
TODO.
\makesubanswer{A homogeneous cone}{\basename:problem:4:5}
TODO.
\makesubanswer{Three point particles}{\basename:problem:4:6}
TODO.
\makesubanswer{A homogeneous torus}{\basename:problem:4:7}
TODO.
}
\shipoutAnswer

\makeproblem{Find components of angular momentum in spherical and cylindrical coordinates}{\basename:problem:5}{

\makesubproblem{Find $M_x, M_y, M_z, \BM^2$ in spherical coordinates $(r, \theta, \phi)$.}{\basename:problem:5:1}
\makesubproblem{Find $M_x, M_y, M_z, \BM^2$ in cylindrical coordinates $(r, \phi, z)$.}{\basename:problem:5:2}
...
\makeanswer{\basename:problem:5}{

\makesubanswer{Spherical coordinates}{\basename:problem:5:1}
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:50}
\boxed{
\BM =
\inv{2} m r^2
\begin{bmatrix}
...
\begin{dmath}\label{eqn:classicalMechanicsPs2:70}
\boxed{
\BM^2
=
...
\makesubanswer{Spherical coordinates - a smarter way}{\basename:problem:5:1}
...
\makesubanswer{Cylindrical coordinates}{\basename:problem:5:2}
