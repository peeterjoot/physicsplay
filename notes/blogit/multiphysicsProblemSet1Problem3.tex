...
To discretize the equation, let's divide the interval into \( N \) segments, as illustrated in \cref{fig:ps1p3:ps1p3Fig8}.
...
The linearized Poisson equation at interior nodes \( 1 \le i \le N \) is
...
Identification of the \( - \kappa_a \Delta x/\kappa_m \lr{ T^i - T_0 } \) term as a current \( I = \Delta V/R \) (with temperatures as voltages) means that we can identify \( R \sim \kappa_m \Delta x/\kappa_a \).  This allows us to create the equivalent circuit sketched in \cref{fig:ps1p3:ps1p3Fig9}.
...
\begin{aligned}
-q_0 + Q^0 - \frac{\kappa_a \Delta x}{\kappa_m} \lr{ T^0 - T_0 } + \frac{H^0 \Delta x}{\kappa_m} &= 0 \\
-q_1 -Q^{N} - \frac{\kappa_a \Delta x}{\kappa_m} \lr{ T^{N+1} - T_0 } + \frac{H^{N+1} \Delta x}{\kappa_m} &= 0. \\
\end{aligned}
\end{equation}

At these nodes is only one current term, but we also have to model the heat sinks.  This equivalent circuit is sketched in \cref{fig:ps1p3:ps1p3Fig10}.

\imageFigure{../../figures/ece1254/ps1p3Fig10}{Endpoints in equivalent circuit}{fig:ps1p3:ps1p3Fig10}{0.3}

Note: missed the \( \Delta x \) factors in the equivalent circuit figures.
...
\begin{dmath}\label{eqn:multiphysicsProblemSet1Problem3:n}
