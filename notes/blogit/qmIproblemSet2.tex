...
Observe that the integral on the RHS of \ref{eqn:qmIproblemSet2:5000} is a summation over all $x'$, so we can think of this as adding the probabilities that the particle was at each point to arrive at the total probablility for finding it at the new location $x$.  The time evolution operator matrix element provides the weighting in this conditional probability.
