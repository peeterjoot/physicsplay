...
Last time we found the \textAndIndex{density of states}

\begin{dmath}\label{eqn:condensedMatterLecture13:20}
D(E) = \inv{2 \pi^2} \lr{\frac{2m}{\Hbar^2}}{3/2} \sqrt{E},
\end{dmath}

and \textAndIndex{Fermi-Dirac distribution} as illustrated in \cref{fig:qmSolidsL13:qmSolidsL13Fig2}.

\imageFigure{qmSolidsL13Fig2}{2: CAPTION}{fig:qmSolidsL13:qmSolidsL13Fig2}{0.3}

\begin{dmath}\label{eqn:condensedMatterLecture13:40}
f(E, T) = \inv{ e^{(E - \mu)/\kB T} + 1}.
\end{dmath}

We found an estimate for the specific heat

\begin{dmath}\label{eqn:condensedMatterLecture13:60}
C(T) \sim 2 \kB D(\EF) T,
\end{dmath}

where the ``exact'' calculation comes from
...
In terms of the density of states, this is
\begin{dmath}\label{eqn:condensedMatterLecture13:120}
C(T) = \int dE D(E) E \PD{T}{f(E, T)}.
\end{dmath}

In \cref{fig:qmSolidsL13:qmSolidsL13Fig2} the distribution derivative $\PDi{T}{f}$ is also plotted.  An important feature is that this derivative is zero except in the region $E \sim \EF \pm \kB T$.
...
%}
%{
%chemical potential shifted at $\Br$ by $- e \phi(\Br)$
%}
)/\kB T} + 1}
\approx
-e
