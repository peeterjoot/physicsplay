...
A couple weeks ago I'd attempted a problem to show there must be degeneracy in a system constrained by

\begin{equation}\label{eqn:angularMomentumAndCentralForceCommutators:n}
[A,H] = [B,H] = 0,
\end{equation}

but

\begin{dmath}\label{eqn:angularMomentumAndCentralForceCommutators:n}
[A,B] != 0.
\end{dmath}

I went back to that last night, and figured it out this time.  It wasn't actually complicated, but required using the given information in a precise way.  In particular, not using notation that accidentally implies simultaneously diagonalization of both \( A \) and \( B \).

Suppose that we are working in a basis that simulaneously diagonalizes operator \( A \) and the Hamiltonian \( H \).  To make life easy consider the simplest case where this basis is also an eigenbasis for the second operator \( B \) for all but two of that operators eigenvectors.  For such a system let's write
