...
This adiabatic evolution displays a ``level repulsion'', quadradic in \( \Abs{c} \), and is described as a non-degenerate perbutation.
...
Here we loose the adiabaticity, and have ``level repulsion'' that is linear in \( \Abs{c} \).  We no longer have the sign of \( a - b \) in the expansion.  This is described as a degenerate perbutation.
...
We want a specific interpretation of \( \lr{ E_n^{(0)} - H_0 }^{-1} \), and this is made concrete when acting on a unpertubed state
