...
The thermal occupancy is given by the \indexAndText{Boltzman distribution}
...
\begin{dmath}\label{eqn:condensedMatterLecture10:140}
\myBoxed{
\mathcal{E}(\omega_q T) 
=
\Hbar \omega \lr{ \inv{2 } +
...
the \indexAndText{Bose distribution}.  In the $\kB T \gg \Hbar \omega$ we have
...
\mathcal{E}(\omega_q T) 
\approx
\inv{\Hbar \omega}{2} + \Hbar \omega e^{-\Hbar \omega/\kB T}.
\end{dmath}

These limits are plotted in \cref{fig:qmSolidsL10:qmSolidsL10Fig1}, and \cref{fig:qmSolidsL10:qmSolidsL10Fig2}.

\imageFigure{qmSolidsL10Fig1}{Average energy vs temperature}{fig:qmSolidsL10:qmSolidsL10Fig1}{0.3}
\imageFigure{qmSolidsL10Fig2}{Average energy vs frequency}{fig:qmSolidsL10:qmSolidsL10Fig2}{0.3}

\section{Lattice specific heat capactity}

Define $C(T)$ as the change in the energy $U(T)$ per unit change in $T$.
...
U(T) = \sum_q \mathcal{E}(\omega_q, T) = \inv{V} \int_0^\infty Z(\omega) \mathcal{E}(\omega_q, T) d\omega.
\end{dmath}

In the Debye model we found %in \eqnref{eqn:condensedMatterLecture9:360} that
%\eqnref{eqn:condensedMatterLecture9:440}
...
so the specific heat for the Deybe model is

\begin{dmath}\label{eqn:condensedMatterLecture10:240}
\CV(T) 
= \inv{2 \pi^2} \lr{ 
\inv{C_L^3} + \frac{2}{C_T^3} 
}
 \int_0^{\omega_D} \omega^2 \frac{d}{dT} \mathcal{E}(\omega, T) d\omega
= 
\frac{ 9 r N}{V} 
\inv{\omega_D^3}
\frac{d}{dT}
...
where $r$ is the number of atoms in the unit cell.  In the limit $\kB T \gg \Hbar \omega_D$, we have
...
\mathLabelBox{\kB \Theta}{Debye temp} = \Hbar \omega_D
\end{dmath}
\end{subequations}
...
y(\omega_D) = \frac{\Hbar \omega_D}{ \kB T} = \frac{\kB \Theta}{\kB T} = \frac{\Theta}{T},
\end{dmath}
...
Modes are \underline{frozen out}.

In the $T \ll \Hbar \omega_D/\kB$ limit, we have
...
The $T^3$ dependence here is \underline{very important}.  This comes from 
...
With $\omega \propto q$, the volume of of thermally occuplied $q$ space $\propto T^3$ and the energy of a thermally occupied mode $= T$, we have
...
%\imageFigure{qmSolidsL10Fig3}{Occupied volume in $q$ space}{fig:qmSolidsL10:qmSolidsL10Fig3}{0.3}
