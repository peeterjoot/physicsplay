...
In a source free, lossless ( \( \sigma = 0 \) ) simple medium the electric field satisfies
...
\( \beta z = \textrm{constant} \) describes planes perpendicular to the \( z \) axis.
...
Consider a volume V, surrounded by a closed surface S

F5: B.9

The instananeous power density outflowing the surfaces S s given by the Poynting vector 
...
Consider again a volume V enclosed in a surface S.  Assume that electric currents may exist in V and that V is filled with a material characterized by \( \epsilon, \mu \) and a conductivity \( \sigma \), where \( \epsilon = \epsilon' -j \epsilon'' \), and \( \mu = \mu' - j \mu'' \) as illustrate in F6
