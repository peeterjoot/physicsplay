...
If the response to a time harmonic signal \( e^{j \omega t} \) is known then the response to an arbitrary waveform \( f(t) \) can be synthesized, as shown in F2 where the waveform input is \( f(t) = \inv{2 \pi} \int_{-\infty}^\infty \tilde{f}(\omega) e^{j \omega t} dt \) and the waveform output is \( \inv{2 \pi} \int_{-\infty}^\infty \tilde{f}(\omega) G(\omega) e^{j \omega t} dt \)
