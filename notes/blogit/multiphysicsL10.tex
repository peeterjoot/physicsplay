...
The core idea of this method is sketched in \cref{fig:lecture10:lecture10Fig4}.  To find the intersection with the x-axis, we follow the slope closer to the intersection.
...
and subtracting this from \cref{eqn:multiphysicsL10:280} we are left with
...
We see that convergence is quadratic in the error from the previous iteration.  We will have trouble if the derivative goes small at any point in the iteration region, for example in \cref{fig:lecture10:lecture10Fig7}, we could easily end up in the zero derivative region.
...
Alternatively, if the slope is shallow as in \cref{fig:lecture10:lecture10Fig7}, then checking for just \( \Abs{ f(x^{k+1} } < \epsilon_f \) may also mean we are off target.

Finally, we may also need a relative error check to avoid false convergence.  In \cref{fig:lecture10:lecture10Fig10}, we may have both
...
This can become problematic in real world engineering examples such as to diode of \cref{fig:lecture10:lecture10Fig11}, where we have shallow regions and fast growing or dropping regions.
