...
For the \( x^2 \) and \( y^2 \) expectations we have

\begin{dmath}\label{eqn:gradQuantumProblemSet2Problem2:280}
\frac{\Hbar \omega}{2} 1 \times 2 \times
\bra{0} \lr{ a e^{-i \omega t} + a^\dagger e^{i \omega t} }^2 \ket{0}
=
\Hbar \omega
\lr{ e^{i \omega t} \ket{1} }^\dagger
e^{i \omega t} \ket{1}
=
\Hbar \omega,
\end{dmath}

for the \( z^2 \) expectation we have

\begin{dmath}\label{eqn:gradQuantumProblemSet2Problem2:300}
\lr{ a e^{-i \omega t} + a^\dagger e^{i \omega t} } \lr{\ket{0} + \ket{2}}
=
e^{i \omega t} \lr{ \ket{1} + \sqrt{3} \ket{3} }
+ e^{-i \omega t} \sqrt{2} \ket{1}
=
\lr{ e^{i \omega t} + e^{-i \omega t} \sqrt{2} } \ket{1}
+ \sqrt{3} \ket{3},
\end{dmath}

so

\begin{dmath}\label{eqn:gradQuantumProblemSet2Problem2:320}
\begin{aligned}
\frac{\Hbar \omega}{2}
\braket{0_x}{0_x}\braket{0_y}{0_y}
\lr{\bra{0_z} + \bra{2_z}}\lr{ a_z e^{-i \omega t} + a_z^\dagger e^{i \omega t} }^2 
...
\frac{\Hbar \omega}{2}
\lr{
1 + 2 + 2 \sqrt{2} \cos( 2 \omega t ) + 3
} \\
&=
\Hbar \omega
\lr{ 3 + \sqrt{2} \cos( 2 \omega t ) }.
\end{aligned}
\end{dmath}

Adding up all the pieces we have
...
As before the squared momentum operator has been factored into conjugate pairs, so only the action on \( \ket{\psi} \) need be computed.  For the \( x \) component of that operation we have
...
Noting that  \( \ifrac{\Hbar^2}{2 x_0^2 m} = \Hbar \omega/2 \), summation of all the component contributions gives

\begin{dmath}\label{eqn:gradQuantumProblemSet2Problem2:440}
\expectation{\frac{\Bp^2}{m}}
=
\frac{\Hbar \omega}{2} \lr{ 
 \braket{1,0,0}{1,0,0} + \braket{1,0,2}{1,0,2} 
+\braket{0,1,0}{0,1,0} + \braket{0,1,2}{1,0,2} 
+(\sqrt{3})^2 \braket{0,0,3}{0,0,3}
+ \Abs{e^{i\omega t} - e^{-i\omega t} \sqrt{2}}^2
}
=
...
This matches \cref{eqn:gradQuantumProblemSet2Problem2:340} as expected, confirming for this state that 

\begin{dmath}\label{eqn:gradQuantumProblemSet2Problem2:520}
0 = \expectation{\frac{\Bp^2}{m}} - \expectation{ \Bx \cdot \spacegrad V },
\end{dmath}

even when the state is not an eigenstate for which all the time dependence will perfectly cancel out after taking expectations.
}
