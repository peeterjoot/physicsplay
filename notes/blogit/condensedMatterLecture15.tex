...
\section{Nearly free electron model, periodic potential (cont.)}
...
the Schr\"{o}dinger equation takes the form
...
where our trial solution had the form
...
$C_{\Bk - \BG}$ is large.  i.e. at (1) in \cref{fig:qmSolidsL14:qmSolidsL14Fig4}{0.4} 
...
and $\frac{\Hbar^2 \lr{\Bk}^2}{2m} = \frac{\Hbar^2 \lr{\Bk -\BG}^2}{2m}$.
...
\begin{aligned}
\lr{ E(\Bk) - \frac{\Hbar^2 k^2}{2m} } C_k
- V_{\frac{2\pi}{a}} C_{k - 2\pi/a} &= 0 \\
\lr{ E(\Bk) - \frac{\Hbar^2 (k - 2\pi/a)^2}{2m} } C_{k - 2\pi/a}
- V_{-\frac{2\pi}{a}} C_{k} &= 0,
\end{aligned}
\end{dmath}
...
E^{\pm}(k) = 
\inv{2} \lr{ E_k^0 + E_{k - 2\pi/a}^0 }
\pm
\sqrt{
\inv{4} \lr{ E_k^0 + E_{k - 2\pi/a}^0 }^2 + \Abs{V_{2 \pi/a}}^2
}
\end{dmath}
...
Two sollutions to $E_k^\pm$ correspond to $C_{k = \pi/a} = \pm C_{(k = \pi/a) - 2\pi/a}$.
...
The $\cos$ is the lower energy branch because electron density is high where $V(x)$ is low.  The $\sin$ is the higher energy branch because electron density is high when $V(x)$ is high.
}
