...
where we will see that the vector $d^{k-1} \Bx I$ can rougly be characterized as a normal to the tangent space.  Using primes to indicate the scope of the action of the gradient, cyclic permutation within the scalar selection operator can be used to factor out the pseudoscalar
...
In the second step were able to eliminate the dot product since $k$ is also the dimension of the space.  It is less obvious what the geometry of this product is, but observe that we have the same sort of product, in this case including a boundary volume weighted product of two pseudoscalars.  Let's consider some specific examples to see how we can justify calling this a normal.  We will see that the general structure is

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:4810}
\int_V dV \spacegrad \cdot \Bf = \int_{\partial V} dA_i \ncap^i \cdot \Bf,
\end{dmath}

but that we have to assign specific meanings to $dA_i$ and $\ncap^i$ that are specific to the orientation of the chosen volume element parameterization and the metric.
