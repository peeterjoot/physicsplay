...
T = \inv{3!} \Bx^i \wedge \Bx^j \wedge \Bx_k T_{ijk} + T_\perp,
\end{equation}
\end{subequations}
...
For the trivector components are also antisymmetric, changing sign with any interchange of indexes.
...
In both of these we have an alternation of sign, where the tensor index skips one of the volume element indexes is sequence.  I am unsure if there is a notation that can express both of these compactly.
...
Given a trivector $T$ and three vectors $\Ba, \Bb$, and $\Bc$, we have

\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:2580}
\begin{aligned}
...
This follows from the asymmetric expansion of the wedge product.  Expanding within a scalar selection operator, and then expanding the products from the right, eliminating any terms that cannot contribute to a scalar term
