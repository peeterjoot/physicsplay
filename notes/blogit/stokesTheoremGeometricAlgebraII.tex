...
Observe that \eqnref{eqn:stokesTheoremGeometricAlgebraII:4010} is a vector valued integal that expands to
...
There is a geometric interpretation to these oriented area integrals, especially when written out explicitly in terms of the differentials along the parameterization directions, as in
...
%Here the sign has been pulled out explicitly so that the upper evaluation range matches the top faces.
...
FIXME: write out the mathematica results and the corresponding factorizations.
}
