...
One detail that was that I failed to understand, was that we have a requirement for treating a infinetesimal region in the proof, then summing over such regions to express the boundary integral.  Understanding that the boundary integral form and its dot product with the blade whos curl we had taken are both evaulated only at the end points of the integral region is an important detail that follows from such an argument (as used in proof of Stokes theorem for a 3D Cartesian space in \citep{schwartz1987pe}.)  

I also realized that my previous attempts could only work for the special cases where the dimension of the integraion volume also equaled the dimension of the vector space.  The key to resolving this issue is the concept of the tangent space, and an understanding of how to express the projection of the gradient onto the tangent space.  These concepts are covered thoroughly in \citep{aMacdonaldVAGC}, which also introduces Stokes theorem as a special case of a more fundamental theorem for integration of geometric algebraic objects.  My objective, for now, is still just to understand the generalization of Stokes theorem, and will leave the fundamental theorem of geometric calculus to later.
...
It takes some work to give this more concrete meaning.  I will attempt to do so in a gradual fashion, and provide a number of examples that illustrate some of the relavant details.
...
where it is implied that both the vector $\Bf$ and the differential elements are evaluated on the boundaries of the integration ranges respectively.  We can see in \cref{fig:cylindricalPolarParameterization:cylindricalPolarParameterizationFig6} (with $u^1 = r, u^2 = \theta$) that this specifies a counterclockwise orientation, so for the clockwise orientation used above, we can loosely express the boundary differential form as
...
\boxedEquation{eqn:stokesTheoremGeometricAlgebraII:3840}{
\int d^2 \Bx \cdot \lr{ \boldpartial \wedge \Bf }
=
-
\int
...
\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:3920}
\begin{aligned}
\Bx_\rho 
...
We can, in general, compute the reciprocal basis using the duality relation
...
Naively attempting this without algebraic software is a route that requires a lot of care, and is easy to make mistakes doing.  In this case it is really not necessary since the tangent space basis only requires scaling to orthonormalize, satisfying for $i,j \in \setlr{\rho, \theta, \phi}$
...
When the curl integral is a scalar result we are able to apply duality relationships to obtain the divergence theorem for the cooresponding space.  We will be able to show that a relationship of the following form holds

\boxedEquation{eqn:stokesTheoremGeometricAlgebraII:4810}{
\int_V dV \spacegrad \cdot \Bf = \int_{\partial V} dA_i \ncap^i \cdot \Bf.
}

Here $\Bf$ is a vector, $\ncap^i$ is normal to the boundary surface, and $dA_i$ is the area of this bounding surface element.  We wish to quanify these more precisely, especially because the orientation of the normal vectors are metric dependent.  Working a few specific examples will show the pattern nicely, but it is helpful to first consider some aspects of the general case.  
...
where we will see that the vector $d^{k-1} \Bx I$ can rougly be characterized as a normal to the boundary surface.  Using primes to indicate the scope of the action of the gradient, cyclic permutation within the scalar selection operator can be used to factor out the pseudoscalar
...
The second last step uses \cref{thm:stokesTheoremGeometricAlgebraII:4690}, and the last writes $d^k \Bx = I_k I d^k u = s_k dV$, where $s_k$ depends on the metric (i.e. $I^2 = \pm 1$), and on the orientation of the pseudoscalar volume element.  We assume that the parameterization is non-degenerate over the integration volume (i.e. no $d\Bx_i = 0$), so the sign of this product cannot change.

Let's now return to the normal vector $d^{k-1} \Bx I$.  With $d^{k-1} u_i = du^1 du^2 \cdots du^{i-1} du^{i+1} \cdots du^k$ (the $i$ indexed differential omitted), we have

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:4750}
d^{k-1} \Bx I
=
d^{k-1} u_i \Bx \cdot \Bx^i I
= 
d^{k-1} u_i I_k \Bx^i I
= 
(-1)^{k+1} d^{k-1} u_i I_k I \Bx^i
%= 
%d^{k-1} u_i \lr{\Bx_1 \wedge \Bx_2 \cdots \hat{\Bx}_i \cdots \wedge \Bx_k} \hat{\Bx}^i I
%= dA_i \frac{I_{k-1}}{\Abs{I_{k-1}}} I.
\end{dmath}

In the second step were able to eliminate the dot product since $k$ is also the dimension of the space.  Since $I_k I$ is a scalar, \eqnref{eqn:stokesTheoremGeometricAlgebraII:4750} is a scalar multiple of $\Bx^i$, which is normal to the surface element $d\Bx_1 \wedge d\Bx_2 \wedge \cdots \wedge d\Bx_{i-1} \wedge d\Bx_{i+1} \wedge \cdots d\Bx_k$ (the wedge of all the differentials that omit $d\Bx_i$).  The scaling of this normal is the area of the bounding element.

Without introducing the somewhat nebulous metric and orientation specific term $s_k$ the most general relationship that we can write is

\boxedEquation{eqn:stokesTheoremGeometricAlgebraII:4830}{
\int_V I_k I d^k u \lr{ \spacegrad \cdot \Bf} = 
\int_{\partial V} d^{k-1} u_i I_k I \Bx^i \cdot \Bf.
}

Notice that the $(-1)^{k+1}$ factor that appeared on both sides has been cancelled, however, we are not able to cancel the $I_k I$ product since $I_k = I_k(\Bx)$ is position dependent.

Let's now move on to consider some specific cases.
...
For a 3D system, and three parameter volume, duality applied to the bivector Stokes result provides the divergence theorem.  In this case we have $\boldpartial = \spacegrad$.  For a bivector $B$ let $I B = \Bf$.  Provided the orientation of the volume element picked is positive, we can write $d^3 \Bx = I dV$, so that the differential form for the curl is

\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:4490}
\begin{aligned}
d^3 \Bx \cdot
\lr{ \spacegrad \wedge B }
&=
\gpgradezero{ d^3 \Bx ( \spacegrad \wedge B ) } \\
&=
\inv{2} \gpgradezero{ I dV ( \spacegrad B + B\spacegrad ) } \\
&=
\inv{2} \gpgradezero{ I ( \spacegrad (-I \Bf) + (-I \Bf)\spacegrad ) } dV \\
&=
\spacegrad \cdot \Bf dV \\
\end{aligned}
\end{equation}

FIXME: see paper notes.
Similarly for the boundary integral
, and $d^2 x = I \ncap dA$, so that the curl form is

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:4510}
B \cdot d^2 x
=
\gpgradezero{ 
-I\Bf I \ncap dA
...
\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:4530}
