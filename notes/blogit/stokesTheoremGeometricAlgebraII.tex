...
A finite vector space, not neccessarily Euclidean, with basis $\setlr{\Be_1, \Be_2, \cdots}$ will be assumed to be the generator of the geometric algebra.  A dual or reciprocal basis $\setlr{\Be^1, \Be^2, \cdots}$ for this basis can be calculated, defined by the property
...
Let's now consider a cylindrical parameterization of a 3D space with Euclidean metric $+++$ or Minkowski metric $++-$.

With $\kappa = \Be_2 \Be_3$, such a space is convientently parameterized as illustrated in \cref{fig:cylindricalPolarParameterization:cylindricalPolarParameterizationFig5} as

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3080}
\Bx(\rho, \theta, h) = h \Be_1 + \rho \Be_2 e^{\kappa \theta}.
\end{dmath}
...
Note that the Euclidean case where $\lr{\Be_3}^2 = 1$ rejection of the non-axial components of $\Bx$ expands to

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3100}
\lr{ \Bx \wedge \Be_1} \cdot \Be^1 =
\rho \lr{ \Be_2 \cos\theta + \Be_3 \sin \theta },
\end{dmath}

whereas for the Minkowski case where $\lr{\Be_3}^2 = -1$ we have a hyperbolic expansion

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3120}
\lr{ \Bx \wedge \Be_1} \cdot \Be^1 =
\rho \lr{ \Be_2 \cosh\theta + \Be_3 \sinh \theta }.
\end{dmath}

Within such a space consider the surface along $h = c$, for which the vectors are parameterized by

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3140}
\Bx(\rho, \theta) = c \Be_1 + \rho \Be_2 e^{\kappa \theta}.
\end{dmath}
...
\Bx_\theta
= \PD{\theta}{\Bx}
= \rho \Be_2 \Be_2 \Be_3 e^{\kappa \theta}
= \rho \Be_3 e^{\kappa \theta}.
\end{dmath}
...
We wish to compute the volume element for the tangent plane.  Noting that $\Be_2$ and $\Be_3$ both anticommute with $\kappa$ we have for $\Ba \in \Span \setlr{\Be_2, \Be_3}$
...
=
\gpgradetwo{
\Be_2 e^{\kappa \theta} \rho \Be_3 e^{\kappa \theta}
}
=
\rho \gpgradetwo{
\Be_2 e^{\kappa \theta} e^{-\kappa \theta} \Be_3
}
=
\rho \Be_2 \Be_3.
\end{dmath}
...
d^2 \Bx = \rho d\rho d\theta \Be_2 \Be_3.
\end{dmath}

Using can use the duality relationship of \cref{thm:stokesTheoremGeometricAlgebraII:3200} to compute the reciprocal basis for the tangent plane vectors, however since $\Bx_\rho$ and $\Bx_\theta$ are perpendicular, this isn't too hard to do by inspection

\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:3540}
\Bx^\rho = \Be_2 e^{\kappa \theta},
\end{equation}

and
\begin{equation}\label{eqn:stokesTheoremGeometricAlgebraII:3560}
\Bx^\theta = \frac{\Be^3}{\rho} e^{\kappa \theta}.
\end{equation}

Observe that the latter depends on the metric signature.
%%%.  This calculation is metric dependent, since $\lr{\Be_3}^2 = \pm 1$ for the specified Euclidean and Minkowski spaces respectively
%%%
%%%\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3440}
%%%\lr{\Bx_\theta \wedge \Bx_\rho}^2
%%%=
%%%\rho^2 \Be_2 \Be_3 \Be_2 \Be_3
%%%=
%%%-\rho^2 \Be_2 \Be_2 \Be_3 \Be_3
%%%=
%%%-\rho^2 \lr{\Be_3}^2
%%%=
%%%\mp \rho^2.
%%%\end{dmath}
%%%
%%%For the radial dual vector we have
%%%
%%%\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3460}
%%%\Bx^\rho 
%%%=
%%%\frac{ \Bx_\theta \cdot \lr{ \Bx_\rho \wedge \Bx_\theta } }
%%%{
%%%\lr{ \Bx_\rho \wedge \Bx_\theta }^2
%%%}
%%%=
%%%\inv{\mp \rho^2} \gpgradeone{
%%%\rho \Be_3 e^{\kappa \theta} \rho \Be_2 \Be_3
%%%}
%%%=
%%%\mp \gpgradeone{
%%%\Be_3 \Be_2 \Be_3 e^{\kappa \theta} 
%%%}
%%%=
%%%\pm \gpgradeone{
%%%\Be_2 \Be_3 \Be_3 e^{\kappa \theta} 
%%%}
%%%=
%%%\lr{\pm 1}^2 \Be_2 e^{\kappa \theta}
%%%= 
%%%\Bx_\rho.
%%%\end{dmath}
%%%
%%%In the azimuthal direction the dual is
%%%
%%%\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3480}
%%%\Bx^\theta =
%%%-\frac{ \Bx_\rho \cdot \lr{ \Bx_\rho \wedge \Bx_\theta } }
%%%{
%%%\lr{ \Bx_\rho \wedge \Bx_\theta }^2
%%%}
%%%=
%%%\inv{\pm \rho^2} \gpgradeone{
%%%\Be_2 e^{\kappa \theta} \rho \Be_2 \Be_3
%%%}
%%%=
%%%\pm \inv{\rho}
%%%\gpgradeone{
%%%\Be_2 \Be_2 \Be_3 e^{\kappa \theta} 
%%%}
%%%=
%%%\pm \inv{\rho}
%%%\Be_3 e^{\kappa \theta} 
%%%=
%%%\pm \frac{\Bx_\theta}{\rho^2},
%%%\end{dmath}
%%%
%%%or
...
We can now write out the vector derivative and its curl, needed to express the LHS of Stokes theorem, as well as the loop integral on the RHS.  The vector derivative, the projection of the gradient on the tangent space, is
...
The power of Stokes theorem becomes clear, since the reduction of the volume element differential form is quite a chore.  The curl portion is

%\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:3520}
%d\Bx^2 \cdot \lr{\boldpartial \wedge \Bf}
%=
%\end{dmath}
...
It should not be suprising that this has the structure found in the theory of differential forms.  Using the differentials for each of the parameterization ``directions'', we can write this dot product expansion as
...
To proceed with the integration, we must again consider an infinitesimal volume element, for which the partial can be evaluated as the difference of the endpoints, with all else held constant.  For this three variable parameterization, say, $(u,v,w)$, let's delimit such an infinesimal volume element by the parameterization ranges $[u_0,u_0 + du]$, $[v_0,v_0 + dv]$, $[w_0,w_0 + dw]$.  The integral is
...
FIXME: work an example for both a vector and bivector field.

FIXME: example for non-Euclidean space.
...
FIXME: use this to write out the theorem explicity in tensor form (LHS and RHS both).
