...
One detail that was that I failed to understand, was that we have a requirement for treating a infinitesimal region in the proof, then summing over such regions to express the boundary integral.  Understanding that the boundary integral form and its dot product with the blade whos curl we had taken are both evaluated only at the end points of the integral region is an important detail that follows from such an argument (as used in proof of Stokes theorem for a 3D Cartesian space in \citep{schwartz1987pe}.)  
...
FIXME: this use of $d^1 \Bx$ isn't consistent with later uses of $d^{k-1} \Bx$.
...
FIXME: didn't prove this here, or refer to a theorem, but did prove the 2D duality relation.  Then proved this one separately in normal/dual section.  Move that proof to a lemma to reference in both places (leave the pseudoscalar parts).
...
\boxedEquation{example:stokesTheoremGeometricAlgebraII:4950}{
%\acap^\conj 
\aconjcap
...
FIXME: moved theorem 4950.  re-write this section.

Since the pseudoscalar varient of this theorem follows by inspection, this completes the proof.
...
We've seen in \eqnref{example:stokesTheoremGeometricAlgebraII:4950} and \cref{thm:stokesTheoremGeometricAlgebraII:4950} that the dual of vector $\Ba$ with respect to the unit pseudoscalar $I_{\Bb \cdots \Bc \Bd}$ in a subspace spanned by $\setlr{\Ba, \cdots \Bc, \Bd}$ is
...
%%%The boundary differential form for a two parameter Stokes integral is
%%%
%%%\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:4890}
%%%d^1 \Bx 
...
%%%\end{dmath}
%%%
%%%Multiplying this by the pseudoscalar gives something that we expect to be related to the normals (i.e. length scaled dual basis vectors).
%%%
Let's start back at

\begin{dmath}\label{eqn:stokesTheoremGeometricAlgebraII:n}
...
This provides us a divergence and normal relationship, with $-I^2$ terms on each side that can be cancelled.  Restoring explicit range evaluation, that is
...
Let's consider this graphically for an Euclidean metric as in

FIXME: pic illustrating that this has the form of integration dotted with the outward normal component.
}
...
or
\boxedEquation{eqn:stokesTheoremGeometricAlgebraII:4870}{
\Ba^\conj 
...
The canceled term is eliminated since it is the product of a vector and trivector producing no scalar term.  Subsituting $\Ba, \Bb, \Bc$, and noting that $\Bu \wedge \Bu = 0$, we have
