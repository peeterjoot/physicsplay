...
\begin{theorem}\label{thm:stokesTheoremGeometricAlgebraII:1660}
...
To demonstrate this, we first evaluate the loop integral along the differential element of the surface at the point $\Bx(u_0, v_0)$ evaluated over the range $(du, dv)$, as shown in the infinitesimal loop of \cref{fig:loopIntegralInfinitesimal:loopIntegralInfinitesimalFig1}.
...
It is assumed that the parameterization change $(du, dv)$ is small enough that this loop integral can be considered planar (regardless of the dimension of the vector space).  Making use of the fact that $\Bx^\perp \cdot \Bx^\alpha = 0$ for $\alpha \in \{u,v\}$, the loop integral is
...
We will see that Green's theorem is a special case of the Curl (Stokes) theorem.  This observation will also provide a geometric interpretation of the area integral of \cref{thm:stokesTheoremGeometricAlgebraII:1660}.
