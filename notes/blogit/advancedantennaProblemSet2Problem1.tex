...
\item The effective aperature \(A_{\textrm{eff}}\) is also the maximum effective aperature, so it (and the directivity) has no directional dependence.
\end{enumerate}

The scattering geometry for this problem is sketched in \cref{fig:scatteringOffPlane:scatteringOffPlaneFig1}.

\imageFigure{../../figures/ece1229/scatteringOffPlaneFig1}{Scattering off of plane surface.}{fig:scatteringOffPlane:scatteringOffPlaneFig1}{0.3}
...
where \( A_p \) is the area of the plate.  Treating all the power as radiated as if from a point source, measured at distance \( R \) from the plate, and assuming a perfect radiator (i.e. where \( G = D_0 \)), the 
