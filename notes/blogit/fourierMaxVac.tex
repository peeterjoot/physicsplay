...
where summation is over all wave number triplets $\Bk = (p/\lambda_1,q/\lambda_2,r/\lambda_3)$.  The fourier cooefficients $A_\Bk = {A_\Bk}^\mu \gamma_\mu$ are allowed to be complex valued, as is the resulting four vector $A$, and the associated bivector field $F$.
...
but only this orthogonality relationship and not the Fourier cooefficients themselves
...
The grade selection used here doesn't change the result since we already have a scalar, but will just make it convienent to filter out any higher order products that will cancel anyways.  Integrating over the volume element and taking advantage of the orthogonality relationship \ref{eqn:fourierMaxVac:6}, the exponentials are removed, leaving the energy contained in the volume
...
\gpgradezero{ 
(\gamma^m \wedge \conjugateStar{A_{\Bk}} )
(\gamma^n \wedge A_\Bk )
}
\end{align}
\end{subequations}

The expectation is to obtain a Hamiltonian for the field that has the structure of a harmonic oscillators, so the middle two products are either zero or sum to zero or have real parts that sum to zero.  The first is expected to contain only products of $\Abs{{\dot{A}_\Bk}^m}$, and the last only products of $\Abs{{A_\Bk}^m}$.
