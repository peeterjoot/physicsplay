...
\imageFigure{../../figures/ece1229//verticalDipoleReflectionFig1}{Vertical dipole reflection geometry}{fig:verticalDipoleReflection:verticalDipoleReflectionFig1}{0.3}
...
For no transmission the transmitted speed of the radiation \( v_t \rightarrow 0 \), and the the index of refraction of the ground approaches \( n_t = c/v_t \rightarrow \infty \).  This shows that the reflection coefficient for the vertical dipole configuration is \( + 1 \).  Because of the symmetry of this dipole's orientation the sign of the reflection coefficient has no azimuthal dependency. % (as it appears to for the horizontal dipole, despite the use of \( -1 \) for all azimuthal orientations in \citep{} )
...
where an average distance \( r = \sqrt{ h_t^2 + d^2 } = \sqrt{ 50^2 + 5000^2} \), the distance from the origin to the base station has been factored out in the denominator.

The sum of the phase terms is

\begin{dmath}\label{eqn:advancedantennaProblemSet2Problem5:300}
e^{-j k \sqrt{ r^2 + 2 h_t h_r + h_r^2}}
+
e^{-j k \sqrt{ r^2 - 2 h_t h_r + h_r^2}}
\approx
e^{-j k \sqrt{ r^2 + 2 h_t h_r }}
+
e^{-j k \sqrt{ r^2 - 2 h_t h_r }}
=
e^{-j k r \sqrt{ 1 + 2 h_t h_r/r^2 }}
+
e^{-j k r \sqrt{ 1 - 2 h_t h_r/r^2 }}
\approx
e^{-j k r}
...
=
2 e^{-j k r} \cos\lr{ \frac{k h_t h_r}{r} },
\end{dmath}
...
j \omega A_0
\cos\theta_\txtA \lr{ \sin\theta_\txtA \ycap - \cos\theta_\txtA \zcap }
\inv{r} 2 e^{-j k r} \cos\lr{ \frac{k h_t h_r}{r} }.
\end{dmath}

This differs from the line of sight field by a factor of \( 2 \cos\lr{ \ifrac{k h_t h_r}{r} } \).
...
\begin{dmath}\label{eqn:advancedantennaProblemSet2Problem5:380}
G = 10^{2.55 \si{dB}/10} = 1.8,
\end{dmath}

Noting that \( \textrm{EIRP} = P_t G_t \), the Friis transmission equation, after adjusting for the reflection effects, provides the power at the mobile

\begin{dmath}\label{eqn:advancedantennaProblemSet2Problem5:400}
P_r 
= \lr{ \frac{\lambda}{4 \pi r} }^2 \lr{ P_t G_t } G_r 1.2^2
= \lr{ \frac{0.33}{4 \pi 5000.25} }^2 \lr{ 30 \times 10^{-3} } \times 1.8 \times 1.2^2 \si{W}
= 2.1 \times 10^{-12} \si{W}.
\end{dmath}

The total power received is just \( 2.1 pW \), assuming no polarization losses (we know the polarization at the mobile, but not for the base station.)

The numerical calculations above can be found in
