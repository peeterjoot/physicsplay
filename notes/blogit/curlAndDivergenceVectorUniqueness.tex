...
Dropping the suffix one above, the gradient of the vector can be written as a single multivector expression
...
Since \( \BM \) is a vector the bivector and trivector grades on the RHS must sum to zero.  With \( \Br = \Bx - \By \), the vector grade is
...
We must also have zero for the trivector components, meaning that
...
If this constraint is met, and the magnitude of \( \BM(\Br) < r^2 \) as \( r \rightarrow \infty \), then \( \BM \) is uniquely determined by
