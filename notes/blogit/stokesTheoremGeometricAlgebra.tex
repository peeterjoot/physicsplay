...
\begin{theorem}\label{thm:stokesTheoremGeometricAlgebra:600}
\begin{equation}
\myBoxed{
...
FIXME: There is an implicit assumption here that the wedge products of these one forms, which are functions of all the parameters, have been evaluated on the surfaces.  That is

\begin{equation}\label{eqn:stokesTheoremGeometricAlgebra:641}
\begin{aligned}
dA_F &= \evalbar{dx_\alpha \wedge dx_\beta}{\sigma = \sigma_1} \\
dA_R &= \evalbar{dx_\beta \wedge dx_\sigma}{\alpha = \alpha_1} \\
dA_T &= \evalbar{dx_\sigma \wedge dx_\alpha}{\beta = \beta_1}.
\end{aligned}
\end{equation}

This assumption is used below to turn the differences of $f$ on the faces into an integral without requiring the two forms to be functions of the surface parameters.  The other issue here is that the two forms for the back and front, or left and right, or top and bottom faces could have different orientations.  END FIXME.

These faces of the subspace are the surfaces of constant parametrization, respectively, $\sigma = \sigma_1$, $\alpha = \alpha_1$, and $\beta = \beta_1$.  For a bivector $f$, the flux through the surface is

\begin{equation}\label{eqn:stokesTheoremGeometricAlgebra:660}
\begin{aligned}
\iint d^2 x \cdot f
&=
\int_{\alpha_0}^{\alpha_1}
\int_{\beta_0}^{\beta_1}
\lr{ dx_\alpha \wedge dx_\beta } \cdot 
\lr{
f(\alpha, \beta, \sigma_1) 
- f(\alpha, \beta, \sigma_0) 
} \\
&+
...
\lr{
f(\alpha_1, \beta, \sigma) 
- f(\alpha_0, \beta, \sigma) 
} \\
&+
\int_{\sigma_0}^{\sigma_1}
\int_{\alpha_0}^{\alpha_1}
...
\lr{
f(\alpha, \beta_1, \sigma) 
- f(\alpha, \beta_0, \sigma) 
} \\
&=
\int_{\alpha_0}^{\alpha_1}
\int_{\beta_0}^{\beta_1}
...
FIXME: The differentials $dx_m$ are functions of all the free parameters.  We saw something similar in 
\eqnref{eqn:stokesTheoremGeometricAlgebra:641}, not yet resolved.  Have to think this through.
