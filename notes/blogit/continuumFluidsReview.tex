...
\section{Some worked solutions}

\subsection{Channel flow}

We walked through solutions to the channel and shearing flow problems for one dimensional flow.  For fluid flowing between surfaces at $z = \pm h$, the lower surface moving at velocity $v$ and pressure gradient $dp/dx = -G$ we find that Navier-Stokes for an assumed flow of $\Bu = u(z) \xcap$ takes the form
...
We can also calculate the force of the boundaries on the fluid.  For example, the force per unit volume of the boundary at $z = -h$ on the fluid is found by calculating the tangential component of the traction vector taken with normal $\ncap = \zcap$.  That tangent vector is found to be

\begin{equation}\label{eqn:continuumFluidsReview:1390}
\BT = -p \zcap + 2 \mu \Be_i e_{ij} \delta_{j 3} = - p \zcap + \xcap \mu \PD{z}{u}.
\end{equation}

The tangential component is the $\xcap$ component evaluated at $z = -h$

\begin{equation}\label{eqn:continuumFluidsReview:1410}
\evalbar{\BT \cdot \xcap}{z = -h} = -G (-h) - \frac{v \mu}{2 h},
\end{equation}

so the force per unit area that the boundary applies to the fluid is

\begin{equation}\label{eqn:continuumFluidsReview:1430}
\int_{x = 0}^L \evalbar{\BT \cdot \xcap}{z = -h} dx = L \left( G h - \frac{v \mu}{2 h} \right).
\end{equation}

FIXME: does the sign of the velocity term doesn't make sense.  Is this actually the force of the fluid on the wall?  I'm missing something here.
