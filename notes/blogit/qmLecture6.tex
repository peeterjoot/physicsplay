...
\beginArtNoToc
\generatetitle{PHY1520H Graduate Quantum Mechanics.  Lecture 6: XXX.  Taught by Prof.\ Arun Paramekanti}
%\chapter{XXX}
...
Acting again with \( \lr{ -i \Hbar \spacegrad - q \BA - q \spacegrad \chi } \), we have

on board:
% B e^{i\theta} \psi = e^{i\theta} \tilde{B} \psi
%\begin{dmath}\label{eqn:qmLecture6:380}
%\lr{ -i \Hbar \spacegrad - q \BA - q \spacegrad \chi } e^{i \theta} \psi = e^{i\theta}
%\lr{ -i \Hbar \spacegrad - q \BA } \psi = e^{i\theta}
%\end{dmath}
...
We've constructed a simultaneous transformation of the Hamiltonian and its wave function for which the transformed wave function has the same energy eigenvalues as the original.
...
\begin{aligned}
H' &= \inv{2m} \lr{ \Bp - q \BA - q \spacegrad \chi}^2 \\
\psi'(\Br) &= e^{i \theta(\Br)}, \qquad \text{where}\, \theta(\Br) = q \chi(\Br)/\Hbar
\end{aligned}
}

\paragraph{Aharonov-Bohm effect}

Consider a periodic motion in a fixed ring as sketched in 
