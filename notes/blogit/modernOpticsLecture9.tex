...
We'll consider a Michaelson interferometer setup as illustrated in
...
we suppose that we have some sort of system, perhaps due to atomic interactions, we have random discrete phase jumps at regular intervals, as in 
...
= e^{i \omega T} \lim_{T \rightarrow \infty} \int_0^T e^{i \Delta(t) - i\Delta(t + \tau)} dt
\end{dmath}

With the illustrated example phase transitions above we have
...
\inv{N \tau_0} \sum_{n = i}^{N} \int_{n \tau_0}^{(n+1) \tau_0} e^{i (\Delta(t) - \Delta(t+\tau)} dt
=
\frac{\sum^{N}}{N \tau_0} (\tau_0 - \tau)
+\cancel{\frac{\sum^{N}}{N \tau_0} \tau e^{-i\Delta_i}}
\end{dmath}
...
where the $\Delta$'s are random.  We can illustrate that as in
