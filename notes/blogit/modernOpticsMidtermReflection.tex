...
\tau = \frac{2 k h y}{L \omega} = \frac{2 h y c}{L} = \text{constant}.
\end{dmath}
...
\Psi 
= 
\frac{\Psi_s}{i \lambda} \int_{h - \Delta x/2}^{h + \Delta x/2}
dx' 
\left(
\frac{ e^{i k x' d}}{d} 
+\frac{ e^{i k x' b}}{b} 
\right)
\approx
\frac{\Psi_s}{i \lambda L} \int_{h - \Delta x/2}^{h + \Delta x/2}
dx' 
\left(
e^{i k L \sqrt{ 1 + (x' - x)^2/L^2 } }
+ e^{i k L \sqrt{ 1 + (x' + 2h + x)^2/L^2 } }
\right)
\approx
\frac{\Psi_s e^{i k L} }{i \lambda L} \int_{h - \Delta x/2}^{h + \Delta x/2}
dx' 
\left(
e^{i k (x' - x)^2/(2 L) } 
+ e^{i k (x' + 2h + x)^2/(2 L) } 
...
\frac{k(x' - x)^2}{2 L}
=
\frac{ \pi (x' - x)^2}{L \lambda}
\end{equation}
\begin{equation}\label{eqn:modernOpticsMidtermReflection:720}
\frac{\pi}{2} w^2
=
\frac{k(x' + 2 h + x)^2}{2 L}
=
\frac{ \pi (x' + 2 h + x)^2}{L \lambda}
\end{equation}
\end{subequations}

We find that our interference wave function is

\begin{dmath}\label{eqn:modernOpticsMidtermReflection:740}
\Psi(x)
=
\frac{\Psi_s e^{i k L} }{i \lambda L} \sqrt{\frac{\pi L}{k}}
\left(
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{k}{L \pi}}(h - \Delta x/2 -x)}
{\sqrt{\frac{k}{L \pi}}(h + \Delta x/2 -x)}
+
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{k}{L \pi}}(3h - \Delta x/2 + x)}
{\sqrt{\frac{k}{L \pi}}(3h + \Delta x/2 + x)}
\right),
\end{dmath}
...
%\left(
%\evalrange{\left(C(w) + iS(w)\right)}
%{\sqrt{\frac{2}{L \lambda}}(h - \Delta x/2 -x)}
%{\sqrt{\frac{2}{L \lambda}}(h + \Delta x/2 -x)}
%+
%\evalrange{\left(C(w) + iS(w)\right)}
%{\sqrt{\frac{2}{L \lambda}}(3h - \Delta x/2 + x)}
%{\sqrt{\frac{2}{L \lambda}}(3h + \Delta x/2 + x)}
%\right)
%=
%\frac{\Psi_s e^{i k L} }{i \sqrt{2 \lambda L}} 
...
\left(
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{2}{L \lambda}}(x -h - \Delta x/2)}
{\sqrt{\frac{2}{L \lambda}}(x -h + \Delta x/2)}
+
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{2}{L \lambda}}(x + 3h - \Delta x/2)}
{\sqrt{\frac{2}{L \lambda}}(x + 3h + \Delta x/2)}
\right)
\end{dmath}

As in the point source treatment above, we can get a slightly more pleasant looking result by switching our origin to the mirror plane using $y = x + h$.  This gives us

\begin{dmath}\label{eqn:modernOpticsMidtermReflection:765}
\Psi(y)
=
\frac{\Psi_s e^{i k L} }{i \sqrt{2 \lambda L}} 
\left(
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{2}{L \lambda}}(y - 2h - \Delta x/2)}
{\sqrt{\frac{2}{L \lambda}}(y - 2h + \Delta x/2)}
+
\evalrange{\left(C(w) + iS(w)\right)}
{\sqrt{\frac{2}{L \lambda}}(y + 2h - \Delta x/2)}
...
We should verify that this reproduces the point source result in the limit.
