...
Consider the experiments illustrated in figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig1}), and figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig2}).

\imageFigure{modernOpticsLecture6Fig1}{Intensity observed with no blockages}{fig:modernOpticsLecture6:modernOpticsLecture6Fig1}{0.2}
\imageFigure{modernOpticsLecture6Fig2}{Intesity observed with blockage just above line of sight}{fig:modernOpticsLecture6:modernOpticsLecture6Fig2}{0.2}
...
\Psi(\Br) = \frac{A}{i\lambda} \frac{e^{ik (r_s + r)}}{r_s r} \int_{\text{aperature}} e^{i k f(\Br')} da'
\end{equation}

In the Fraunhofer limit (the far field) we had

\begin{equation}\label{eqn:modernOpticsLecture6:30}
k f \rightarrow ( \Bk_s - \Bk ) \cdot \Br',
\end{equation}

where $r \gg d^2/\lambda$, and $d$ is a typical aperature size.

Recalling that the exact expression for $f$ was
...
We'll now consider the \underline{Fresnel} limit where $\Bk_s = \Bk$, and
...
\Psi(\Br) 
= \frac{A}{i\lambda} \frac{e^{ik (r_s + r)}}{r_s r} \int_{\text{aperature}} e^{i k f(\Br')} da'
= \frac{A}{i\lambda} \frac{e^{ik (r_s + r)}}{r_s r} 
\frac{\lambda/2}{r_s^{-1} + r^{-1}}
\int_{\text{aperature}} e^{i \frac{\pi}{2} (u^2 + v^2} du dv
= 
...
Referring to figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig3}),

\imageFigure{modernOpticsLecture6Fig3}{Region of integration}{fig:modernOpticsLecture6:modernOpticsLecture6Fig3}{0.2}

let's do this integral.  Putting in our limits we have
...
We use \footnote{Can be computed by integrating $\cos(\pi v^2/2) + i \sin(\pi v^2/2)$, or using the gaussian integral result.}

\begin{equation}\label{eqn:modernOpticsLecture6:210}
\int_{-\infty}^\infty dv e^{i \frac{\pi}{2} v^2 } = 1 + i = \sqrt{2} e^{i \pi/4}
\end{equation}

FIXME: do this integral in the ways suggested as an exersize.
...
Parametrically plotting these we get the \underline{Cornu Spiral} as roughly illustrated in figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig4}).

\imageFigure{modernOpticsLecture6Fig4}{Cornu spiral (rough illustration)}{fig:modernOpticsLecture6:modernOpticsLecture6Fig4}{0.2}
...
\begin{dmath}\label{eqn:modernOpticsLecture6:310}
\boxed{
dl = dw
}
\end{dmath}
...
\begin{dmath}\label{eqn:modernOpticsLecture6:330}
\boxed{
\tan\theta = \frac{dy}{dx}
}
\end{dmath}
...
We've got $w \rightarrow \infty$, so that $C(w) = 1/2$ and $C(w) = 1/2$.  This gives us
...
In general our \underline{Intensity} is

\begin{dmath}\label{eqn:modernOpticsLecture6:430}
I 
= \Abs{\Psi}^2
= \Abs{\Psi_infty}^2 \inv{2} \left( 
\left( 
\inv{2} + C(w)
\right)^2
+
...
I_\infty \inv{2} \left( \inv{4} + \inv{4} \right) = I_\infty/4.
\end{dmath}

Other examples.

Diffraction from an edge $w = d$: figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig5}).

\imageFigure{modernOpticsLecture6Fig5}{Diffraction spectrum with partial blockage above line of sight (brutally rough illustration)}{fig:modernOpticsLecture6:modernOpticsLecture6Fig5}{0.2}

Poisson spot.  Poisson crafted a counter arguement for the wave theory of light stating that if it was true, then you should be able to see a spot behind a circular blockage, as if some of the light was going around the blockage.  This is illustrated in figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig6}), and can in fact be observed with the right setup.

\imageFigure{modernOpticsLecture6Fig6}{Poisson spot}{fig:modernOpticsLecture6:modernOpticsLecture6Fig6}{0.2}

Once we understand that light does in fact take all the paths, we can utilize this to build a \underline{Fresnel lens} by blocking selectively as illustrated very roughly in figure (\ref{fig:modernOpticsLecture6:modernOpticsLecture6Fig7}).

\imageFigure{modernOpticsLecture6Fig7}{Diffraction grating (imagined to have been constructed to focus x-rays)}{fig:modernOpticsLecture6:modernOpticsLecture6Fig7}{0.2}

A diffraction setup like allows us block all the portions of the phase that negatively interfere.  This can be used for example to focus x-rays.
