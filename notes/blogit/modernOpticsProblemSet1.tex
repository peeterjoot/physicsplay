...
\begin{enumerate}
\item[(a)] My interpretation of this problem is that are given the transfer matrix from class

\begin{dmath}\label{eqn:modernOpticsProblemSet1:1030}
M = 
\begin{bmatrix}
1 & 0 \\
-1/f & 1
...
We wish to relate this to the geometries of the lens system illustrated in figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1a}) that we expect this describes.

\imageFigure{modernOpticsProblemSet1Fig1a}{Thin paraxial lens with image in input and output conjugate planes}{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1a}{0.3} 

At the point $(0,a)$ with a horizontal input ray, our transfer matrix gives us

\begin{dmath}\label{eqn:modernOpticsProblemSet1:1050}
\begin{bmatrix}
a \\
-\beta
\end{bmatrix}
...
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1070}
-\beta = -\frac{a}{f} + 0,
\end{dmath}

or

\begin{dmath}\label{eqn:modernOpticsProblemSet1:1090}
\beta = \frac{a}{f}.
\end{dmath}

Similarily, when the output is horizontal at the point $(0, -b)$ we have

\begin{dmath}\label{eqn:modernOpticsProblemSet1:1110}
\begin{bmatrix}
-b \\
0
\end{bmatrix}
...
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1130}
0 = \frac{b}{f} - \theta
\end{dmath}

or
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1150}
\theta = \frac{b}{f}.
\end{dmath}

Because this describes a paraxial system, these angles approximate the tangents, so we can recast \ref{eqn:modernOpticsProblemSet1:1090} and \ref{eqn:modernOpticsProblemSet1:1150} as

\begin{subequations}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1170}
\tan \beta = \frac{a}{f}
\end{dmath}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1190}
\tan \theta = \frac{b}{f}.
\end{dmath}
\end{subequations}
...
\begin{subequations}
\label{eqn:modernOpticsProblemSet1:1210a}
\begin{equation}\label{eqn:modernOpticsProblemSet1:1210}
\tan \theta = \frac{a}{x} = \frac{b}{f} = \frac{a + b}{s}
\end{equation}
\begin{equation}\label{eqn:modernOpticsProblemSet1:1230}
...
\begin{subequations}
\label{eqn:modernOpticsProblemSet1:1250a}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1250}
\frac{b}{a} = \frac{f}{x}
\end{dmath}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1270}
\frac{b}{a} = \frac{x'}{f}
\end{dmath}
\end{subequations}

but we've seen that $x = s - f$ and $x' = s' - f$, so we have

\begin{equation}\label{eqn:modernOpticsProblemSet1:1290}
\frac{b}{a} = \frac{f}{s - f} = \frac{s' -f}{f}.
\end{equation}

Rearranging we have

\begin{equation}\label{eqn:modernOpticsProblemSet1:1310}
f^2 = (s' - f)(s - f) = s s' - f s - s' f + f^2,
\end{equation}

or

\begin{equation}\label{eqn:modernOpticsProblemSet1:1330}
s s' = f s + f s'.
\end{equation}

Dividing through by $f s s'$ we have

\begin{equation}\label{eqn:modernOpticsProblemSet1:1350}
\inv{f} = \inv{s'} + \inv{s},
\end{equation}
...
% FIXME: uncomment for reference in final notes compilation.
%
%In the class notes, the magnification of the thin lens system described by \ref{eqn:modernOpticsProblemSet1:1350} was given by
%
%\begin{equation}\label{eqn:modernOpticsProblemSet1:1370}
%m = -\frac{s'}{s}.
%\end{equation}
%
%Other than the negation, this is a logical definition, the ratio of the output size with respect to the input size.  I'm guessing that this is defined as negated because the image is inverted.  From \ref{eqn:modernOpticsProblemSet1:1270} we have
%
%\begin{equation}\label{eqn:modernOpticsProblemSet1:1390}
%\frac{b}{a} = \frac{x'}{f}.
%\end{equation}
%
%Dividing the last two equalities in \ref{eqn:modernOpticsProblemSet1:1210a} we have
%
%\begin{equation}\label{eqn:modernOpticsProblemSet1:1410}
%\frac{b}{a} = \frac{s'}{s}.
%\end{equation}
%
%We can conclude that the magnification, expressed in $x'$ and $f$ is
%
%\begin{equation}\label{eqn:modernOpticsProblemSet1:1430}
%m = -\frac{s'}{s} = \frac{x'}{f},
%\end{equation}
%
%and that figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1a}) with the distances as labelled describes the same system.  The remainder of the task is therefore algebraic.  We have
%
%\begin{dmath}\label{eqn:modernOpticsProblemSet1:1450}
%0 = -\inv{f} + \inv{s} + \inv{s'} = -\inv{f} + \inv{x + f} + \inv{x' + f}.
%\end{dmath}
%
%Multiplying through by $f(x + f)(x' + f)$ we have
%
%\begin{dmath}\label{eqn:modernOpticsProblemSet1:1470}
%0 
%= -(x + f)(x' + f) + f(x' + f) + f(x + f)
...
%or 
%
%\begin{dmath}\label{eqn:modernOpticsProblemSet1:1490}
%x x' = f^2.
%\end{dmath}
%

Let's consider the system as the compound action of three transfer matrices as illustrated in figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig2b1}).

\imageFigure{modernOpticsProblemSet1Fig2b1}{2b1: FIXME: CAPTION}{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig2b1}{0.2}
...
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1570}
\begin{bmatrix}
0 \\
\alpha
\end{bmatrix}
...
Here we refer to figure

\item[(d)]

Consider figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1d}).

\imageFigure{modernOpticsProblemSet1Fig1d}{1d: FIXME: CAPTION}{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1d}{0.2}

The transfer matrix $M = M_5 M_4 M_3 M_2 M_1$ for the system is

\begin{dmath}\label{eqn:modernOpticsProblemSet1:n}
M 
= M_5 M_4 M_3 M_2 M_1
=
\begin{bmatrix}
...
x(y) \sim L \ln\left( -\frac{2 y}{L} \right).
\end{dmath}
...
The Ray equation now takes the form

\begin{subequations}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:870}
\dds{} \left( 
\left( \theta(x) n_2 
+
...
= \Delta n \delta(x).
\end{dmath}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1010}
\left( \theta(x) n_2 
+
...
We also have a dependence between $s$, $x$ and $y$ ($ds^2 = dx^2 + dy^2$, or $1 = (dx/ds)^2 + (dy/ds)^2$) to deal with.  In particular, once we choose to not neglect the step discontinuity, we can no longer directly integrate as was done to find Snell's law from the y-component of the equation.
