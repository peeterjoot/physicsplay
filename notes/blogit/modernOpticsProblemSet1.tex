...
\left( \theta(x) n_1 
+
\theta(-x) n_2 \right) \dds{x}
= \Delta n \delta(x).
\end{dmath}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:1010}
\left( \theta(x) n_1 
+
\theta(-x) n_2 \right) \dds{y}
= 0,
\end{dmath}
\end{subequations}

We also have a dependence between $s$, $x$ and $y$ ($ds^2 = dx^2 + dy^2$) to deal with.  In particular, once we choose to not neglect the step discontinuity, we can no longer directly integrate as was done to find Snell's law from the y-component of the equation.

Can this system, or one for which a limiting form of the unit step and delta functions is used (i.e. the sinc representation of the delta function), even be solved exactly?
