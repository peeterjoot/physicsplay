...
Let's consider the system as the compound action of three transfer matrices as illustrated in figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig2b1}), this time labelling the figure in terms of the variables for this problem.
...
Here we refer to figure (\ref{fig:modernOpticsProblemSet1:modernOpticsProblemSet1Fig1c}), this time considering no ray that passes the focus past the lens.  Our system transfer matrix, given the reduced free propagation distantce past the lens is
...
While we can still directly integrate the y-component equation once (as done above), our original assumed parameterization of $\Br(s) = s(\cos\theta, \sin\theta)$ looses it's convienient form since we now have $\theta = \theta(s)$ in the neighbourhood of the origin.  Once we choose to not neglect the step discontinuity, we have a coupled, much more difficult, system to deal with.
