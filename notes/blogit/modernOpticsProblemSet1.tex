...
\item[(f)]
These plots were included above, although matching the limiting curves was found to be fairly range dependent.  A Mathematica notebook with Manipulate expressions allowing the ranges of the plots to be changed can be found at \href{https://raw.github.com/peeterjoot/physicsplay/master/notes/phy485/mathematica/modernOpticsProblemSet1.cdf}{http://goo.gl/OumcI}.
\end{enumerate}
}
...
The y-component of the Ray equation with this chosen parameterization is therefore reduced to just

\begin{dmath}\label{eqn:modernOpticsProblemSet1:730}
\left\{
\begin{array}{l l}
\dds{} \left( n_2 \sin\theta_2 \right) = 0 & \quad \mbox{if $x > 0$} \\
\dds{} \left( n_1 \sin\theta_1 \right) = 0 & \quad \mbox{if $x < 0$} \\
\end{array}
\right.
\end{dmath}

Integrating once we have
\begin{dmath}\label{eqn:modernOpticsProblemSet1:750}
\left\{
\begin{array}{l l}
n_2 \sin\theta_2 = \text{constant} & \quad \mbox{if $x > 0$} \\
n_1 \sin\theta_1 = \text{constant} & \quad \mbox{if $x < 0$} \\
\end{array}
\right.
\end{dmath}

A match at the boundary $x = 0$ requires the same constant so that we have Snell's law
...
\item[(b)] 
We can produce a condractory result with a similar argument.  With the same arguments that led to \ref{eqn:modernOpticsProblemSet1:730} above, we would find that the x-component of the Ray equation takes the form

\begin{dmath}\label{eqn:modernOpticsProblemSet1:790}
\left\{
\begin{array}{l l}
\dds{} \left( n_2 \cos\theta_2 \right) = 0 & \quad \mbox{if $x > 0$} \\
\dds{} \left( n_1 \cos\theta_1 \right) = 0 & \quad \mbox{if $x < 0$} \\
\end{array}
\right.
\end{dmath}

Integrating once as above and matching the constant would lead to the conclusion that

\begin{dmath}\label{eqn:modernOpticsProblemSet1:810}
n_2 \cos\theta_2 = n_1 \cos\theta_1,
\end{dmath}

which contradicts Snell's law.  This conclusion isn't valid because we have avoided the origin, where the index of refraction is not continuous.  What is missing is proper treatment of this step discontinuity.  To frame this properly, let's express the index of refraction a bit more precisely.  That is
...
\spacegrad n = \Delta n \delta(x).
\end{dmath}

Without this avoidance of the origin the Ray equation is

\begin{subequations}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:870}
\theta(x)
\dds{} \left( n_2 \cos\theta_2 \right) 
+
\theta(-x)
\dds{} \left( n_1 \cos\theta_1 \right) 
= \Delta n \delta(x)
\end{dmath}
\begin{dmath}\label{eqn:modernOpticsProblemSet1:890}
\theta(x)
\dds{} \left( n_2 \sin\theta_2 \right) 
+
\theta(-x)
\dds{} \left( n_1 \sin\theta_1 \right) 
= 0
\end{dmath}
\end{subequations}

This complete system, or one that uses a limiting form of the delta function, such as the $\sinc$ representation, is what has to be solved.  Clearly a careful treatment must result in the conclusion that \ref{eqn:modernOpticsProblemSet1:870} produces no contribution either away from or at the origin $x = 0$.
