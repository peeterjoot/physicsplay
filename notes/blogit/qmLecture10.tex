...
\beginArtNoToc
\generatetitle{PHY1520H Graduate Quantum Mechanics.  Lecture 10: XXX.  Taught by Prof.\ Arun Paramekanti}
%\chapter{XXX}
...
\paragraph{Dirac scattering off a potential step}
...
The \( r_1 \) and \( r_2 \) branches are sketched in 
...
The non-relativatistic case is sketched in 

F3

For the relativatistic case we must consider three different cases, sketched in

F4

To consider this analyitically we want to ensure that the current across the barrier is conserved (no particles are lost), as sketched in
...
-\frac{\Hbar^2}{2m} \lr{ \PD{x}{\Psi}_\txtR - \PD{x}{\Psi}_\txtL } = 0.
\end{equation}

We have to match 

For the relativisitic case
...
Let's look for a solution for the \( E + m c^2 > V_0 \) case on the right branch, as sketched in
...
R = \frac{1 + \cos( 2 \theta_1 - 2 \theta_w) }{1 - \cos( 2 \theta_1 - 2 \theta_w)}.
}
