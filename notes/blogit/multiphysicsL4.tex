...
\item all elements for which it is impossible or inconvieient to write \( i = f(v_1, v_2) \).

Imagine, for example, that we have a component illustrated in 
...
v_1 - v_2 = 3 i^2
\end{equation}
\item any current which is controlling dependent sources, as in
...
Writing out our Gaussian transformation in the form \( U \overbar{x} = b \) we have
...
Using this \textAndIndex{LU} decomposition is generally superior to standard Gaussian elimination, since we can use this for many different \(\overbar{b}\) vectors using the same amount of work.
...
We can now solve \( L y = b \), using substitution for \( y_1 \), then \( y_2 \), and finally \( y_3 \).  This is called \textAndIndex{forward substitution}.
