...
In this section, the form of Maxwell's equations to be used are expressed in terms of \( \bcE \) and \( \bcH \), assume linear media, and do not assume a phasor representation
...
%Repeating the divergence calculation of the reciprocity derivation in the time domain shows that the reciprocity theorem as stated also has a dependency on single frequency sources and fields
...
\begin{dmath}\label{eqn:energyMomentumWithMagneticSources:640}
\bcE^{(b)} \cdot \partial_t \bcE^{(a)} 
-\bcE^{(a)} \cdot \partial_t \bcE^{(b)} 
=
\inv{2} \Real
