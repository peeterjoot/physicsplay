...
Introducing the \textAndIndex{nodal matrix} \(G\), we write this as
...
where \(\overbar{I}_B\) was the branch currents, \(A\) was the \textAndIndex{incidence matrix}, and \(\alpha = \begin{bmatrix}\inv{R_1} & & \\ & \inv{R_2} & \\ & & \ddots \end{bmatrix} \).

The \textAndIndex{stamp} can be observed in the multiplication of the contribution for a single resistor, where we see that the incidence matrix has the form \( G = A \alpha A^\T \)
...
This is the method that we find in software such as \textAndIndex{spice}.  
