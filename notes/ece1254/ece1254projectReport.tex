...
\IEEEPARstart{T}{he} purpose of this project is to extend the circuit solution system developed in ece1254 to non linear circuits with periodic single frequency sources, specifically adding the capability to solve diode models to the RLC solution capacities developed previously.  Anharmonic effects will be ignored, and it will be assumed that only periodic outputs with harmonics that are multiples of the input frequency will be observed.  Such a solution will be modelled using the \emph{Harmonic Balance} method described in \citep{giannini2004NonlinearMicrowaveCircuitDesign}.  This method models the time variation of the inputs and outputs as discrete time Fourier series, expressing them in a matrix form that can be handled readily in software.
...
\begin{bmatrix}
 ...
 \alpha^{-N N }        &  \alpha^{-  (1-N) N }                & . & \alpha^{-  \lr{N-1} N }          &  \alpha^{-  N N } \\
\end{bmatrix},
\end{equation}
...
The dimensions of these matrices, with \( R \) equal to the number of unknowns, are
...
The matrix \( \Bu(t) \) contains has a non-zero value for each source frequency source in the system.  For example, if there is a DC source, and one source at twice the fundamental frequency, this would be
...
\begin{IEEEbiographynophoto}{Michael Royle}
Hi Mike.  anything you want to say about yourself?
\end{IEEEbiographynophoto}
%\begin{IEEEbiographynophoto}{Peeter Joot}
%Part time ECE electromagnetics M.Eng student, Systems programmer with IBM DB2 since 1997.
%\end{IEEEbiographynophoto}
