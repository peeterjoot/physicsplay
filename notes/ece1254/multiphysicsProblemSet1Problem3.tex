...
\PDSq{x}{T(x)} = \frac{\kappa_a}{\kappa_m}\lr{ T(x) - T_0 } - \frac{H(x)}{\kappa_m}
\end{equation}
...
Use your Matlab simulator to numerically solve the above Poisson equation for \( T(x), x \in [0,1] \) , given \( H(x) = 50 \sin^2(2 \pi x) \) for \( x \in [0,1], \kappa_a = 0.001, and \kappa_m = 0.1 \). In addition, assume the ambient air temperature is \( T_0 = 400 \), and \( T(0) = 250 \) and \( T(1) = 250 \). The boundary conditions at \( x = 0 \) and \( x = 1 \) model heat sink connections to a cool metal cabinet at both ends of the package. That is, it is assumed that the heat sink connections will insure both ends of the package are fixed at near room temperature.
...
\makesubproblem
{
	Now use your simulator to numerically solve the above equation for \( T(x), x \in [0,1]\), given \( H(x) = 50 \) for \( x \in [0,1], \kappa_a = 0.001 \) , and \( \kappa_m = 0.1\). In addition, assume the ambient air temperature is \(T_0 = 400\), and there is not heat flow at both ends of the bar. The zero heat flow boundary condition at \( x = 0 \) and \( x = 1 \) implies that there are no heat sinks at the ends of the package. Since heat flow is given by 
...
To discretize the equation, let's divide the interval into \( N - 1 \) segments, as illustrated in \cref{fig:ps1p3:ps1p3Fig8}.

\imageFigure{../../figures/ece1254/ps1p3Fig8}{Discretization intervals}{fig:ps1p3:ps1p3Fig8}{0.05}
...
With \( Q^i = \lr{T^i - T^{i-1}}/{\Delta x} \) we have an equivalence to the circuit element \( I = \Delta V/R \).  This analogy makes sense physically since a smaller interval length has less resistance to heat flow.

The linearized Poisson equation at interior nodes \( 1 < i < N \) is

\begin{equation}\label{eqn:multiphysicsProblemSet1Problem3:40}
Q^i - Q^{i-1} - \frac{\kappa_a \Delta x}{\kappa_m} \lr{ T^i - T_0 } + \frac{H^i \Delta x}{\kappa_m} = 0.
\end{equation}

Identification of the \( - \kappa_a \Delta x/\kappa_m \lr{ T^i - T_0 } \) term as a current \( I = \Delta V/R \) (with temperatures as voltages) means that we can identify \( R \sim \kappa_m /(\kappa_a \Delta x) \).  
%This allows us to create the equivalent circuit sketched in \cref{fig:ps1p3:ps1p3Fig9}.
%
%\imageFigure{../../figures/ece1254/ps1p3Fig9}{Equivalent circuit}{fig:ps1p3:ps1p3Fig9}{0.3}
%
At the endpoints things are a slightly different.  At \( x^0, x^{N+1} \) respectively, we have

\begin{equation}\label{eqn:multiphysicsProblemSet1Problem3:60}
\begin{aligned}
-q^0 + Q^1 - \frac{\kappa_a \Delta x}{\kappa_m} \lr{ T^1 - T_0 } + \frac{H^1 \Delta x}{\kappa_m} &= 0 \\
-q^1 -Q^{N-1} - \frac{\kappa_a \Delta x}{\kappa_m} \lr{ T^{N} - T_0 } + \frac{H^{N} \Delta x}{\kappa_m} &= 0. \\
\end{aligned}
\end{equation}

At these nodes is only one current term, but we also have to model the heat sinks.  This equivalent circuit containing all these elements is sketched in 
...
\begin{itemize}
\item 
\href{https://raw.github.com/peeterjoot/matlab/master/ece1254/ps1/generateHeatNetlist.m}{generateHeatNetlist.m}
\item 
\href{https://raw.github.com/peeterjoot/matlab/master/ece1254/ps1/plotHeatQ.m}{plotHeatQ.m}
\end{itemize}
...
%\imageFigure{../../figures/ece1254/ps1p3Fig12n30}{Plot of numerical solution for \( n = 30 \)}{fig:ps1p3:ps1p3Fig12n30}{0.3}
\imageFigure{../../figures/ece1254/ps1p3Fig12n60}{Plot of numerical solution for \( n = 60 \)}{fig:ps1p3:ps1p3Fig12n60}{0.3}

This plot was generated with a call to \( \mathrm{plotHeatQ}(60, 1, 0) \).

\makeSubAnswer{ }{multiphysics:problemSet1:3c}

We'd like to keep the variation of the \( H_i \) term small.  That difference between two adjacent intervals scales with the squared sine
...
This is biggest when the sine is near unity.  If we want the heat terms to differ by no more than a fraction \( f \) we can determine \( \Delta x \) from
...
For example, if we desire \( f = 1/10 \) we can pick \( N \approx 60 \), which was what was used in the plot above.

\makeSubAnswer{ }{multiphysics:problemSet1:3d}

%%%If we have constraints on the heat flow, we can express those by integrating our PDE once
%%%
%%%\begin{equation}\label{eqn:multiphysicsProblemSet1Problem3:140}
%%%\PD{x}{T}
%%%= 
%%%\int_{x' = a}^x dx' \lr{ \frac{\kappa_a}{\kappa_m} \lr{ T(x') - T_0 } - \frac{H(x')}{\kappa_m} }.
%%%\end{equation}
%%%
%%%We see that the zero constraint at \( x = 0 \) can be satisfied by setting the integration boundary constant \(a = 0\).  With \( H(x') \) is constant for this part of the problem that condition is
%%%
...
%%%T(x')
%%%- 
%%%\frac{\kappa_a}{\kappa_m} T_0 - \frac{H}{\kappa_m},
%%%\end{equation}
%%%
%%%or
%%%
...
%%%\sum_{i = 1}^N \Delta x T^i = T_0 + \frac{H}{\kappa_a}.
%%%\end{equation}
%%%
%%%That is
%%%
%%%\begin{equation}\label{eqn:multiphysicsProblemSet1Problem3:220}
%%%\sum_{i = 1}^N \lr{ T^i - T_0 } = N \frac{H}{\kappa_a}.
%%%\end{equation}
%%%
...
%%%T^i - T_0 = -\sum_{i = 1}^{N-1} \lr{ T^i - T_0 } + N \frac{H}{\kappa_a}.
%%%\end{equation}
%%%
...
%%% Try 2:
%%% This generates a numerically unstable result.  Changing N keeps flipping the orientation of the temperature curve, and the desired zero derivative at the endpoints isn't there.  Instead we have a zero derivative in the interior.
%%%
...
\imageFigure{../../figures/ece1254/ps3dFig15WithFlatSource}{Temperature distribution with uniform heat source and no heat sinks}{fig:ps3d:ps3dFig15WithFlatSource}{0.3}
...
\( \mathrm{plotHeatQ}(60, 0, 0) \).

This is a somewhat boring heat source, and leaves room to doubt that the boundary constraints are matched by more than luck.  The sinusoidal heat distribution from \ref{multiphysics:problemSet1:3a}, shows a more interesting temperature curve.  We also see that this has the desired zero heat flow constraints.  Such a plot is given in \cref{fig:ps3d:ps3dFig16WithSineSource}, and was generated with a call to \( \mathrm{plotHeatQ}(60, 0, 1) \).

\imageFigure{../../figures/ece1254/ps3dFig16WithSineSource}{Sinusoidal heat source with zero heat flow constraint.}{fig:ps3d:ps3dFig16WithSineSource}{0.3}

\makeSubAnswer{ }{multiphysics:problemSet1:3f}

With the uniform heating distribution \( H(x) = 50 \) and no heat sinks, we expect the heat in the bar to have a uniform steady state distribution, and this is what we see above in the visualization of the numerical solution.  We also see that the temperature of the bar raises considerably above the ambient temperature.  Eventually the \( T(x) - T_0 \) term in the PDE gets big enough that more significant leakage into the air occurs despite the small ratio of \( \kappa_a/\kappa_m \), limiting the steady state temperature of the bar.
}
