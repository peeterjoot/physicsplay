...
1.  Suppose $P$ is a vector, $P = \gamma^k P_k$, with $P_k$ scalars, then we would have
...
This is a vector, whereas your expression would be a scalar, so it does not look like what you are after.

2.  Suppose $P$ is a bivector, $P = (\gamma^k \wedge \gamma^m) P_{km}$, then the commutator will be a bivector not a vector.  This is a messier expansion (perhaps there is a clever way to do it, but I do not know what it is).
...
This looks something like what you are after, but has one too many non-free indexes.  That leaves just a trivector bivector product to consider ... but I had rather go for a hot tub right now then consider that case;)
