...
%%\hat{L}_{x,y} \ket{a,b} = \sum_{b'} \calA^{x,y}_{a; b, b'} \ket{a,b'},
%%\end{dmath}
%%
%%by applying 
%%
...
The convention is to write \( m \) for the \( \hat{L}_z \) eigenvalue (the magnetic quantum number), and \( j \), the azimthual quantum number, to describe the \( j(j+1) \) eigenvalue of the \( \hat{\BL}^2 \) operator. 
