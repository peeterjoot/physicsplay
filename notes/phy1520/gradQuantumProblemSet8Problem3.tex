...
Find the perturbed eigenstate and the corresponding energy shifts upto \( O(\lambda^2) \) for the ground state.  Ignore terms of \( O(\lambda^3) \).
...
The LHS is also zero as desired, showing that \cref{eqn:gradQuantumProblemSet8Problem3:140} is the desired pertubation relationship.  
...
In \nbref{ps8:2dHarmonicOscillatorOperators.nb} (attached), for
...
a & b \\
b & a
\end{bmatrix} \) are just \( a \pm b \), the energy splitting to first order for these first two degenerate states (see notebook) is
...
While the \( x y \) pertubation potential left the ground state untouched, it is responsible for the energy level splitting in the degenerate states \( \ket{1,0}, \ket{1,0} \).
}
