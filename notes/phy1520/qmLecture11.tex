...
%FIXME: show that \( \hat{\Pi}^\dagger \hat{p} \hat{\Pi} = - \hat{p} \).
...
where \( V(\hat{x}) \) is even, or \( \antisymmetric{V(\hat{x})}{\hat{\Pi} } = 0 \).  The squared momentum commutes with the parity operator
