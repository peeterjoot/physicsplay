...
%
\makeproblem{Aharonov Bohm effect}{gradQuantum:problemSet3:1}{ 
...
Next, imagine a thin solenoidal flux \( \Phi \) being placed between the two slits, so that electron paths which encircle the flux once will pick up an Aharonov Bohm phase \( e \Phi/h \).
Compute the resulting shift in the interference pattern on the screen.
Show that when the flux \( \Phi \) is increased from \( 0 \rightarrow h/e \), the interference pattern shifts by exactly one fringe, so the new pattern appears the same as the old.
