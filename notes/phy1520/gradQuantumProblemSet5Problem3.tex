...
Because the eigenvalues of the parity operator can only be \( \pm 1 \), if such a simultaneous eigenstate exists in the momentum basis, it must have a \( p = 0 \) eigenvalue.  To see this explicitly, assume there is some value \( p \) for which \( \ket{p} \) is a simultaneous eigenstate of \( \hat{\Pi} \) and \( \hat{p} \).  First consider
