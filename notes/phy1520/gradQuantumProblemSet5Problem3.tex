...
The action of the commutator on this state is then
...
The concrete example of the parity operator \( \hat{\Pi} \) and the momentum operator \( \hat{p} \) demonstrates this.
Because the eigenvalues of the parity operator can only be \( \pm 1 \), if such a simultaneous eigenstate exists in the momemtum basis, it must have a \( p = 0 \) eigenvalue.  To see this explicitly, assume there is some value \( p \) for which \( \ket{p} \) is a simultaneous eigenstate of \( \hat{\Pi} \) and \( \hat{p} \).  First consider
...
The only way this can be satisifed is when the eigenvalue of this state is \( p = 0 \).
}
