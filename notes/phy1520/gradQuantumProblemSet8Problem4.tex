...
\( \mu_\txtp = e/2m_\txtp \). The large nuclear mass ensures \( \mu_\txte/\mu_\txtp \sim 2000 \), so let us simply set \( \mu_\txtp = 0\). For convenience, you can club \( B g_\txte \mu_\txte \rightarrow B_{\textrm{eff}} \) so the Hamiltonian becomes
...
This is demonstrated in \nbref{ps8:PauliMatrixSpinOperators.nb} (attached).
...
For the strong field limit, we can flip the problem, and consider \( A \BS_\txte \cdot \BS_\txtp \) to be a pertubation of an initial Hamiltonian \( H_0 = -B_{\textrm{eff}} S^z_\txte \).  The diagonalization of that pertubation is just \cref{eqn:gradQuantumProblemSet8Problem4:200} so the first order energy shifts are
