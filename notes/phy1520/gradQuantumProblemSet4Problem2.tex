...
Now consider the matrix element of this commutator with respect to a position basis wavefunction \( \bra{x} \antisymmetric{H}{\hatcalP_{\textrm{Dirac}}} \ket{\Psi} \).  To compute this we must first understand the behaviour of the anticommutators of scalar operators \( m, \hat{p} \) with \( \pi \).  That is
...
=
\begin{bmatrix}
c \hat{p} f -i m(x) c^2 f \\
m(x) c^2 f + i c \hat{p} f,
\end{bmatrix}
\end{dmath}
