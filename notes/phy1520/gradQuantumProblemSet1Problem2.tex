...
\( S_{\txtv \txtN} = -\Tr(\rho_\txtR \ln \rho_\txtR ) \) where \( \rho_\txtR \) is the reduced
...
where the zero logarithm is interpretted in the limit \( \lim_{x \rightarrow 0} -x \ln x = 0 \).
...
In a diagonal representation of the reduced density matrix, the \( n \rightarrow 1 \) Renyi entropy 
...
will have a numerator sums to unity, so it's evaluation requires the dividing two nearly zero quantities.  To evaluate this, l'H\^{o}pital's rule can be used.  That is
