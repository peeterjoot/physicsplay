...
%\makeoproblem{Aharanov Bohm effect}{gradQuantum:problemSet3:3}{\citep{sakurai2014modern} pr. 2.28, phy1520 2015 ps3.3}
...
A magnetic field that is isolated to the hole of the cavity is described by the peicewise vector potential
...
= \inv{2 m} \lr{ -i \Hbar \spacegrad - e \frac{B}{2} \rho_a^2 \frac{\phicap}{\rho} }^2
= \inv{2 m} \lr{ 
- \Hbar^2 \spacegrad^2 
+ i \Hbar e \frac{B}{2} \rho_a^2 \lr{ \spacegrad \cdot \frac{\phicap}{\rho} + \frac{\phicap}{\rho} \cdot \spacegrad }
+ \lr{ \frac{e B \rho_a^2}{2 \rho} }^2
}
\end{dmath}
...
+ \frac{i \Hbar e B \rho_a^2}{2 m \rho^2} \partial_\phi \psi 
+
\inv{2m}
\lr{ \frac{e B \rho_a^2}{2 \rho} }^2 \psi.
\end{dmath}
...
+ \frac{i \Hbar e B \rho_a^2}{2 m \Phi} \partial_\phi \Phi 
+
\inv{2m}
\lr{ \frac{e B \rho_a^2}{2} }^2.
\end{dmath}

Now set 

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:800}
\frac{-\Hbar^2}{2 m \Phi} \partial_{\phi\phi} \Phi + \frac{i \Hbar e B \rho_a^2}{2 m \Phi} \partial_\phi \Phi
= E_\phi.
\end{dmath}
...
E_\phi 
=
\frac{-\Hbar^2}{2 m } (-i \nu)^2 + \frac{i \Hbar e B \rho_a^2}{2 m \Phi} (i\nu)
=
\frac{\Hbar^2 \nu^2}{2 m } - \frac{\Hbar e B \rho_a^2}{2 m} \nu
=
\frac{\Hbar^2}{2 m} \lr{ \nu^2 - \frac{e B \rho_a^2}{\Hbar} \nu } 
=
\frac{\Hbar^2}{2 m} 
\lr{
\lr{ \nu - \frac{e B \rho_a^2}{2 \Hbar} }^2 - \lr{ \frac{e B \rho_a^2}{2 \Hbar} }^2
}.
\end{dmath}
...
\frac{\Hbar^2}{2 m} 
\lr{
\lr{ \nu - \frac{e B \rho_a^2}{2 \Hbar} }^2 - \cancel{\lr{ \frac{e B \rho_a^2}{2 \Hbar} }^2}
}
=
\frac{-\Hbar^2}{2 m} \lr{ 
\frac{\rho}{P} \partial_\rho \lr{ \rho \partial_\rho P } 
...
\lr{ \nu - \frac{e B \rho_a^2}{2 \Hbar} }^2 } P = 0.
\end{dmath}

With

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:880}
\nu' = \nu - \frac{e B \rho_a^2}{2 \Hbar},
\end{dmath}
...
With \( \theta = \ifrac{e B \rho_a^2}{2 \Hbar} \), \cref{eqn:gradQuantumProblemSet3Problem3:820} takes the form

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:960}
\frac{2 m E_\phi}{\Hbar^2} = \lr{ \nu - \theta }^2 - \theta^2,
\end{dmath}

which has zeros for any combination \( \nu = 2 \theta \), or

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:980}
\frac{\nu}{2} = \frac{e B \rho_a^2}{2 \Hbar},
\end{dmath}

Shuffling terms, this is

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:1000}
\frac{\pi nu \Hbar}{e} = B \pi \rho_a^2.
\end{dmath}

Noting that \( \nu \) is an integer, this is the desired result except for a pesky factor of two difference.  In the rough draft of my solution, I actually had the desired result, but realized I had dropped a factor of two elsewhere.  i.e., I expanded

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:1020}
\spacegrad \cdot \BA + \BA \cdot \spacegrad = \BA \cdot \spacegrad = \frac{ B \rho_a^2}{2 \rho^2 }\partial_\phi.
\end{dmath}

However, this is an operator equation and the gradient should be operating on both \( \BA \) plus the wave function that follows.  That is

\begin{dmath}\label{eqn:gradQuantumProblemSet3Problem3:1040}
\lr{ \spacegrad \cdot \BA + \BA \cdot \spacegrad } \psi
=
\spacegrad \cdot \lr{ \BA \psi} + \BA \cdot \spacegrad \psi
=
\lr{ \psi \spacegrad \cdot \BA + \BA \cdot \spacegrad \psi} + \BA \cdot \spacegrad \psi
=
\psi \spacegrad \cdot \BA + 2 \BA \cdot \spacegrad \psi
=
\frac{ B \rho_a^2}{\rho^2 }\partial_\phi \psi.
\end{dmath}

If I hadn't corrected that mistake, I'd still have the desired factor of two.  My conclusion is that there is either an error in the statement of this problem (also 2.28 of the text), or I have dropped a factor of two somewhere else (and cannot find such an error.)
}
