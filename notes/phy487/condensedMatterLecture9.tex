...
\paragraph{Exam hint.} He'll expect us to look at a phonon diagram and see where the density of states is high or low.
...
\left\{
\begin{array}{l l}
C_L q & \quad \mbox{longitudinal acoustic} \\
C_T q & \quad \mbox{transverse acoustic (two of these)} 
\end{array}
\right.
\end{dmath}
...
{$q$ space surface area element $4 \pi q^2$}
}{\frac{d\omega}{dq}}
d\omega
=
\int
...
Here we sum over the two transverse and the single longitudinal state, and refer back to \eqnref{eqn:condensedMatterLecture9:340} introduce the $C_L$ and $C_T$ factors.
...
Define the Debye frequency $\omega_D$ by

\begin{dmath}\label{eqn:condensedMatterLecture9:380}
\int_0^{\omega_D} Z(\omega) d\omega = 3 r N
\end{dmath}
...
3 r N
=
\frac{V}{ 2 \pi^2} \lr{ \inv{C_L^3} + \frac{ 2}{C_T^3} } \int_0^{\omega_D} \omega^2 d\omega 
=
\frac{V}{ 2 \pi^2} \lr{ \inv{C_L^3} + \frac{ 2}{C_T^3} } \inv{3} \omega_D^3,
\end{dmath}
...
\myBoxed{
\frac{V}{ 2 \pi^2} \lr{ \inv{C_L^3} + \frac{ 2}{C_T^3} } \omega_D^3 = 9 r N.
}
\end{dmath}
