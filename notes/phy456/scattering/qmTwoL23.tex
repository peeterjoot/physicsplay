...
Consider scattering off of a positive potential as depicted in figure (\ref{fig:qmTwoL23:qmTwoL23fig1})
\imageFigure{figures/qmTwoL23fig1}{Radially bounded potential}{fig:qmTwoL23:qmTwoL23fig1}{0.4}
...
\imageFigure{figures/qmTwoL23fig2}{Scattering cross section}{fig:qmTwoL23:qmTwoL23fig2}{0.3}
...
For \ref{eqn:qmTwoL23:420} we are referred to any ``good book on electromagnetism'' for details.  I thought that perhaps the weighty \citep{jackson1975cew} would be to be such a book, but it also leaves out the details.  In \S 16.1 the spherical Bessel and Neumann functions are related to the plain old Bessel functions with
...
This does give us our desired identities, but there is no hint in the text how one would derive \ref{eqn:qmTwoL23:460} from the power series that was computed by solving the Bessel equation.
...
It does not matter that we started with $a_0 \ne 0$.  If we instead start with $a_1 \ne 0$ we find that we must have $r = l-1, -l-2$, so end up with exactly the same functional form as \ref{eqn:qmTwoL23:1080}.  It ends up slightly simpler if we start with \ref{eqn:qmTwoL23:1080} instead, since we now know that we do not have any odd powered $a_k$'s to deal with.  Doing so we find
...
we have $f = j_l(x)$ as given in \ref{eqn:qmTwoL23:880}.
