...
\date{ April 2d, 2009.  Last Revision: $Date: 2009/04/25 02:53:12 $ }
...
&= \int \psi^\conj O ( \lambda \psi + \epsilon ) \\
&= \lambda \expectation{O} + \int \psi^\conj O \epsilon \\
&= \lambda \expectation{O} + \int \epsilon^\conj O \psi \\
&= \lambda \expectation{O} + \int \epsilon^\conj (\lambda \psi + \epsilon) \\
&= (\expection{O} - \int \psi^\conj \epsilon )\expectation{O} + 
\lambda \int \epsilon^\conj \psi 
+ \int \epsilon^\conj \epsilon 
\\
&= \expection{O}^2 - \int \psi^\conj \epsilon \expectation{O} + ( \int \psi^\conj \epsilon - \expectation{O}) \int \epsilon^\conj \psi + \int \epsilon^\conj \epsilon 
\\
\end{align*}
