...
\date{ March 21, 2009.  Last Revision: $Date: 2009/04/29 14:12:13 $ }
...
(\grad^2 + m^2 + e^2 A^2 ) \phi &= -i e (\grad \cdot A) \phi^\conj - i e A \cdot (\grad \phi^\conj) \\
(\grad^2 + m^2 + e^2 A^2 ) \phi^\conj &= i e (\grad \cdot A) \phi + i e A \cdot (\grad \phi) 
\end{align}
...
(having guessed slightly wrongly initially to expand $\phi^\conj \grad \phi - \phi \grad \phi^\conj$).
...
\left( m^2 + e^2 A^2 + \grad^2 \right) \phi
&=
- i e (\grad \cdot A + 2 A \cdot \grad) \phi
\\
\end{align*}

FIXME: this isn't consistent with the original result \ref{eqn:complexifiedSolution} based on the two real fields.  Where's the error(s)?
