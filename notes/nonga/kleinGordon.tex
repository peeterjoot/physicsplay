...
\date{ March 27, 2009.  Last Revision: $Date: 2009/03/30 00:56:38 $ }
...
\subsubsection{ Gauge covarient derivative. }

Observe that in \ref{eqn:gaugeInvariant}, we also have the
...
%Equivalently, one could group the conjugates a bit more naturally.
%That is
%
%\begin{align*}
%\LL
%&=
%(\Dslash \psi) \cdot (\Dslash \psi)^\conj
%+ \frac{m^2 c^2}{\hbar^2} \psi {\psi}^\conj
%\end{align*}

This is tidier, but comes with the cost of hiding even more in the notation.
%I believe this should result in the same field equations, but this should
%be verified.
...
then the net effect of the gauge variation is to add a $A^\mu A_\mu$ vector product (a Lorentz invariant) to the mass term.
