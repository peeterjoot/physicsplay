...
\begin{align}\label{eqn:klein_gordon:withConstant}
...
Now, that wasn't really any easier to do it that way.  Mechanically plugging into the Euler-Lagrange equations would have gotten us there faster, but Feynman's method from the Lectures \citep{feynman1963flp} is pretty cool for its simplicity.
...
Satisfies this relativistic scalar wave equation \ref{eqn:klein_gordon:withConstant}.
...
seems somewhat problematic logically, since computing the derivative, as done in \citep{gabook:PJprobCurrent}
...
\begin{align}\label{eqn:klein_gordon:pcons}
\rho &= \psi\psi^\conj \\
...
Also see here that this is exactly the result that was found in \citep{gabook:PJNoethersField} where the Noether equation for the conserved current
